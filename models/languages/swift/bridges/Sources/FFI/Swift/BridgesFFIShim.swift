...
@_exported import Vision_Classification_FastViT
...
public func agentbridge_model_create(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        let _ = configJson.map { String(cString: $0) }
...
public func agentbridge_audio_whisper_transcribe(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        outSegmentsJson.pointee = try MemoryManager.createJSONString(from: result.segments)
...
public func agentbridge_vision_yolo_detect(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        // Use semaphore to handle async call in sync context
        ...
        Task {
            do {
                let result = try await model.detectObjects(
                    in: data,
        ...
        guard let result = detectionResult else {
            throw BridgeError.unsupportedOperation("Failed to get detection result")
        }

        // Convert detections to JSON-serializable format
        let detectionDicts = result.detections.map { detection in
            [
        ...
        let jsonString = String(data: jsonData, encoding: .utf8) ?? "[]"
        outDetectionsJson.pointee = MemoryManager.createCString(from: jsonString)
        outDetectionCount.pointee = Int32(result.detections.count)

        return 0
    }, errorPtr: outError) ?? 1
}

@_cdecl("agentbridge_vision_fastvit_create")
public func agentbridge_vision_fastvit_create(
    _ modelPath: UnsafePointer<CChar>,
    _ outModelRef: UnsafeMutablePointer<ModelRef>,
    _ outError: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>
) -> Int32 {
    return MemoryManager.executeFFIOperation({
        ...
        // Create FastViT model instance
        let model = try FastViTBridge(modelPath: path)
...
@_cdecl("agentbridge_vision_fastvit_classify")
public func agentbridge_vision_fastvit_classify(
    _ modelRef: ModelRef,
    _ imageData: UnsafePointer<UInt8>,
    _ dataLength: Int32,
    _ topK: Int32,
    _ outLabelsJson: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>,
    _ outError: UnsafeMutablePointer<UnsafeMutablePointer<CChar>?>
) -> Int32 {
    return MemoryManager.executeFFIOperation({
        guard let model = modelHandleRegistry.get(modelRef) as? FastViTBridge else {
            throw BridgeError.invalidInput("Invalid FastViT model reference")
        }
        ...
        Task {
            do {
                let result = try await model.classify(
                    imageData: data,
                    topK: Int(topK)
                )
        ...
        guard let result = classificationResult else {
            throw BridgeError.unsupportedOperation("Failed to get classification result")
        }

        outLabelsJson.pointee = try MemoryManager.createJSONString(from: result)
...
public func agentbridge_system_get_metrics(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        let jsonString = String(data: jsonData, encoding: .utf8) ?? "{}"
        outMetrics.pointee = MemoryManager.createCString(from: jsonString)
...
public func agentbridge_system_profile_stop(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        let jsonString = String(data: jsonData, encoding: .utf8) ?? "{}"
        outReport.pointee = MemoryManager.createCString(from: jsonString)
...
public func agentbridge_model_download(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        let asset = try manager.getCachedModel(identifier: id, channel: modelChannel)
        guard let asset = asset else {
            throw BridgeError.modelNotFound("Model not found: \(id)")
        }
...
public func agentbridge_model_is_cached(
    ...
    do {
        ...
        return try manager.getCachedModel(identifier: id, channel: modelChannel) != nil ? 1 : 0
    } catch {
        return -1 // Error
    }
}
...
public func agentbridge_model_get_cache_stats(
    ...
    return MemoryManager.executeFFIOperation({
        ...
        let jsonString = String(data: jsonData, encoding: .utf8) ?? "{}"
        outStats.pointee = MemoryManager.createCString(from: jsonString)
...
// Placeholder YOLOBridge class removed - using real YOLOBridge from Vision.OD.YOLO module
...
public func agentbridge_free_string(_ str: UnsafeMutablePointer<CChar>?) {
