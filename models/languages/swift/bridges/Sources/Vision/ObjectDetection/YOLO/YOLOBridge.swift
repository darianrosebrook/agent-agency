...
public class YOLOBridge: BridgeProtocol {
    ...
    private func ensureModelLoaded() async throws {
        ...
        if let asset = try globalModelManager?.getCachedModel(identifier: "yolov3", channel: .stable) {
            ...
            let asset = try await globalModelManager!.downloadModel(identifier: "yolov3", channel: .stable)
            try loadModel(from: asset.localURL)
        }
    }
...
public struct Detection: Codable {
    ...
    public init(label: Int, className: String, confidence: Float, boundingBox: CGRect) {
        self.label = label
