...
public class SpeechFrameworkBridge: BridgeProtocol {
    ...
    public func transcribe(audioPath: String) async throws -> TranscriptionResult {
        ...
        return try await performRecognition(request: request, with: recognizer)
    }

    /// Transcribe audio buffer
    public func transcribeAudioBuffer(_ buffer: AVAudioPCMBuffer) async throws -> TranscriptionResult {
        let recognizer = try getSpeechRecognizer()
    ...
    private func performRecognition(request: SFSpeechRecognitionRequest, with recognizer: SFSpeechRecognizer) async throws -> TranscriptionResult {
