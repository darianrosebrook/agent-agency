...
async fn test_multi_dimensional_risk_scoring() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(surveillance_risks.overall_risk_score > 0.7, "Surveillance system should be high-risk overall");
...
fn assess_multi_dimensional_risks(spec: &MockWorkingSpec) -> RiskAssessmentResult {
    ...
    let technical_feasibility = if desc.contains("complex") || desc.contains("advanced") || desc.contains("ai") {
    ...
    let overall_risk_score = (technical_risk_score * 0.25) +
                            (ethical_risk_score * 0.25) +
