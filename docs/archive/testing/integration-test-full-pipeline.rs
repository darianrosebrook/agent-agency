...
fn analyze_pipeline_performance(all_metrics: &[PipelineMetrics]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let estimated_concurrent_time = avg_total_duration * concurrent_users as u32;
    println!("   👥 With {} concurrent users: {:.1}s total processing time", concurrent_users, estimated_concurrent_time.as_secs_f64());
    println!("   📊 Throughput: {:.1} tasks/minute", 60.0 / avg_total_duration.as_secs_f64());
    println!("   🔄 Maximum sustainable load: ~{} concurrent tasks", (60.0 / avg_total_duration.as_secs_f64()).round() as u32);
