...
impl GlobalTypeRegistry {
        ...
        T: serde::Serialize + for<'de> serde::Deserialize<'de> + Clone + Send + Sync + 'static,
    {
        ...
        let entry = TypeRegistryEntry {
            ...
            schema_version,
            serializer: |value| {
                if let Some(typed_value) = (value as &dyn Any).downcast_ref::<T>() {
                    serde_json::to_vec(typed_value)
                        .map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)
                } else {
                    Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, "Type mismatch")) as Box<dyn std::error::Error + Send + Sync>)
                }
            },
            deserializer: |data| {
    ...
    V: Clone + Send + Sync + 'static,
{
    ...
    /// Check if key exists (non-async version for compatibility)
    fn exists_sync(&self, key: &String) -> bool {
        // This is a simplified sync check - in practice, we'd need async runtime
        false // Placeholder
    }
    ...
    V: serde::Serialize + for<'de> serde::Deserialize<'de> + Clone + Send + Sync + 'static,
{
    ...
    async fn invalidate_redis_by_tags<V>(
        &self,
        redis: &RedisCache<V>,
        tags: &[String]
    ) -> CacheResult<usize>
