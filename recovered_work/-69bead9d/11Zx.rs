...
impl CacheManager {
    ...
    pub async fn global_stats(&self) -> HashMap<String, CacheStats> {
        ...
        for (name, cache_any) in caches.iter() {
            // Use type-safe downcasting to get cache statistics
            let cache_stats = if let Some(cache_arc) = cache_any.downcast_ref::<Arc<dyn Cache<String, String> + Send + Sync>>() {
                // Try to get stats from String cache
                cache_arc.stats().await.ok()
            } else if let Some(cache_arc) = cache_any.downcast_ref::<Arc<dyn Cache<String, serde_json::Value> + Send + Sync>>() {
                // Try to get stats from JSON cache
                cache_arc.stats().await.ok()
            } else if let Some(cache_arc) = cache_any.downcast_ref::<Arc<dyn Cache<String, Vec<u8>> + Send + Sync>>() {
                // Try to get stats from Bytes cache
                cache_arc.stats().await.ok()
            } else {
                // For unknown cache types, create default stats with warning
                warn!("Unknown cache type for '{}', using default statistics", name);
                Some(CacheStats::default())
            };
            ...
            if let Some(stat) = cache_stats {
                ...
                // If stats collection failed, insert default stats
