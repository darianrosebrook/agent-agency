...
impl GlobalTypeRegistry {
        ...
        T: serde::Serialize + for<'de> serde::Deserialize<'de> + Clone + Send + Sync + 'static,
    {
        ...
        let entry = TypeRegistryEntry {
            ...
            schema_version,
            serializer: |_value| {
                // Simplified: for now, we can't easily serialize through erased_serde
...
impl SafeAny {
    ...
    pub fn serialize(&self) -> CacheResult<Vec<u8>> {
        // Simplified serialization using serde_json directly
        // In a full implementation, this would use the registry
        if let Some(typed_value) = self.value.downcast_ref::<T>() {
            ...
            Err(CacheError::SerializationError {
                message: "Type downcast failed".to_string(),
            })
        }
    }
