...
function getQuarantinedTestCount() {
  try {
    const quarantineFile = path.join(process.cwd(), '.caws', 'quarantined-tests.json');

    if (!fs.existsSync(quarantineFile)) {
      return 0;
    }

    const quarantinedTests = JSON.parse(fs.readFileSync(quarantineFile, 'utf8'));
...
function analyzeTestConsistency(testHistory) {
  ...
  for (const [testName, results] of testResults) {
    ...
    const hasIntermittentFailures = passedResults.length > 0 && failedResults.length > 0;
