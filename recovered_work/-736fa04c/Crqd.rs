...
static DIALECT: Lazy<PostgreSqlDialect> = Lazy::new(|| PostgreSqlDialect {});
    ...
    fn extract_tables_from_statement(&self, statement: &Statement) -> CacheResult<(HashSet<String>, QueryType, u8, bool)> {
        ...
        match statement {
            ...
            Statement::Insert { table, .. } => {
                if let sqlparser::ast::TableFactor::Table { name, .. } = table {
            ...
            Statement::Update { table, from, selection, .. } => {
                ...
                if let Some(from_clause) = from {
                    if let sqlparser::ast::FromClause { tables: from_tables, .. } = from_clause {
            ...
            Statement::Delete { from, selection, .. } => {
                // Extract table from the from clause
                if let sqlparser::ast::FromClause { tables, .. } = from {
                    for table in tables {
                        if let sqlparser::ast::TableWithJoins { relation, .. } = table {
                            if let sqlparser::ast::TableFactor::Table { name, .. } = relation {
                                tables.insert(self.normalize_table_name(name));
