...
static DIALECT: Lazy<PostgreSqlDialect> = Lazy::new(PostgreSqlDialect::default);
    ...
    fn extract_tables_from_statement(&self, statement: &Statement) -> CacheResult<(HashSet<String>, QueryType, u8, bool)> {
        ...
        match statement {
            ...
            Statement::Update { table, selection, .. } => {
                ...
                if let sqlparser::ast::TableWithJoins { relation, .. } = table {
            ...
            Statement::Delete { from, selection, .. } => {
                // Handle the table reference in FROM clause
                for table_with_joins in &from.tables {
    ...
    fn extract_tables_from_query(&self, query: &Query, tables: &mut HashSet<String>, complexity: &mut u8) {
        ...
        match &**body {
                ...
                sqlparser::ast::SetExpr::SetOperation { left, right, .. } => {
                    ...
                    *complexity += 15;
                    // For set operations, we need to extract from the nested queries
                    // This is simplified - in practice, we'd need to handle the Box<SetExpr> properly
                    // For now, we'll just increase complexity but not extract tables
                }
                _ => {}
            }
    }
...
impl CacheWarmer {
    ...
    pub async fn warm_db_cache(&self, common_queries: Vec<(String, Vec<serde_json::Value>)>) -> CacheResult<()> {
        if let Ok(db_cache) = self.cache_manager.get_or_create_cache::<Vec<serde_json::Value>>("db_queries").await {
            let warmer = DatabaseQueryCache::<Vec<serde_json::Value>>::new(db_cache, CacheConfig::default());
