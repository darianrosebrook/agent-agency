...
export default function DebugPanel({ apiClient }: DebugPanelProps) {
  ...
  const loadDebugInfo = async () => {
    try {
      ...
      const recentErrors = events.entries
        ...
        .map((event) => ({
          ...
          severity: event.severity === "error" ? "high" : "medium" as "low" | "medium" | "high",
        }));

      // Identify system health issues based on patterns
      const systemHealth = {
        registryTimeout: events.entries.some(e => e.message?.includes("timeout")),
        taskProcessingIssues: events.entries
          .filter(e => e.type.includes("task") && e.severity === "error")
          .map(e => e.message || e.type),
        performanceIssues: events.entries
          .filter(e => e.message?.includes("slow") || e.message?.includes("timeout"))
          .map(e => e.message || e.type),
        connectionIssues: events.entries
          .filter(e => e.message?.includes("connection") || e.message?.includes("disconnect"))
          .map(e => e.message || e.type),
      };

      const performanceMetrics = {
        avgResponseTime: metrics?.systemHealth?.responseTimeMs || 0,
        errorRate: metrics?.taskStatus?.successRate ? (1 - metrics.taskStatus.successRate) * 100 : 0,
        queueDepth: status?.queueDepth || 0,
        activeConnections: 1, // Placeholder - would need actual connection tracking
      };
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-center">
          ...
          <span className="text-yellow-800 text-sm">Loading debug information...</span>
        </div>
      </div>
    );
  }
      ...
      {expanded && (
        <div className="mt-4 space-y-4">
          ...
          {(debugInfo.systemHealth.registryTimeout ||
            ...
            debugInfo.systemHealth.connectionIssues.length > 0) && (
            <div>
              <h4 className="text-yellow-800 font-medium mb-2">System Health Issues</h4>
              <div className="space-y-2">
                ...
                {debugInfo.systemHealth.taskProcessingIssues.map((issue, i) => (
                  <div key={i} className="text-sm text-red-700 bg-red-100 p-2 rounded">
                    🚫 Task Processing: {issue}
                  </div>
                ))}
                {debugInfo.systemHealth.performanceIssues.map((issue, i) => (
                  <div key={i} className="text-sm text-orange-700 bg-orange-100 p-2 rounded">
                    🐌 Performance: {issue}
                  </div>
                ))}
                {debugInfo.systemHealth.connectionIssues.map((issue, i) => (
                  <div key={i} className="text-sm text-blue-700 bg-blue-100 p-2 rounded">
                    🔌 Connection: {issue}
                  </div>
                ))}
              </div>
          ...
          {debugInfo.recentErrors.length > 0 && (
            <div>
              <h4 className="text-yellow-800 font-medium mb-2">Recent Errors & Warnings</h4>
              <div className="space-y-2">
                {debugInfo.recentErrors.map((error, i) => (
                  <div key={i} className="text-sm bg-white p-2 rounded border">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-gray-500 text-xs">{error.timestamp}</span>
                    </div>
          ...
          {/* Performance Metrics */}
          <div>
            <h4 className="text-yellow-800 font-medium mb-2">Performance Metrics</h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-white p-2 rounded border">
                <div className="text-gray-600">Avg Response Time</div>
                <div className="font-medium">{debugInfo.performanceMetrics.avgResponseTime}ms</div>
              </div>
              <div className="bg-white p-2 rounded border">
                <div className="text-gray-600">Error Rate</div>
                <div className="font-medium">{debugInfo.performanceMetrics.errorRate.toFixed(1)}%</div>
              </div>
              <div className="bg-white p-2 rounded border">
                <div className="text-gray-600">Queue Depth</div>
                <div className="font-medium">{debugInfo.performanceMetrics.queueDepth}</div>
              </div>
              <div className="bg-white p-2 rounded border">
                <div className="text-gray-600">Active Connections</div>
                <div className="font-medium">{debugInfo.performanceMetrics.activeConnections}</div>
              </div>
            </div>
          </div>
          ...
          {hasIssues && (
            <div>
              <h4 className="text-yellow-800 font-medium mb-2">Debugging Suggestions</h4>
              <div className="space-y-1 text-sm text-yellow-700">
                ...
                {debugInfo.performanceMetrics.errorRate > 5 && (
                  <div>• High error rate detected - review recent task failures</div>
                )}
                {debugInfo.performanceMetrics.queueDepth > 10 && (
                  <div>• Queue depth is high - consider scaling or optimization</div>
                )}
              </div>
            </div>
          )}
