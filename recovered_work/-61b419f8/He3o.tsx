...
export default function DebugPanel({ apiClient }: DebugPanelProps) {
  ...
  const loadDebugInfo = async () => {
    try {
      ...
      // Mock agent health data (would come from backend agent registry API)
      const agentHealth = [
        {
              ...
              {debugInfo.agentHealth.map((agent, i) => (
                <div key={i} className="bg-white p-3 rounded border text-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs text-gray-600">
                    <div>Last Active: {new Date(agent.lastActive).toLocaleTimeString()}</div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="font-medium flex items-center">
                  <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                    <div
                      ...
                      style={{ width: `${debugInfo.systemResources.memoryUsage}%` }}
                    ></div>
                  </div>
                ...
                <div className="font-medium flex items-center">
                  <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                    <div
                      ...
                      style={{ width: `${debugInfo.systemResources.cpuUsage}%` }}
                    ></div>
                  </div>
                ...
                <div className="font-medium flex items-center">
                  <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                    <div
                      ...
                      style={{ width: `${debugInfo.systemResources.diskUsage}%` }}
                    ></div>
                  </div>
              ...
              <div className="bg-white p-3 rounded border">
                ...
                <div className="font-medium">{debugInfo.systemResources.networkConnections}</div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-white p-3 rounded border">
                ...
                <div className="font-medium text-lg">{debugInfo.taskExecutionStats.totalTasks}</div>
              </div>
              <div className="bg-white p-3 rounded border">
                <div className="text-gray-600">Success Rate</div>
                <div className="font-medium text-lg text-green-600">
                  {((debugInfo.taskExecutionStats.completedTasks / debugInfo.taskExecutionStats.totalTasks) * 100).toFixed(1)}%
                </div>
              </div>
              <div className="bg-white p-3 rounded border">
                <div className="text-gray-600">Completed</div>
                <div className="font-medium text-green-600">{debugInfo.taskExecutionStats.completedTasks}</div>
              </div>
              <div className="bg-white p-3 rounded border">
                <div className="text-gray-600">Failed</div>
                <div className="font-medium text-red-600">{debugInfo.taskExecutionStats.failedTasks}</div>
              </div>
              <div className="bg-white p-3 rounded border col-span-2">
                <div className="text-gray-600">Avg Execution Time</div>
                <div className="font-medium">{debugInfo.taskExecutionStats.averageExecutionTime}ms</div>
              </div>
            </div>
          </div>
          ...
          {hasIssues && (
            <div>
              ...
              <div className="space-y-1 text-sm text-yellow-700">
                ...
                {debugInfo.systemResources.memoryUsage > 80 && (
                  <div>• High memory usage detected - consider memory optimization</div>
                )}
                {debugInfo.systemResources.cpuUsage > 80 && (
                  <div>• High CPU usage detected - consider performance optimization</div>
                )}
                {debugInfo.agentHealth.some(agent => agent.status === "unhealthy") && (
                  <div>• Unhealthy agents detected - review agent health and restart if needed</div>
                )}
              </div>
            </div>
          )}
