...
export default function DebugPanel({ apiClient }: DebugPanelProps) {
  ...
  const loadDebugInfo = async () => {
    try {
      ...
      const systemHealth = {
        registryTimeout: events.events.some(e => (e.metadata?.message as string)?.includes("timeout")),
        ...
        performanceIssues: events.events
          .filter(e => (e.metadata?.message as string)?.includes("slow") || (e.metadata?.message as string)?.includes("timeout"))
          .map(e => (e.metadata?.message as string) || e.type),
        connectionIssues: events.events
          .filter(e => (e.metadata?.message as string)?.includes("connection") || (e.metadata?.message as string)?.includes("disconnect"))
          .map(e => (e.metadata?.message as string) || e.type),
      };

      const performanceMetrics = {
        avgResponseTime: 0, // Placeholder - would need actual response time tracking
        errorRate: metrics?.taskSuccessRate ? (1 - metrics.taskSuccessRate) * 100 : 0,
