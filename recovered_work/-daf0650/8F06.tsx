...
import { ChatInterfaceProps, ChatSession, ChatMessage, ChatContext, ConnectionState, ChatMessagePayload } from '@/types/chat';
...
export default function ChatInterface({
  ...
  const loadExistingSession = useCallback(async (id: string) => {
    try {
      ...
      // Initialize WebSocket connection
      const wsUrl = `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/api/proxy/chat/ws/${id}`;
  ...
  const initializeWebSocket = useCallback((sessionId: string, wsUrl: string) => {
  ...
  const handleSendMessage = useCallback(async (payload: ChatMessagePayload) => {
    ...
    try {
      ...
      wsClientRef.current.sendMessage(payload.content, payload.context, payload.intent_hint);
  ...
  const handleContextUpdate = useCallback(async (updates: Partial<ChatContext>) => {
    ...
    try {
      ...
      onError?.(error instanceof Error ? error : new Error('Failed to update context'));
    }
  }, [state.session, onError]);
    ...
    return (
      <div className={styles.chatInterface}>
        <div className={styles.error}>
          ...
          <p>{state.error}</p>
          <button onClick={sessionId ? () => loadExistingSession(sessionId) : createNewSession}>
            Retry
          </button>
        </div>
      </div>
    ...
    <div className={styles.chatInterface}>
      <div className={styles.header}>
        <h2 className={styles.title}>
          {state.session?.title || `Chat Session ${state.session?.id.slice(-8)}`}
        </h2>
        <ConnectionStatus
