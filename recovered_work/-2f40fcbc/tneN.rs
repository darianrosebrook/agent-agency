...
impl RuntimeOptimizer {
    ...
    pub async fn with_config(config: OptimizationConfig) -> Result<Self> {
        ...
        let precision_engineer = PrecisionEngineer::new(config.precision_config.clone())?;
        ...
        let thermal_scheduler = ThermalScheduler::new(config.thermal_config.clone())?;
        let chunked_executor = ChunkedExecutor::new(config.chunk_config.clone())?;
        let kokoro_tuner = KokoroTuner::new(config.kokoro_config.clone())?;
