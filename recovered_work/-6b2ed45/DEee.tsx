...
import { PerformancePredictorProps, PerformancePrediction, AnalyticsFilters } from "@/types/analytics";
...
export default function PerformancePredictor({
  ...
  const loadPredictions = useCallback(async () => {
    ...
    try {
      ...
      const response = await analyticsApiClient.getPerformancePredictions(state.filters);
      setState((prev) => ({
        ...prev,
  ...
  const handlePredictionSelect = useCallback((prediction: PerformancePrediction) => {
    setState((prev) => ({ ...prev, selectedPrediction: prediction, showChart: true }));
  ...
  const generatePredictions = useCallback(async () => {
    try {
      ...
      const response = await analyticsApiClient.generateForecasting(forecastingRequest);
      setState((prev) => ({
        ...prev,
        predictions: response.predictions,
        isPredicting: false,
    ...
    <div className={styles.performancePredictor}>
      <div className={styles.predictorHeader}>
        ...
        <p className={styles.description}>
          AI-powered forecasting of system performance metrics using time series analysis and machine learning models.
        </p>
      ...
      {state.showChart && state.selectedPrediction && (
        <div className={styles.chartModal}>
          <div className={styles.modalHeader}>
            ...
            <button
              onClick={() => setState((prev) => ({ ...prev, showChart: false }))}
          ...
          <div className={styles.modalContent}>
            <ForecastingChart
              ...
              historicalData={timeSeriesData?.find((d) => d.name === state.selectedPrediction.metric)}
            />
          </div>
        </div>
      )}
      ...
      <div className={styles.predictionsSection}>
        {state.isPredicting ? (
          ...
          <>
            <div className={styles.predictionsHeader}>
              ...
              <div className={styles.predictionsStats}>
                <span>
                  High Confidence: {state.predictions.filter((p) => p.model_accuracy >= 0.8).length}
                </span>
                <span>
                  Medium Confidence: {state.predictions.filter((p) => p.model_accuracy >= 0.6 && p.model_accuracy < 0.8).length}
                </span>
                <span>
                  Low Confidence: {state.predictions.filter((p) => p.model_accuracy < 0.6).length}
                </span>
              </div>
            </div>
              ...
              {state.predictions.map((prediction) => (
                <div
                  ...
                  className={`${styles.predictionCard} ${
                    state.selectedPrediction?.metric === prediction.metric ? styles.selected : ""
                  }`}
                  onClick={() => handlePredictionSelect(prediction)}
                >
                  <div className={styles.predictionHeader}>
                    <div className={styles.predictionTitle}>
                      <span className={styles.predictionMetric}>{prediction.metric}</span>
                      <span className={`${styles.accuracyBadge} ${getAccuracyColor(prediction.model_accuracy)}`}>
                  ...
                  <div className={styles.predictionDetails}>
                    <div className={styles.predictionMetrics}>
                      ...
                      <div className={styles.metricItem}>
                        <span className={styles.metricLabel}>Predicted Values:</span>
                        <span className={styles.metricValue}>
                          {prediction.predicted_values.length} points
                        </span>
                      </div>
                      <div className={styles.metricItem}>
                        <span className={styles.metricLabel}>Forecast Horizon:</span>
                        <span className={styles.metricValue}>
                        ...
                        <div className={styles.factorsList}>
                          {prediction.factors.slice(0, 3).map((factor, index) => (
                            <div key={index} className={styles.factorItem}>
                              ...
                              <span className={`${styles.factorConfidence} ${getConfidenceColor(factor.confidence)}`}>
                    ...
                    <div className={styles.confidenceIntervals}>
                      ...
                      <div className={styles.intervalsGrid}>
                        {prediction.confidence_intervals.slice(0, 3).map((interval, index) => (
          ...
          <div className={styles.noPredictions}>
            ...
            <p>
              Performance predictions will be available once forecasting models are configured
              and historical data is available for analysis.
            </p>
            <div className={styles.predictionInfo}>
              <h4>About Performance Prediction:</h4>
              <ul>
                <li>Uses time series forecasting models (ARIMA, Prophet, Linear Regression)</li>
                <li>Provides confidence intervals for prediction uncertainty</li>
                <li>Analyzes factor impacts on performance metrics</li>
                <li>Includes actionable recommendations based on predictions</li>
