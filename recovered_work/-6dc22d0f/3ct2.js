...
const testDocuments = [
  {
    content: "The quick brown fox jumps over the lazy dog. This is a test document for multimodal RAG processing.",
    modality: "text",
    type: "document"
  },
  {
...
export default function() {
  ...
  const searchResponse = http.post(`${baseUrl}/api/v1/search`, 
    JSON.stringify({
      query: query,
      modality: 'text',
      limit: 10,
  ...
  const processingResponse = http.post(`${baseUrl}/api/v1/process`, 
    JSON.stringify({
  ...
  const processingSuccess = check(processingResponse, {
    ...
    'multimodal processing response time < 2s': (r) => r.timings.duration < 2000,
  });
  ...
  const embeddingResponse = http.post(`${baseUrl}/api/v1/embeddings`, 
    JSON.stringify({
      text: query,
      model: 'e5-small-v2'
    }), 
  ...
  check(embeddingResponse, {
    ...
    'embedding generation response time < 500ms': (r) => r.timings.duration < 500,
  });

  // Test 5: Cross-modal Validation
  const validationStartTime = Date.now();
  
  const validationResponse = http.post(`${baseUrl}/api/v1/validate`, 
    JSON.stringify({
      claim: "This document discusses machine learning concepts",
      evidence: [
        { content: query, modality: 'text', source: 'search_result' },
        { content: testFile.content, modality: modality, source: 'uploaded_file' }
      ],
  ...
  check(validationResponse, {
    ...
    'cross-modal validation response time < 1s': (r) => r.timings.duration < 1000,
  });

  // Test 6: Batch Processing
  if (Math.random() < 0.3) { // 30% chance of batch processing
    const batchResponse = http.post(`${baseUrl}/api/v1/batch/process`, 
      JSON.stringify({
        files: [
  ...
  return `
    ...
    <body>
        ...
        <div class="metric">
            ...
            <p>Multimodal Processing Success Rate: ${(data.metrics.multimodal_processing_success_rate.values.rate * 100).toFixed(2)}%</p>
            <p>Average Vector Search Time: ${data.metrics.vector_search_duration.values.avg.toFixed(2)}ms</p>
            <p>Average Embedding Generation Time: ${data.metrics.embedding_generation_duration.values.avg.toFixed(2)}ms</p>
        </div>
    </body>
    </html>
  `;
