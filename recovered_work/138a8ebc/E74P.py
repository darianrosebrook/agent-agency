...
class TestCacheOptimizerPhonemeCache:
    ...
    @patch('api.model.optimization.cache_optimizer.time.sleep')
    def test_phoneme_prewarming_logic(self, mock_sleep):
        """Test the actual phoneme prewarming logic."""
        optimizer = CacheOptimizer()

        # Mock dependencies that the prewarming function uses
        with patch('api.model.optimization.cache_optimizer.os.path.exists', return_value=True), \
             patch('api.model.optimization.cache_optimizer.json.load') as mock_json_load, \
             patch('api.model.optimization.cache_optimizer.logging') as mock_logging:

            # Mock phoneme cache data
            mock_json_load.return_value = {
                "phoneme_1": {"data": "test"},
                "phoneme_2": {"data": "test2"}
            }

            # Call the optimization method
            optimizer.optimize_phoneme_cache()

            # Verify sleep was called for timeout handling
...
class TestCacheOptimizerPersistence:
    ...
    def test_implement_cache_type_persistence(self):
        ...
        with patch('api.model.optimization.cache_optimizer.os.makedirs'), \
             patch('api.model.optimization.cache_optimizer.threading.Timer') as mock_timer:
    ...
    @patch('api.model.optimization.cache_optimizer.os.makedirs')
    @patch('api.model.optimization.cache_optimizer.json.dump')
    @patch('api.model.optimization.cache_optimizer.hashlib.sha256')
    def test_cache_save_functionality(self, mock_sha256, mock_json_dump, mock_makedirs):
        ...
        with patch('builtins.open', create=True) as mock_open:
            ...
            # Call the persistence implementation
            optimizer._implement_cache_type_persistence("phoneme", "/tmp/test")

            # Verify file operations occurred
            mock_open.assert_called()
            mock_json_dump.assert_called()

    @patch('api.model.optimization.cache_optimizer.os.path.exists', return_value=True)
    @patch('api.model.optimization.cache_optimizer.json.load')
    @patch('api.model.optimization.cache_optimizer.hashlib.sha256')
    def test_cache_load_functionality(self, mock_sha256, mock_json_load, mock_exists):
        ...
        with patch('builtins.open', create=True) as mock_open:
            ...
            # This would be called internally, but let's test the logic exists
            optimizer._implement_cache_type_persistence("phoneme", "/tmp/test")

            # Verify load operations would work
...
class TestCacheOptimizerUtilityFunctions:
    ...
    def test_apply_cache_optimizations(self):
        """Test apply cache optimizations function."""
        from api.model.optimization.cache_optimizer import apply_cache_optimizations

        with patch('api.model.optimization.cache_optimizer.CacheOptimizer') as mock_optimizer_class:
            mock_optimizer = Mock()
            ...
            mock_optimizer.optimize_phoneme_cache.assert_called_once()
            mock_optimizer.optimize_inference_cache.assert_called_once()
            mock_optimizer.optimize_primer_microcache.assert_called_once()
            mock_optimizer.implement_cache_persistence.assert_called_once()
    ...
    def test_get_cache_optimization_summary_global(self):
        ...
        with patch('api.model.optimization.cache_optimizer.CacheOptimizer') as mock_optimizer_class:
            mock_optimizer = Mock()
            mock_optimizer.get_cache_optimization_summary.return_value = {"test": "summary"}
            mock_optimizer_class.return_value = mock_optimizer
