...
iterations/v3/api-server/Cargo.toml:
  37  
  38: # Database client (TODO: Re-enable when database crate compiles)
  39  # agent-agency-database = { path = "../database" }

iterations/v3/api-server/src/main.rs:
  399      let api_router = Router::new()
  400:         // .route("/tasks", post(submit_task)) // TODO: Fix state handling
  401          .route("/tasks", get(list_tasks))
...
iterations/v3/src/bin/api-server.rs:
  45      let orchestrator = Arc::new(Orchestrator::new(
  46:         // TODO: Initialize with proper configuration
  47          Default::default(),

  58          require_api_key: args.require_api_key,
  59:         api_keys: vec![], // TODO: Load from config
  60          enable_rate_limiting: false,
