...
class TestConfigBenchmarkFrequency:
    ...
    def test_benchmark_frequency_default(self):
        ...
        assert api.config.BENCHMARK_FREQUENCY == "daily"
    ...
    def test_benchmark_frequency_valid_values(self):
        ...
        for input_value, expected in test_cases:
            ...
            import api.config
            importlib.reload(api.config)
            assert api.config.BENCHMARK_FREQUENCY == expected
...
class TestConfigDevelopmentProfiles:
    ...
    def test_dev_performance_profile_default(self):
        ...
        assert api.config.DEV_PERFORMANCE_PROFILE == "stable"
    ...
    def test_dev_performance_profile_valid_values(self):
        ...
        for input_value, expected in test_cases:
            ...
            import api.config
            importlib.reload(api.config)
            assert api.config.DEV_PERFORMANCE_PROFILE == expected
    ...
    def test_development_mode_profile_application(self, profile_name):
        ...
        profile = api.config.DEV_PERFORMANCE_PROFILES[profile_name]
        assert api.config.FORCE_CPU_PROVIDER == profile["force_cpu_provider"]
        assert api.config.DISABLE_DUAL_SESSIONS == profile["disable_dual_sessions"]
        assert api.config.SKIP_BACKGROUND_BENCHMARKING == profile["skip_background_benchmarking"]
        assert api.config.ENABLE_COREML_OPTIMIZATIONS == profile["enable_coreml_optimizations"]
    ...
    def test_production_mode_defaults(self):
        ...
        # Production defaults
        assert api.config.FORCE_CPU_PROVIDER == False
        assert api.config.DISABLE_DUAL_SESSIONS == False
        assert api.config.SKIP_BACKGROUND_BENCHMARKING == False
        assert api.config.ENABLE_COREML_OPTIMIZATIONS == True  # Default for Apple Silicon
...
class TestConfigBenchmarkCache:
    ...
    def test_get_benchmark_cache_duration_daily_default(self):
        ...
        expected = api.config.BENCHMARK_FREQUENCY_OPTIONS["daily"]
    ...
    def test_get_benchmark_cache_duration_weekly(self):
        ...
        result = TTSConfig.get_benchmark_cache_duration()
        expected = api.config.BENCHMARK_FREQUENCY_OPTIONS["weekly"]
        assert result == expected
    ...
    def test_get_benchmark_cache_duration_development_profiles(self, profile, expected_days):
        ...
        result = TTSConfig.get_benchmark_cache_duration()
        expected = expected_days * 86400  # Convert days to seconds
        assert result == expected
...
class TestConfigLogging:
    ...
    def test_log_verbose_false_defaults(self):
        ...
        assert api.config.CONSOLE_LOG_LEVEL == "INFO"
