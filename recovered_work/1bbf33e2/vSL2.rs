...
pub struct SafeTensorsModel {
    tensors: HashMap<String, safetensors::tensor::Tensor>,
    dimension: usize,
    vocab_size: usize,
}
...
impl SafeTensorsModel {
    ...
    pub async fn load_from_path(model_path: &Path) -> Result<Self> {
        ...
        // Use deserialize with owned data to avoid lifetime issues
        ...
        let tensor_map = tensors.tensors().iter().map(|(name, tensor_view)| {
            let owned_tensor = tensor_view.to_owned();
            (name.clone(), owned_tensor)
        }).collect();
