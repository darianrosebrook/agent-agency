...
export class SecurityProvenanceManager extends CawsBaseTool {
  ...
  async verifySignature(
    ...
    try {
      ...
      if (publicKeyPath && fs.existsSync(publicKeyPath)) {
        ...
        console.warn('No public key provided, falling back to hash-based verification');
  ...
  private generateSignature(content: string, privateKeyPath?: string): string {
    try {
      ...
      if (privateKeyPath && fs.existsSync(privateKeyPath)) {
        ...
        const { privateKey: generatedPrivateKey } = crypto.generateKeyPairSync('rsa', {
          modulusLength: 2048,
   ...
   */
  async generateDevelopmentKeyPair(keyDir: string = ".caws/keys"): Promise<{ privateKeyPath: string; publicKeyPath: string }> {
    try {
  ...
  private async verifyModelChecksum(
    ...
    try {
      ...
      console.log(`DEBUG: Loaded checksum store with ${Object.keys(checksumStore).length} entries`);

      // Determine model file path based on modelId and version
      const modelPath = this.getModelFilePath(modelId, version);
      console.log(`DEBUG: Model path for ${modelId}@${version}: ${modelPath}`);

      if (!modelPath || !fs.existsSync(modelPath)) {
        console.log(`DEBUG: Model file not found: ${modelPath}`);
        this.logger?.warn(
      ...
      console.log(`DEBUG: Calculated hash for ${modelPath}: ${fileHash}`);
      ...
      console.log(`DEBUG: Expected checksum for ${key}: ${expectedChecksum}`);

      if (!expectedChecksum) {
        console.log(`DEBUG: No checksum found in store for ${key}`);
        this.logger?.warn(
      ...
      console.log(`DEBUG: Hash comparison result: ${isValid}`);

      if (!isValid) {
        console.log(`DEBUG: SECURITY VIOLATION - Hash mismatch!`);
        this.logger?.error(
        ...
        console.log(`DEBUG: Model checksum verified successfully`);
        this.logger?.debug(`Model checksum verified for ${modelId}@${version}`);
      }

      return isValid;
    } catch (error) {
      console.log(`DEBUG: Exception in verifyModelChecksum:`, error);
      this.logger?.error(
  ...
  private getModelFilePath(modelId: string, version: string): string | null {
    ...
    const projectRoot = path.resolve(__dirname, '../../../..'); // Go up from apps/tools/caws to project root
