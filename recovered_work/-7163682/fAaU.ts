...
export class SecurityProvenanceManager extends CawsBaseTool {
  ...
  private async verifyModelChecksum(
    ...
    try {
      ...
      if (!modelPath || !fs.existsSync(modelPath)) {
        this.logger?.warn(`Model file not found for ${modelId}@${version}: ${modelPath}`);
        return false;
      }
      ...
      const expectedChecksum = checksumStore[`${modelId}@${version}`]?.sha256;
      if (!expectedChecksum) {
        this.logger?.warn(`No checksum found for ${modelId}@${version}, cannot verify integrity`);
      ...
      if (!isValid) {
        this.logger?.error(`SECURITY VIOLATION: Model checksum mismatch for ${modelId}@${version}`);
      ...
      this.logger?.error(`Failed to verify model checksum for ${modelId}@${version}:`, error);
      return false;
    }
  }

  private loadChecksumStore(): Record<string, { sha256: string; sha512?: string }> {
    try {
      const checksumPath = path.join(process.cwd(), ".caws", "model-checksums.json");
      if (!fs.existsSync(checksumPath)) {
        this.logger?.warn("Model checksum store not found, creating empty store");
        return {};
      }
  ...
  private getModelFilePath(modelId: string, version: string): string | null {
    // Map modelId/version to actual file paths
    // This could be extended to support different model providers and versions
  ...
  private calculateFileHash(filePath: string, algorithm: 'sha256' | 'sha512' = 'sha256'): string {
    try {
      const fileContent = fs.readFileSync(filePath);
      return crypto.createHash(algorithm).update(fileContent).digest("hex");
    } catch (error) {
      this.logger?.error(`Failed to calculate ${algorithm} hash for ${filePath}:`, error);
      throw error;
    }
  }
  ...
  public async registerModelChecksum(
    ...
    try {
      ...
      this.logger?.error(`Failed to register checksum for ${modelId}@${version}:`, error);
      throw error;
    }
  }
  ...
  (async () => {
    ...
    switch (command) {
      ...
      case "register": {
        ...
        if (!modelId || !version) {
          console.error("Usage: security-provenance register <modelId> <version> [modelPath]");
          console.error("Example: security-provenance register kokoro-v1.0 v1.0 ./kokoro-v1.0.int8.onnx");
          process.exit(1);
        }
