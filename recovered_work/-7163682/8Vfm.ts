...
export class SecurityProvenanceManager extends CawsBaseTool {
  ...
  private async scanDependencies(
    ...
    try {
      ...
      const criticalVulnerabilities = scanResults.filter(v => v.severity === 'CRITICAL' || v.severity === 'HIGH');

      this.logger?.info(`Dependency scan completed: ${scanResults.length} vulnerabilities found, ${criticalVulnerabilities.length} critical/high`);
  ...
  private async runDependencyVulnerabilityScan(projectDir: string): Promise<Array<{
    package: string;
    version: string;
    severity: string;
    ...
    if (vulnerabilities.length === 0) {
      try {
        const manualResults = await this.checkKnownVulnerablePackages(projectDir);
  ...
  private async runNPMAudit(projectDir: string): Promise<Array<{
    package: string;
    version: string;
    severity: string;
    vulnerability: string;
  ...
  private async runYarnAudit(projectDir: string): Promise<Array<{
    package: string;
    version: string;
    severity: string;
    vulnerability: string;
          ...
          try {
            ...
            if (auditEntry.type === 'auditAdvisory') {
              vulnerabilities.push({
                ...
                severity: this.mapYarnSeverity(auditEntry.data.advisory.severity),
  ...
  private async runPipAudit(projectDir: string): Promise<Array<{
    package: string;
    version: string;
    severity: string;
    vulnerability: string;
  ...
  private async checkKnownVulnerablePackages(projectDir: string): Promise<Array<{
    package: string;
    version: string;
    severity: string;
    vulnerability: string;
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        for (const [pkg, version] of Object.entries(dependencies)) {
          if (knownVulnerabilities[pkg]) {
            ...
            if (this.isVersionVulnerable(version as string, vuln.maxSafeVersion)) {
              vulnerabilities.push({
                package: pkg,
        ...
        for (const req of requirements) {
          if (req.trim() && !req.startsWith('#')) {
            ...
            if (match) {
              ...
              if (knownVulns[pkg] && this.isVersionVulnerable(version, knownVulns[pkg].maxSafeVersion)) {
  ...
  private isVersionVulnerable(currentVersion: string, maxSafeVersion: string): boolean {
    try {
      // Simple version comparison (basic implementation)
      const current = currentVersion.replace(/[^0-9.]/g, '').split('.').map(Number);
      const safe = maxSafeVersion.replace(/[^0-9.]/g, '').split('.').map(Number);
