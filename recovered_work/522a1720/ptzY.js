  ...
  if (specIds.length === 1) {
    ...
    for (const id of specIds) {
      ...
      try {
        ...
        const statusColor = status === 'active' ? chalk.green : status === 'completed' ? chalk.blue : chalk.yellow;
        const typeColor = SPEC_TYPES[type] ? SPEC_TYPES[type].color : chalk.white;

        console.log(chalk.yellow(`   - ${id} ${typeColor(`(${type})`)} ${statusColor(`[${status}]`)} - ${spec.title || 'Untitled'}`));
