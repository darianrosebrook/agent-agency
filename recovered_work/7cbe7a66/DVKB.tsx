...
const triggerVoicemailForEvent = (eventType: string, details: string, priority: "low" | "medium" | "high" = "medium") => {
  const messages = {
    task_complete: "Task completed successfully. All processes finished without errors.",
    security_scan: "Security scan completed. System is secure and up to date.",
    backup_complete: "System backup completed successfully. All data is safely stored.",
    performance_optimized: "Performance optimization completed. System performance improved.",
    deployment_ready: "Deployment ready. All tests passed and system is prepared for release.",
    error_resolved: "System error has been resolved. Normal operations restored.",
  };
    ...
    try {
      ...
      if (healthResponse.backend.status !== "healthy") {
        ...
        if (process.env.NODE_ENV === "development") {
          setTimeout(() => {
            triggerVoicemailForEvent("error_resolved", "System has recovered from previous issues and is operating normally.", "low");
          }, 5000);
        }
      }
    } catch (err) {
            ...
            <div className="settings-section">
              ...
              <div className="system-settings-placeholder">
                ...
                {process.env.NODE_ENV === "development" && (
                  <div style={{ marginTop: "2rem", paddingTop: "1rem", borderTop: "1px solid #e1e5e9" }}>
                    <h4>Test Voicemail Triggers</h4>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                      <button
                        onClick={() => triggerVoicemailForEvent("task_complete", "Data processing pipeline completed successfully. All records updated.", "medium")}
                        style={{ padding: "0.5rem", background: "#f0fdf4", border: "1px solid #d1d5db", borderRadius: "4px", cursor: "pointer" }}
                      >
                        Trigger Task Complete Voicemail
                      </button>
                      <button
                        onClick={() => triggerVoicemailForEvent("security_scan", "Automated security scan finished. No vulnerabilities detected.", "high")}
                        style={{ padding: "0.5rem", background: "#fef2f2", border: "1px solid #d1d5db", borderRadius: "4px", cursor: "pointer" }}
                      >
                        Trigger Security Scan Voicemail
                      </button>
                      <button
                        onClick={() => triggerVoicemailForEvent("performance_optimized", "System optimization completed. Performance improved by 23%.", "low")}
                        style={{ padding: "0.5rem", background: "#eff6ff", border: "1px solid #d1d5db", borderRadius: "4px", cursor: "pointer" }}
                      >
