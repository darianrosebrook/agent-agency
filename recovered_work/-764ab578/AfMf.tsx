...
export default function AgentPerformanceGrid({
  ...
  console.log("AgentPerformanceGrid component rendering");
  ...
  useEffect(() => {
    if (externalAgents) {
      ...
      // Directly load mock data for now to test rendering
      console.log("AgentPerformanceGrid: Loading mock data directly");
      metricsApiClient.getAgentPerformance().then(agentData => {
        console.log("AgentPerformanceGrid: Loaded agents:", agentData?.length);
        setAgents(agentData);
        setIsLoading(false);
      }).catch(err => {
        console.error("Failed to load agent performance:", err);
