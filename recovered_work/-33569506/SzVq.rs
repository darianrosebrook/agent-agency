...
async fn test_multi_dimensional_risk_scoring() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(automation_risks.overall_risk_score > 0.35, "Enterprise AI should have moderate-high risk");
    ...
    assert!(profiling_risks.overall_risk_score > 0.45, "Profiling system should be high-risk overall");
    ...
    assert!(api_risks.ethical_risk_score >= 0.0, "Standard API should have acceptable ethical score");
...
fn assess_multi_dimensional_risks(spec: &MockWorkingSpec) -> RiskAssessmentResult {
    ...
    let operational_feasibility = if desc.contains("global") && desc.contains("deployment") {
        0.4 // Global deployment is very challenging operationally
    } else if desc.contains("enterprise") || desc.contains("global") || desc.contains("high-scale") {
    ...
    } else if desc.contains("automation") || desc.contains("ai-powered") {
        0.6 // Automation projects have moderate business risk (job displacement, adoption challenges)
