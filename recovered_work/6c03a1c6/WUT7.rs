        ...
        match self.config.algorithm {
            ...
            QuantileAlgorithm::CKMS => {
                if let Some(ckms) = &self.ckms_state {
                    Ok(ckms.query(quantile).map(|(_, value)| value).unwrap_or(0.0))
                } else {
    ...
    pub fn merge(&mut self, other: &QuantileEstimator) -> ::std::result::Result<(), ObservabilityError> {
        match (&mut self.config.algorithm, &other.config.algorithm) {
            (QuantileAlgorithm::CKMS, QuantileAlgorithm::CKMS) => {
                // CKMS merging not implemented - CKMS structures don't expose sample iteration
                // Proper CKMS merging requires algorithmic knowledge of the internal structure
                // For now, keep the existing CKMS state unchanged
            }
...
// Removed invalid trait impl - traits cannot have constructors
