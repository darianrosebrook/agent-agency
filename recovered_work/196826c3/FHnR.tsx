...
export default function MetricsDashboard() {
  ...
  const [kpis, setKpis] = useState({
    ...
    activeAgents: { value: "12", status: "success" as "success" | "warning" | "error" | "neutral", trend: "+2" },
    taskThroughput: {
      value: "45/min",
      status: "warning" as "success" | "warning" | "error",
      trend: "-5%",
    },
    errorRate: { value: "0.8%", status: "success" as "success" | "warning" | "error", trend: "-0.1%" },
    avgResponseTime: {
      value: "245ms",
      status: "success" as "success" | "warning" | "error",
      trend: "-12ms",
  ...
  // System resources state for real-time updates
  ...
  // Calculate trend from historical data
          const calculateTrend = useCallback((key: string, _currentValue: number): string => {
      ...
      switch (event.type) {
        case "system_metrics":
          ...
          if (systemData?.coordination_metrics) {
            ...
            setCoordinationMetrics((prevMetrics: any) => ({
              ...prevMetrics,
              ...metrics,
          ...
          // Extract raw system resources for the SystemResourcesMonitor
          if (event.data) {
            setSystemResources({
              cpu_usage_percent: event.data.cpu_usage_percent || 0,
              memory_usage_percent: event.data.memory_usage_percent || 0,
              active_tasks: event.data.active_tasks || 0,
              completed_tasks: event.data.completed_tasks || 0,
              failed_tasks: event.data.failed_tasks || 0,
              avg_response_time_ms: event.data.avg_response_time_ms || 0,
            });
          }
          break;
