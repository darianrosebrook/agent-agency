...
fn analyze_pipeline_performance(all_metrics: &[PipelineMetrics]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let total_duration_sum: f64 = all_metrics.iter()
        .map(|m| m.total_duration.as_secs_f64())
        .sum();
    let avg_total_duration = total_duration_sum / total_executions as f64;

    let planning_time_sum: f64 = all_metrics.iter()
        .map(|m| m.stage_durations.get("planning").unwrap_or(&Duration::from_secs(0)).as_secs_f64())
        .sum();
    let avg_planning_time = planning_time_sum / total_executions as f64;

    let council_time_sum: f64 = all_metrics.iter()
        .map(|m| m.stage_durations.get("council").unwrap_or(&Duration::from_secs(0)).as_secs_f64())
        .sum();
    let avg_council_time = council_time_sum / total_executions as f64;

    let execution_time_sum: f64 = all_metrics.iter()
        .map(|m| m.stage_durations.get("execution").unwrap_or(&Duration::from_secs(0)).as_secs_f64())
        .sum();
    let avg_execution_time = execution_time_sum / total_executions as f64;
    ...
    println!("   3. **Scalability**: Current architecture supports ~{} concurrent tasks",
             (60.0 / avg_total_duration).round() as u32);
