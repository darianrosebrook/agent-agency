...
impl ChangeDeduplicationEngine {
    ...
    async fn compute_semantic_similarity(
        ...
        *computations += 1;
        ...
        let priority_similarity = self.compute_priority_compatibility(features1.priority, features2.priority);
    ...
    /// Compute priority compatibility score
    fn compute_priority_compatibility(&self, p1: ChangePriority, p2: ChangePriority) -> f64 {
        use crate::judge::ChangePriority::*;

        match (p1, p2) {
            (Critical, Critical) => 1.0,
            (Critical, High) | (High, Critical) => 0.8,
            (High, High) => 1.0,
            (High, Medium) | (Medium, High) => 0.7,
            (Medium, Medium) => 1.0,
            (Medium, Low) | (Low, Medium) => 0.6,
            (Low, Low) => 1.0,
            (Critical, _) | (_, Critical) => 0.5, // Critical should not conflict with others
    ...
    fn detect_conflict(
        ...
        // Check for priority conflicts (similar scope, different priorities)
        if change1.priority != change2.priority &&
           self.compute_scope_similarity(&features1.affected_components, &features2.affected_components) > 0.5 {
            return Some(ChangeConflict {
                ...
                description: format!("Similar scope but different priorities: {:?} vs {:?}", change1.priority, change2.priority),
    ...
    async fn merge_duplicate_group(
        ...
        if indices.is_empty() {
            return Err(CouncilError::Aggregation("Cannot merge empty change group".to_string()));
        }

        let first_change = &all_changes[indices[0]];
        let mut merged_description = first_change.description.clone();
        let mut max_priority = first_change.priority;
        ...
        for &idx in indices {
            ...
            // Update max priority
            if self.priority_value(change.priority) > self.priority_value(max_priority) {
                max_priority = change.priority;
            }
        ...
        let merged_category = if all_categories.len() == 1 {
            ...
            format!("Multiple ({})", all_categories.len())
        };
        ...
        Ok(RequiredChange {
            ...
            effort_estimate: first_change.effort_estimate, // Use first change's estimate
            dependencies: first_change.dependencies.clone(), // Simplified
            rationale: format!("Merged from {} similar changes", indices.len()),
        })
    }

    /// Get numerical value for priority comparison
    fn priority_value(&self, priority: ChangePriority) -> u8 {
        use crate::judge::ChangePriority::*;
        match priority {
            Critical => 4,
            High => 3,
