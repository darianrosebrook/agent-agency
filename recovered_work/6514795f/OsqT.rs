...
impl SelfPromptingLoop {
    ...
    fn apply_task_relevant_only_strategy(&self, task: &mut Task) -> usize {
        ...
        file_scores.sort_by(|a, b| {
            b.1.partial_cmp(&a.1).unwrap_or_else(|| {
                warn!("NaN detected in file relevance scores, treating as equal");
    ...
    fn attempt_dependency_installation(&self, task: &Task) -> bool {
        ...
        let workspace_root = std::env::current_dir().unwrap_or_else(|e| {
            error!("Failed to get current directory: {}. Using fallback.", e);
                    ...
                    match dir_path.to_str() {
                        Some(path_str) => {
                            match Command::new("chmod")
                                ...
                                Ok(_) => {
                                    warn!("chmod command failed for directory: {}", dir);
                                }
                                Err(e) => {
                                    warn!("Failed to execute chmod for directory {}: {}", dir, e);
                                }
                            }
                        }
                        None => {
