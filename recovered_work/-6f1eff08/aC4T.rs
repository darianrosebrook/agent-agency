        ...
        for patch in &changeset.patches {
            ...
            results.push(ChangesetValidationResult {
                ...
                details: if integrity_valid {
                    format!("Patch integrity validated for {}", patch.path.display())
                } else {
                    format!("Patch integrity check failed for {}", patch.path.display())
                },
        ...
        for patch in &changeset.patches {
            ...
            results.push(ChangesetValidationResult {
                ...
                details: if content_valid {
                    format!("Content validation passed for {}", patch.path.display())
                } else {
                    format!("Content validation failed for {}", patch.path.display())
                },
    ...
    async fn apply_changeset_atomically(
        ...
        for (i, patch) in changeset.patches.iter().enumerate() {
            ...
            self.update_progress(&ChangeSetId("temp".to_string()), i + 1, successful_patches, 0,
                               format!("Applying patch to {}", patch.path.display())).await?;
            ...
            match self.apply_single_patch_atomic(patch, workspace_path).await {
                ...
                Err(e) => {
                    tracing::warn!("Failed to apply patch to {}: {}", patch.path.display(), e);
