...
impl SandboxEnvironment {
    ...
    pub async fn apply_diff(&mut self, diff: &UnifiedDiff, iteration: usize) -> Result<ApplyResult, SandboxError> {
        ...
        match apply_result {
            diff_applier::ApplyResult::Failed { failing_hunks } => {
                return Err(SandboxError::DiffApplicationFailed {
                    file_path: diff.file_path.clone(),
                    failing_hunks: failing_hunks.len(),
                    details: format!("Failed to apply {} hunks", failing_hunks.len()),
                });
            }
            diff_applier::ApplyResult::Success => {
                // Continue with successful application
            }
        }

        // Extract modified files from the diff
        let modified_files = vec![diff.file_path.clone()];
...
pub enum SandboxError {
    ...
    #[error("Diff application failed for {file_path}: {failing_hunks} hunks failed - {details}")]
...
impl From<crate::sandbox::git_worktree::GitWorktreeError> for SandboxError {
    fn from(err: crate::sandbox::git_worktree::GitWorktreeError) -> Self {
        SandboxError::GitError(err.to_string())
    }
}

impl From<crate::sandbox::diff_applier::DiffApplyError> for SandboxError {
    fn from(err: crate::sandbox::diff_applier::DiffApplyError) -> Self {
        SandboxError::DiffError(err.to_string())
    }
}

impl From<crate::sandbox::snapshot::SnapshotError> for SandboxError {
    fn from(err: crate::sandbox::snapshot::SnapshotError) -> Self {
        SandboxError::SnapshotError(err.to_string())
    }
}

impl From<crate::sandbox::file_guard::FileGuardError> for SandboxError {
    fn from(err: crate::sandbox::file_guard::FileGuardError) -> Self {
