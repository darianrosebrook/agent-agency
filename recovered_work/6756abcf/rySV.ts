...
export class CawsGateChecker extends CawsBaseTool {
  ...
  async calculateTrustScore(options: GateCheckOptions): Promise<GateResult> {
    try {
      ...
      const a11yScore = this.calculateA11yScore(provenance?.results?.a11y, workingSpec);
      totalScore += a11yScore * weights.a11y;
      totalWeight += weights.a11y;

      // Calculate Performance component score
      const perfScore = this.calculatePerformanceScore(provenance?.results?.perf, workingSpec);
  ...
  private calculateA11yScore(a11yResults: any, workingSpec: any): number {
    try {
      ...
      violations.forEach((violation: any) => {
        ...
        const weight = severityWeights[impact as keyof typeof severityWeights] || 0.3;
        const nodeCount = violation.nodes?.length || 1;

        totalPenalty += weight * nodeCount;

        // Log detailed violation information
        this.logger?.warn(`A11y violation: ${violation.id} (${impact}) - ${violation.description}`);
      ...
      if (incomplete.length > 0) {
        ...
        this.logger?.warn(`${incomplete.length} incomplete accessibility checks`);
      }
      ...
      if (specRequirements.length > 0) {
        const specViolations = specRequirements.filter((req: string) => {
          ...
          return !passes.some((pass: any) => pass.id?.includes(req) || pass.description?.includes(req));
        });

        if (specViolations.length > 0) {
          totalPenalty += 0.5 * specViolations.length;
          this.logger?.warn(`Working spec a11y requirements not met: ${specViolations.join(', ')}`);
        }
      }
      ...
      this.logger?.info(`A11y score: ${score.toFixed(2)} (${passCount} passed, ${violationCount} violations)`);
   ...
   */
  private calculatePerformanceScore(perfResults: any, workingSpec: any): number {
    try {
      ...
      if (apiBudget && perfResults.api_p95_ms !== undefined) {
        ...
        if (actualApiTime > apiBudget) {
          ...
          violations++;
          this.logger?.warn(`API p95 budget violation: ${actualApiTime}ms > ${apiBudget}ms (${(overrun * 100).toFixed(1)}% over)`);
        } else {
          this.logger?.debug(`API p95 budget met: ${actualApiTime}ms ≤ ${apiBudget}ms`);
        }
      }
      ...
      if (lcpBudget && perfResults.lcp_ms !== undefined) {
        ...
        if (actualLcpTime > lcpBudget) {
          ...
          score -= Math.min(0.5, overrun * 0.3); // Penalty based on overrun percentage
          violations++;
          this.logger?.warn(`LCP budget violation: ${actualLcpTime}ms > ${lcpBudget}ms (${(overrun * 100).toFixed(1)}% over)`);
        } else {
          this.logger?.debug(`LCP budget met: ${actualLcpTime}ms ≤ ${lcpBudget}ms`);
        }
      }
      ...
      if (perfResults.baseline_comparison) {
        ...
        if (regression.api_p95_regression > 0.05) { // 5% regression threshold
          score -= 0.1;
          violations++;
          this.logger?.warn(`API performance regression: ${(regression.api_p95_regression * 100).toFixed(1)}% slower than baseline`);
        }
        if (regression.lcp_regression > 0.05) { // 5% regression threshold
          score -= 0.1;
          violations++;
          this.logger?.warn(`LCP performance regression: ${(regression.lcp_regression * 100).toFixed(1)}% slower than baseline`);
        }
      }

      // Additional metrics that could affect score
      if (perfResults.error_rate !== undefined && perfResults.error_rate > 0.01) { // >1% errors
        score -= 0.2;
        violations++;
        this.logger?.warn(`High error rate: ${(perfResults.error_rate * 100).toFixed(2)}%`);
      }
      ...
      this.logger?.info(`Performance score: ${score.toFixed(2)} (${violations} violations)`);
      if (apiBudget) {
        this.logger?.info(`API budget: ${apiBudget}ms (actual: ${perfResults.api_p95_ms || 'N/A'}ms)`);
      }
      if (lcpBudget) {
        this.logger?.info(`LCP budget: ${lcpBudget}ms (actual: ${perfResults.lcp_ms || 'N/A'}ms)`);
      }
