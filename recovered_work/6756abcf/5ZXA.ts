...
export class CawsGateChecker extends CawsBaseTool {
  ...
  private async checkWaiver(
    ...
    try {
      ...
      const resolvedWaiver = await this.resolveWaiverConflicts(applicableWaivers);

      if (resolvedWaiver) {
        return {
          waived: true,
          waiver: resolvedWaiver,
          conflictsResolved: applicableWaivers.length > 1
        };
      }
  ...
  private async resolveWaiverConflicts(
    ...
    const sortedWaivers = waivers.sort((a, b) => {
      ...
      const impactCompare = (impactLevels[b.impactLevel] || 0) - (impactLevels[a.impactLevel] || 0);
      if (impactCompare !== 0) {
  ...
  private isWaiverScopeValid(waiver: any): boolean {
    try {
      ...
      this.logger?.error(`Error validating waiver scope ${waiver.id}: ${error}`);
      return false;
    }
  }
  ...
  private async validateApprovalChain(waiver: any): Promise<boolean> {
    try {
      ...
      if (!requiredApprovers.includes(waiver.approvedBy)) {
        this.logger?.debug(`Waiver ${waiver.id} approved by unauthorized user: ${waiver.approvedBy}`);
        return false;
      }
      ...
      if (approvedAt < createdAt) {
        this.logger?.debug(`Waiver ${waiver.id} has invalid approval timestamp`);
        return false;
      }
      ...
      this.logger?.error(`Error validating approval chain for waiver ${waiver.id}: ${error}`);
      return false;
    }
  }
   ...
   */
  private async handleWaiverConflicts(conflicts: Array<{
    waiverA: any;
    waiverB: any;
    conflictType: string;
  }>): Promise<void> {
