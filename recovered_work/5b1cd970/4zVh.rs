...
async fn submit_task(
    ...
    // Execute task directly via HTTP to worker with circuit breaker protection
    tokio::spawn(async move {
        let client = circuit_breaker::ResilientHttpClient::new();
        ...
        match client.request(
            ...
            Err(e) => {
                match e {
                    circuit_breaker::ResilientHttpError::CircuitOpen => {
                        println!("🚫 Circuit breaker open - Task {} rejected", task_id);
                    }
                    circuit_breaker::ResilientHttpError::Http(status) => {
                        println!("❌ Task {} failed with HTTP status: {}", task_id, status);
                    }
                    circuit_breaker::ResilientHttpError::Network(e) => {
