...
describe('Enhanced Error Handling', () => {
  ...
  describe('Multiple Specs Error Handling', () => {
    test('should provide detailed guidance for multiple specs', async () => {
      ...
      require('js-yaml').load
        .mockReturnValueOnce(specs['user-auth'])
    ...
    test('should suggest most appropriate spec', async () => {
      ...
      const specs = {
        ...
        'completed-spec': { id: 'completed-spec', type: 'feature', status: 'completed', title: 'Completed' },
      };

      require('js-yaml').load
        .mockReturnValueOnce(specs['draft-spec'])
    ...
    test('should show interactive mode suggestion', async () => {
      ...
      require('js-yaml').load
        .mockReturnValueOnce({ id: 'spec1', status: 'draft' })
  ...
  describe('Command Error Handling', () => {
    test('should handle spec resolution errors in validate command', async () => {
      ...
      require('../src/utils/spec-resolver').resolveSpec = jest.fn().mockRejectedValue(
    ...
    test('should handle validation errors with context', async () => {
      ...
      require('../src/validation/spec-validation').validateWorkingSpecWithSuggestions =
        jest.fn().mockReturnValue(mockValidation);

      await expect(validateCommand(null, { specId: 'test-spec' })).rejects.toThrow();

      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Validating feature spec')
      );
    });
  ...
  describe('File System Error Handling', () => {
    ...
    test('should handle registry JSON parsing errors', async () => {
      ...
      fs.pathExists.mockResolvedValue(true);
      require('fs-extra').readJson = jest.fn().mockRejectedValue(
