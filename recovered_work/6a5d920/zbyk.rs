...
async fn intervene_task(
    ...
    match intervention {
        InterventionCommand::Pause => {
            ...
            match current_state {
                TaskState::Running => {
                    registry.insert(task_id, TaskState::Paused);
                    println!("✅ Task paused successfully");
                    println!("   Task {} is now in paused state", task_id);
                }
                TaskState::Paused => {
                    println!("⚠️  Task is already paused");
                }
                TaskState::Aborted => {
                    println!("❌ Cannot pause aborted task");
                }
                TaskState::Completed => {
        ...
        InterventionCommand::Resume => {
            ...
            match current_state {
                TaskState::Paused => {
                    registry.insert(task_id, TaskState::Running);
                    println!("✅ Task resumed successfully");
                    println!("   Task {} is now running", task_id);
                }
                TaskState::Running => {
                    println!("⚠️  Task is already running");
                }
                TaskState::Aborted => {
                    println!("❌ Cannot resume aborted task");
                }
                TaskState::Completed => {
                    println!("✅ Task is already completed");
                }
        ...
        InterventionCommand::Abort => {
            ...
            if input == "y" || input == "yes" {
                let mut registry = TASK_REGISTRY.lock().unwrap();
                registry.insert(task_id, TaskState::Aborted);
