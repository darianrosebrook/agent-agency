...
import { AnomalyDetectorProps, Anomaly, AnalyticsFilters } from "@/types/analytics";
...
export default function AnomalyDetector({
  ...
  const handleAnomalyDismiss = useCallback(async (anomalyId: string) => {
    try {
      await analyticsApiClient.dismissAnomaly(anomalyId);
      setState((prev) => ({
        ...prev,
        anomalies: prev.anomalies.filter((a) => a.id !== anomalyId),
        selectedAnomaly: prev.selectedAnomaly?.id === anomalyId ? null : prev.selectedAnomaly,
      }));
  ...
  const handleFilterChange = useCallback((filterType: keyof AnalyticsFilters, value: any) => {
    setState((prev) => ({
      ...prev,
      filters: {
        ...prev.filters,
  ...
  const runAnomalyDetection = useCallback(async () => {
    try {
      ...
      const response = await analyticsApiClient.detectAnomalies(detectionRequest);
      setState((prev) => ({
        ...prev,
        anomalies: response.anomalies,
        isDetecting: false,
    ...
    <div className={styles.anomalyDetector}>
      <div className={styles.detectorHeader}>
        ...
        <p className={styles.description}>
          Automated detection of unusual patterns and deviations in system metrics using advanced algorithms.
        </p>
          ...
          <div className={styles.filterControls}>
            ...
            <div className={styles.filterGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  handleFilterChange("confidence_threshold", parseFloat(e.target.value))
                }
      ...
      <div className={styles.anomaliesSection}>
        {state.isDetecting ? (
          ...
          <>
            <div className={styles.anomaliesHeader}>
              ...
              <div className={styles.anomaliesStats}>
                ...
                <span>
                  Medium: {state.anomalies.filter((a) => a.severity === "medium").length}
                </span>
                <span>
                  Low: {state.anomalies.filter((a) => a.severity === "low").length}
                </span>
              </div>
            </div>
              ...
              {state.anomalies.map((anomaly) => (
                <div
                  ...
                  className={`${styles.anomalyCard} ${
                    state.selectedAnomaly?.id === anomaly.id ? styles.selected : ""
                  }`}
                  onClick={() => handleAnomalySelect(anomaly)}
                >
                  <div className={styles.anomalyHeader}>
                    <div className={styles.anomalyTitle}>
                      <span className={`${styles.severityIcon} ${getSeverityColor(anomaly.severity)}`}>
                        {getSeverityIcon(anomaly.severity)}
                      </span>
                      <span className={styles.anomalyMetric}>{anomaly.metric}</span>
                      <span className={`${styles.severityBadge} ${getSeverityColor(anomaly.severity)}`}>
                  ...
                  <div className={styles.anomalyDetails}>
                    <div className={styles.anomalyMetrics}>
                      ...
                      <div className={styles.metricItem}>
                        ...
                        <span className={`${styles.metricValue} ${styles.deviation}`}>
          ...
          <div className={styles.noAnomalies}>
            ...
            <p>
              The system is operating within normal parameters. No significant deviations
              from expected behavior have been detected.
            </p>
      ...
      {state.selectedAnomaly && (
        <div className={styles.anomalySidebar}>
          <div className={styles.sidebarHeader}>
            ...
            <button
              onClick={() => setState((prev) => ({ ...prev, selectedAnomaly: null }))}
            ...
            <div className={styles.detailSection}>
              ...
              <div className={styles.detailGrid}>
                ...
                <div className={styles.detailItem}>
                  ...
                  <span className={`${styles.detailValue} ${getSeverityColor(state.selectedAnomaly.severity)}`}>
