...
import { TTSVoice, TTSSettings, AudioPlaybackState, NotificationAlert } from '@/types/tts';
...
export function useTTS() {
  ...
  const generateAttentionAlert = useCallback(async (userName?: string) => {
    if (!settings.enabled || !settings.attentionAlerts || !isServiceAvailable) {
      return null;
    }
  ...
  const generateVoicemailAlert = useCallback(async (message: string) => {
    if (!settings.enabled || !settings.autoPlayNotifications || !isServiceAvailable) {
      return null;
    }
...
export function useNotificationAlerts() {
  ...
  const addAlert = useCallback((alert: Omit<NotificationAlert, 'id' | 'timestamp' | 'played'>) => {
  ...
  const playAlert = useCallback(async (alertId: string) => {
