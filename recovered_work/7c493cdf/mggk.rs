...
impl WebSocketApi {
    ...
    async fn subscribe_to_task(
        ...
        if include_history {
            // Retrieve historical events from audit logs
            match self.get_historical_events(task_id).await {
                Ok(events) => {
    ...
    /// Retrieve historical events for a task from audit logs
    async fn get_historical_events(&self, task_id: Uuid) -> Result<Vec<ExecutionEvent>, WebSocketError> {
        let query = r#"
        ...
        let rows = self.db_client.query(query, &[&task_id])
            .await
            .map_err(|e| WebSocketError::DatabaseError(format!("Failed to query audit logs: {}", e)))?;
        ...
        for row in rows {
            ...
            // Convert audit log entry to ExecutionEvent
            let event = ExecutionEvent {
                id: event_id,
                task_id,
                event_type: action.clone(),
                message: format!("{}: {}", action, change_summary.get("reason")
                    .and_then(|r| r.as_str())
                    .unwrap_or("No details")),
                timestamp: created_at,
                metadata: Some(change_summary),
                actor,
            };
