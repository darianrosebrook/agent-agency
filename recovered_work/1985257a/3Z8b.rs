...
impl DatabaseArtifactStorage {
    ...
    fn map_test_files(&self, artifacts_by_type: &std::collections::HashMap<String, Vec<serde_json::Value>>) -> Vec<agent_agency_contracts::execution_artifacts::TestFile> {
        use agent_agency_contracts::execution_artifacts::*;

        let test_files_data = artifacts_by_type.get("test_files");
        if let Some(values) = test_files_data {
            if let Some(first) = values.first() {
                if let Ok(files) = serde_json::from_value::<Vec<TestFile>>(first.clone()) {
                    return files;
                }
            }
        }
