...
impl ArtifactStorage for DatabaseArtifactStorage {
    ...
    async fn retrieve(
        ...
        if rows.is_empty() {
            return Err(ArtifactStorageError::NotFoundForTask(metadata.task_id));
        }
        ...
        for row in rows {
            ...
            if let Some(stored_checksum) = stored_checksum {
                if computed_checksum != stored_checksum {
                    ...
                    let _ = self.db_client.create_task_audit_event(
                        task_id,
                        "artifact",
                        "integrity_check",
                        "integrity_violation",
        ...
        let execution_artifacts = self.db_rows_to_artifacts(artifact_rows, metadata.task_id)?;
