...
export default function ObservabilityDashboard({
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const errorRate = metricsData?.taskSuccessRate
        ? (1 - metricsData.taskSuccessRate) * 100
        : 0;
      const systemHealth: SystemHealth = {
        status:
          errorRate > 20 ? "critical" : errorRate > 10 ? "degraded" : "healthy",
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-sm font-medium ${
                  (metrics?.queuedTasks || 0) > 10
                    ? "text-red-600"
                    : (metrics?.queuedTasks || 0) > 5
                    ? "text-yellow-600"
                    : "text-green-600"
                }`}
              >
