  ...
  try {
    ...
    try {
      ...
      if (orchestrator) {
        ...
        failed++;
        errors.push("Orchestrator initialization failed");
      }
    } catch (error) {
      failed++;
...
async function runAllTests(): Promise<void> {
  ...
  console.log("Testing AgentOrchestrator.test.ts...");
  const agentResult = await runAgentOrchestratorTest();
  results.push(agentResult);
  console.log(
    `  ✅ Passed: ${agentResult.passed}, ❌ Failed: ${agentResult.failed}`
  );
  if (agentResult.errors.length > 0) {
    agentResult.errors.forEach((error) => console.log(`    ${error}`));
  }
  console.log("");

  // Advanced task router test
