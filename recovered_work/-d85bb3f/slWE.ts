...
async function runMultiTenantMemoryManagerTest(): Promise<TestResult> {
  try {
    // Create minimal mocks for monitoring components to prevent timers
    ...
    const mockProductionMonitor = {
      performHealthCheck: async () => ({ name: "test", status: "pass", timestamp: Date.now(), duration: 1 }),
      generateAlert: async () => {},
      startMonitoring: () => {}, // Mock to prevent timers
      stopMonitoring: () => {}, // Mock to prevent errors
    };

    // Mock the monitoring modules before importing MultiTenantMemoryManager
    const originalRequire = require;
    const mockRequire = (id: string) => {
      if (id === "../performance/performance-monitor.js") {
        return { PerformanceMonitor: class MockPerformanceMonitor {
          constructor(config: any, logger?: any) {
            return mockPerformanceMonitor;
          }
        }};
      }
      if (id === "../production/production-monitor.js") {
        return { ProductionMonitor: class MockProductionMonitor {
          constructor(config: any, performanceMonitor: any, logger?: any) {
            return mockProductionMonitor;
          }
        }};
      }
    ...
    // Test MultiTenantMemoryManager class structure
    ...
    try {
      ...
      const config = {
        tenantIsolation: {
          enabled: false,
          defaultIsolationLevel: "strict" as const,
          auditLogging: false,
          maxTenants: 10,
        },
        contextOffloading: {
      ...
      const manager = new MultiTenantMemoryManager(config);

      if (manager) {
        passed++;
      } else {
        failed++;
        errors.push("MultiTenantMemoryManager instantiation failed");
      }

      // Test that it has the expected methods
      if (typeof manager.storeExperience === "function") {
        passed++;
      } else {
        failed++;
      ...
      if (typeof manager.retrieveContext === "function") {
        passed++;
      } else {
        failed++;
      ...
      if (typeof manager.getSystemHealth === "function") {
        passed++;
      } else {
        failed++;
      ...
      failed++;
      errors.push(`MultiTenantMemoryManager test failed: ${error.message}`);
    }
