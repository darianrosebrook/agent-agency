...
async function runMultiTenantMemoryManagerTest(): Promise<TestResult> {
  try {
    // Test MultiTenantMemoryManager class structure with monitoring disabled
    ...
    try {
      // Create a config that disables all monitoring features to prevent timers
      const config = {
        tenantIsolation: {
          enabled: false, // Disable tenant isolation to prevent monitoring
          defaultIsolationLevel: "strict" as const,
          auditLogging: false,
          maxTenants: 10,
        },
        contextOffloading: {
          enabled: false, // Disable context offloading
          maxContextSize: 1000,
        ...
        federatedLearning: {
          enabled: false, // Disable federated learning to prevent monitoring
