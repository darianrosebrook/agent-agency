...
async function runTenantIsolatorTest(): Promise<TestResult> {
  try {
    ...
    try {
      ...
      if (context && context.tenantId === "test-tenant" && context.projectId === "test-project") {
        passed++;
      } else {
        failed++;
  ...
  try {
    ...
    try {
      ...
      if (memoryManager) {
        ...
        failed++;
        errors.push("Memory manager creation failed");
      }
    } catch (error) {
      failed++;
...
async function runAllTests(): Promise<void> {
  ...
  const memoryResult = await runMultiTenantMemoryManagerTest();
  results.push(memoryResult);
  console.log(
    `  ✅ Passed: ${memoryResult.passed}, ❌ Failed: ${memoryResult.failed}`
  );
