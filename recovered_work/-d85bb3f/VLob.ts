...
async function runMemorySystemContractTest(): Promise<TestResult> {
  try {
    ...
    try {
      ...
      // Test that we can create a memory manager with valid config
      const memoryManager = new MultiTenantMemoryManager(config);

      if (memoryManager) {
        passed++;
      } else {
        failed++;
        errors.push("Memory manager contract creation failed");
      }

      // Test basic contract properties
      if (typeof config.tenantIsolation.enabled === "boolean") {
        passed++;
      } else {
        failed++;
