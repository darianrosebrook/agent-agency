...
async function runMultiLevelCacheTest(): Promise<TestResult> {
  try {
    ...
    jest.mock("./dist/data/cache/RedisCache.js", () => ({
    ...
    const cacheConfig = {
      ...
      demotionThreshold: 30000,
      enableMetrics: false, // Disable metrics for test
    };
    ...
    try {
      ...
      if (cache) {
        ...
        failed++;
        errors.push("Cache creation failed");
      }
    } catch (error) {
      failed++;
