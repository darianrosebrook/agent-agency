...
async function runMultiLevelCacheTest(): Promise<TestResult> {
  try {
    ...
    const { MultiLevelCache } = await import("./dist/data/cache/MultiLevelCache.js");
...
async function runDataLayerTest(): Promise<TestResult> {
  try {
    // Create minimal mocks for DataLayer dependencies
    const mockConnection = {
      connect: async () => {},
      query: async (text: string, params?: any[]) => ({
        success: true,
        data: text.includes("SELECT") ? [{ id: 1, name: "test" }] : [],
        duration: 10,
        queryId: "test-query",
      }),
      transaction: async (callback: any) => {
        const result = await callback(mockConnection);
        return { success: true, data: result, duration: 15, queryId: "test-tx" };
      },
      healthCheck: async () => ({ healthy: true, latency: 5 }),
      getStats: async () => ({ connections: 1, queries: 5 }),
      close: async () => {},
    };
    ...
    // Test DataLayer class structure
    ...
    try {
      // Test that DataLayer can be instantiated
      ...
      if (dataLayer) {
        ...
        failed++;
        errors.push("DataLayer instantiation failed");
      }

      // Test that it has the expected methods
      if (typeof dataLayer.query === "function") {
        passed++;
      } else {
        failed++;
        errors.push("DataLayer missing query method");
      }

      if (typeof dataLayer.transaction === "function") {
        passed++;
      } else {
        failed++;
        errors.push("DataLayer missing transaction method");
      }

      if (typeof dataLayer.healthCheck === "function") {
        passed++;
      } else {
        failed++;
        errors.push("DataLayer missing healthCheck method");
      }

      if (typeof dataLayer.getStats === "function") {
        passed++;
      } else {
        failed++;
        errors.push("DataLayer missing getStats method");
      }
