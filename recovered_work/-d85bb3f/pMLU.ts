...
async function runMultiTenantMemoryManagerTest(): Promise<TestResult> {
  try {
    ...
    try {
      ...
      // Test that it has the expected methods
      if (typeof manager.storeContext === "function") {
        passed++;
      } else {
        failed++;
        errors.push("MultiTenantMemoryManager missing storeContext method");
      }
      ...
      if (typeof manager.cleanupContexts === "function") {
        passed++;
      } else {
        failed++;
        errors.push("MultiTenantMemoryManager missing cleanupContexts method");
      }
