...
async function runVectorDAOTest(): Promise<TestResult> {
  try {
    ...
    try {
      ...
      class TestVectorDAO extends VectorDAO<{ id: string; tenantId: string; embedding?: number[]; createdAt: Date; updatedAt: Date; name: string }> {
      ...
      const dao = new TestVectorDAO(mockDataLayer as any, "test_table", "TestEntity");
      ...
      if (typeof dao.bulkUpsert === "function") {
        passed++;
      } else {
        failed++;
        errors.push("VectorDAO missing bulkUpsert method");
      }
