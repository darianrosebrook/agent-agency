...
class BuildBenchmarker {
  ...
  async benchmarkRustBuild(clean = false) {
    ...
    await this.timeOperation(
      "Rust Cold Build",
      "./scripts/build-wrapper.sh release --quiet"
    );
    ...
    await this.timeOperation(
      "Rust Test Build",
      "./scripts/build-wrapper.sh test --quiet"
    );
  }
  ...
  async benchmarkNodeBuild(clean = false) {
    ...
    if (!fs.existsSync(path.join(nodeDir, "package.json"))) {
      this.log(
        "Node.js project not found, skipping Node.js benchmarks",
        "warn"
      );
      return;
    }
  ...
  generateReport() {
    ...
    report.summary.forEach((item) => {
      console.log(
        `${item.name}: ${item.avgDuration.toFixed(
          2
        )}ms (σ: ${item.stdDev.toFixed(2)}ms)`
      );
    });
  ...
  generateRecommendations() {
      ...
      if (coldBuild && warmBuild) {
        ...
        if (speedup > 2) {
          recommendations.push(
            `Rust caching is effective (${speedup.toFixed(1)}x speedup)`
          );
        } else {
    ...
    if (nodeBuilds.length > 1) {
      ...
      if (coldBuild && warmBuild) {
        ...
        if (speedup > 1.5) {
          recommendations.push(
            `Node.js caching is effective (${speedup.toFixed(1)}x speedup)`
          );
        }
      }
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        "All builds are performing well! Consider enabling distributed caching for team benefits."
      );
    }
