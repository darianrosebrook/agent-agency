  ...
  useEffect(() => {
    if (onMetricsUpdate && metrics) {
      const handleUpdate = (event: any) => {
        if (event.type === "coordination_update" && event.data?.coordination_metrics) {
          const newMetrics = event.data.coordination_metrics;
          ...
          setMetrics(prevMetrics => {
            ...
            if (newMetrics.consensus_rate !== undefined) {
              updated.consensus_rate = newMetrics.consensus_rate;
            }
            if (newMetrics.average_decision_time_ms !== undefined) {
              updated.average_decision_time_ms = newMetrics.average_decision_time_ms;
            }
            if (newMetrics.total_decisions !== undefined) {
              updated.total_decisions = newMetrics.total_decisions;
            }
            if (newMetrics.failed_decisions !== undefined) {
              updated.failed_decisions = newMetrics.failed_decisions;
            }
            if (newMetrics.active_tasks !== undefined) {
              updated.active_tasks = newMetrics.active_tasks;
            }
            if (newMetrics.queued_tasks !== undefined) {
              updated.queued_tasks = newMetrics.queued_tasks;
            }
            if (newMetrics.completed_tasks_per_hour !== undefined) {
              updated.completed_tasks_per_hour = newMetrics.completed_tasks_per_hour;
            }
            if (newMetrics.average_task_duration_ms !== undefined) {
              updated.average_task_duration_ms = newMetrics.average_task_duration_ms;
            }
            if (newMetrics.inter_agent_messages_per_minute !== undefined) {
              updated.inter_agent_messages_per_minute = newMetrics.inter_agent_messages_per_minute;
            }
            if (newMetrics.average_message_latency_ms !== undefined) {
              updated.average_message_latency_ms = newMetrics.average_message_latency_ms;
            }
            if (newMetrics.message_failure_rate !== undefined) {
              updated.message_failure_rate = newMetrics.message_failure_rate;
            }
      ...
      // For now, we'll listen to the parent component's updates
      // In a real implementation, this would be connected to a metrics stream
      // The parent component will call onMetricsUpdate when events occur
