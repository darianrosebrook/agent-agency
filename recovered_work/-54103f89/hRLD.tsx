...
export default function RealTimeMetricsStream({
  ...
  useEffect(() => {
    ...
    const mockEventGenerator = () => {
      ...
      const eventType = mockEventTypes[Math.floor(Math.random() * mockEventTypes.length)];
      ...
      switch (eventType) {
        case "health_update":
          mockData.data = {
            service: ["orchestrator", "worker", "database", "api"][Math.floor(Math.random() * 4)],
        ...
        case "coordination_update":
          mockData.data = {
            coordination_type: ["task_assignment", "resource_allocation", "conflict_resolution"][Math.floor(Math.random() * 3)],
        ...
        case "business_metrics":
          mockData.data = {
            metric_type: ["user_engagement", "task_completion", "system_efficiency"][Math.floor(Math.random() * 3)],
        ...
        case "error_rate":
          mockData.data = {
            service: ["api", "worker", "database", "coordinator"][Math.floor(Math.random() * 4)],
        ...
        case "throughput":
          mockData.data = {
            service: ["api", "worker", "database"][Math.floor(Math.random() * 3)],
    ...
    const intervalId = setInterval(mockEventGenerator, 1000 + Math.random() * 2000); // Random interval 1-3 seconds
