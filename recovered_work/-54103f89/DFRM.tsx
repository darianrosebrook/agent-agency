...
export default function RealTimeMetricsStream({
  ...
  const handleMetricsEvent = useCallback(
    (eventData: string) => {
      ...
      try {
        ...
        const v3Data: V3MetricsData = JSON.parse(eventData);
        ...
        console.error("Failed to process V3 metrics event:", error, eventData);
  ...
  useEffect(() => {
    ...
    const streamUrl = `${process.env.NEXT_PUBLIC_V3_BACKEND_HOST || 'http://localhost:8080'}/api/v1/metrics/stream`;
