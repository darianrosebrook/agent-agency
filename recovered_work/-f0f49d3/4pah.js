...
function calculateA11yScore(a11yResults, workingSpec) {
  try {
    ...
    violations.forEach((violation) => {
      ...
      console.log(`A11y violation: ${violation.id} (${impact}) - ${violation.description}`);
    ...
    if (specRequirements.length > 0) {
      const specViolations = specRequirements.filter((req) => {
        ...
        return !passes.some((pass) => pass.id?.includes(req) || pass.description?.includes(req));
      });

      if (specViolations.length > 0) {
        totalPenalty += 0.5 * specViolations.length;
        console.log(`Working spec a11y requirements not met: ${specViolations.join(', ')}`);
      }
    }
    ...
    console.log(`A11y score: ${(score * 100).toFixed(1)}% (${passCount} passed, ${violationCount} violations)`);
  ...
  incomplete: [
    {
      ...
      description: "aria-roledescription must be on elements with a semantic role"
    }
  ],
  passes: [
    {
