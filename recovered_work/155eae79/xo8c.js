  ...
  try {
    ...
    if (apiBudget && perfResults.api_p95_ms !== undefined) {
      ...
      if (actualApiTime > apiBudget) {
        ...
        violations++;
        console.log(`API p95 budget violation: ${actualApiTime}ms > ${apiBudget}ms (${(overrun * 100).toFixed(1)}% over)`);
      } else {
    ...
    if (lcpBudget && perfResults.lcp_ms !== undefined) {
      ...
      if (actualLcpTime > lcpBudget) {
        ...
        score -= Math.min(0.5, overrun * 0.3); // Penalty based on overrun percentage
        violations++;
        console.log(`LCP budget violation: ${actualLcpTime}ms > ${lcpBudget}ms (${(overrun * 100).toFixed(1)}% over)`);
      } else {
    ...
    if (perfResults.baseline_comparison) {
      ...
      if (regression.api_p95_regression > 0.05) { // 5% regression threshold
        score -= 0.1;
        violations++;
        console.log(`API performance regression: ${(regression.api_p95_regression * 100).toFixed(1)}% slower than baseline`);
      }
      if (regression.lcp_regression > 0.05) { // 5% regression threshold
        score -= 0.1;
        violations++;
        console.log(`LCP performance regression: ${(regression.lcp_regression * 100).toFixed(1)}% slower than baseline`);
      }
    }
    ...
    if (perfResults.error_rate !== undefined && perfResults.error_rate > 0.01) { // >1% errors
      score -= 0.2;
      violations++;
      console.log(`High error rate: ${(perfResults.error_rate * 100).toFixed(2)}%`);
    }
    ...
    console.log(`Performance score: ${(score * 100).toFixed(1)}% (${violations} violations)`);
    if (apiBudget) {
      console.log(`API budget: ${apiBudget}ms (actual: ${perfResults.api_p95_ms || 'N/A'}ms)`);
    }
    if (lcpBudget) {
      console.log(`LCP budget: ${lcpBudget}ms (actual: ${perfResults.lcp_ms || 'N/A'}ms)`);
    }
