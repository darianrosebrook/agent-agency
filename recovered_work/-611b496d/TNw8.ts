...
export class TaskApiClient {
  ...
  async getArbiterVerdict(taskId: string): Promise<any> {
    try {
      const response = await apiClient.get(`${this.baseUrl}/${taskId}/arbiter-verdict`);
      ...
      throw new TaskApiError("server_error", "Failed to get arbiter verdict", true);
    }
  }
  ...
  async getClaimVerificationData(taskId: string): Promise<any> {
    try {
      const response = await apiClient.get(`${this.baseUrl}/${taskId}/claim-verification`);
      return response.data;
    } catch (error: any) {
      if (error.response?.status === 404) {
      ...
      throw new TaskApiError("server_error", "Failed to get claim verification data", true);
    }
  }
  ...
  async requestWaiver(taskId: string, reason: string): Promise<any> {
    try {
      const response = await apiClient.post(`${this.baseUrl}/${taskId}/waiver`, {
        reason,
      });
      return response.data;
    } catch (error: any) {
  ...
  async appealVerdict(taskId: string, reason: string): Promise<any> {
    try {
      const response = await apiClient.post(`${this.baseUrl}/${taskId}/appeal`, {
        reason,
      });
      return response.data;
    } catch (error: any) {
