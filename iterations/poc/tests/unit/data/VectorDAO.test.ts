...
class TestVectorDAO extends VectorDAO<TestVectorEntity> {
  ...
  protected getValues(
    entity: Omit<TestVectorEntity, "id" | "createdAt" | "updatedAt">
  ): any[] {
    return [entity.name];
  }

  protected mapRowToEntity(row: any): TestVectorEntity {
    return new TestVectorEntity(row.id, row.tenant_id, row.embedding, row.name);
  }
...
describe("VectorDAO", () => {
  ...
  describe("vector validation", () => {
    it("should validate correct vectors", () => {
      ...
      expect(() =>
        (vectorDAO as any).validateVector(validVector)
      ).not.toThrow();
    });

    it("should reject empty vectors", () => {
      expect(() => (vectorDAO as any).validateVector([])).toThrow(
        "Vector must be a non-empty array"
      );
    });

    it("should reject non-array inputs", () => {
      expect(() => (vectorDAO as any).validateVector("not an array")).toThrow(
        "Vector must be a non-empty array"
      );
    });
    ...
    it("should reject vectors with non-numeric values", () => {
      expect(() =>
        (vectorDAO as any).validateVector([0.1, "invalid", 0.3])
      ).toThrow("All vector values must be valid numbers");
    });

    it("should reject NaN values", () => {
      expect(() => (vectorDAO as any).validateVector([0.1, NaN, 0.3])).toThrow(
        "All vector values must be valid numbers"
      );
    });

    it("should reject vectors that are too large", () => {
      const largeVector = new Array(5000).fill(0.1);
      expect(() => (vectorDAO as any).validateVector(largeVector)).toThrow(
        "Vector dimension cannot exceed 4096"
      );
    });
  });
  ...
  describe("similarity search", () => {
    ...
    it("should handle search errors", async () => {
      ...
      await expect(vectorDAO.findSimilar(testVector)).rejects.toThrow(
  ...
  describe("similarity search by ID", () => {
    ...
    it("should handle entity without embedding", async () => {
      ...
      await expect(
        vectorDAO.findSimilarById(testId, testTenantId)
        ...
        data: [
          {
            id: "hybrid1",
            score: 0.88,
            metadata: { type: "document", relevance: 0.9 },
          },
          {
            id: "hybrid2",
            score: 0.76,
            metadata: { type: "document", relevance: 0.7 },
          },
        ],
        duration: 20,
  ...
  describe("statistics", () => {
    ...
    it("should handle statistics query errors", async () => {
      ...
      await expect(vectorDAO.getVectorStats("tenant-123")).rejects.toThrow(
