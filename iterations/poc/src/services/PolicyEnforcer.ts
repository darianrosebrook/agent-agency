 ...
 *
 * Extracted from CAWS constitutional enforcer to provide centralized
 * policy enforcement, budget management, and quality gate checking.
 *
...
export class PolicyEnforcer extends EventEmitter {
  ...
  // Default policy limits by tier
  private readonly POLICY_LIMITS = {
    1: {
      // Critical/Auth tier
      maxFiles: 40,
      maxLoc: 1500,
  ...
  /**
   * Enforce policy limits before task execution
   */
  async enforcePolicy(
    taskId: string,
    tenantId: string,
    ...
    try {
      ...
      if (
        ...
        this.logger.info(`Task ${taskId} allowed via policy waiver(s)`);
      }

      // Log enforcement decision
      this.logger.info(
        `Policy enforcement for task ${taskId}: ${
      ...
      this.emit("policy-decision", { taskId, tenantId, result });

      return result;
    } catch (error) {
      this.logger.error(`Policy enforcement failed for task ${taskId}:`, error);
  ...
  startBudgetTracking(taskId: string, tenantId: string, tier: number): void {
    const limits =
      this.POLICY_LIMITS[tier as keyof typeof this.POLICY_LIMITS] ||
      this.POLICY_LIMITS[2];
  ...
  async requestWaiver(
    ...
    this.logger.info(`Policy waiver requested: ${waiverId} (${request.title})`);
  ...
  private async getQualityMetrics(tenantId: string): Promise<{
    ...
    // This would integrate with actual quality tools to get real metrics
    ...
    try {
      await this.memoryManager.storeExperience(budget.tenantId, {
        ...
        contextMatch: {
          ...
          semanticMatches: ["task budget", "resource usage", "policy limits"],
