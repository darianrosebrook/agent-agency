...
import { ExperienceEntity, QueryOptions, QueryResult, VectorSearchOptions } from "../types";
...
export class ExperienceDAO extends VectorDAO<ExperienceEntity> {
  ...
  async findByAgentWithDecay(
    ...
    try {
      ...
      const experiences = (result.data || []).map(row => this.mapRowToEntity(row));
      ...
      this.logger.error("Failed to find experiences by agent", { agentId, tenantId, error });
      throw error;
    }
  }
  ...
  async findSimilarByTaskType(
    ...
    try {
      ...
      const result = await this.dataLayer.query<ExperienceEntity>(query, params, {
        cache: options.cache,
        timeout: options.timeout
      });

      if (!result.success) {
        throw new Error(`Failed to find similar experiences by task type: ${result.error}`);
      }
  ...
  async getInsightsByAgent(
    ...
    try {
      ...
      this.logger.error("Failed to get experience insights", { tenantId, error });
      throw error;
    }
  }
  ...
  async cleanupOldExperiences(
    ...
    try {
      const result = await this.dataLayer.query<{ deleted: number }>(
      ...
      this.logger.error("Failed to cleanup old experiences", { tenantId, error });
      throw error;
    }
  }
  ...
  protected getValues(entity: Omit<ExperienceEntity, "id" | "createdAt" | "updatedAt">): any[] {
    return [
      entity.agentId,
      entity.taskId,
      entity.type,
  ...
  protected mapRowToEntity(row: any): ExperienceEntity {
    return {
      ...
      contextMatch: typeof row.context_match === "string" ? JSON.parse(row.context_match) : row.context_match,
      reasoningPath: row.reasoning_path ? (typeof row.reasoning_path === "string" ? JSON.parse(row.reasoning_path) : row.reasoning_path) : undefined,
      temporalRelevance: typeof row.temporal_relevance === "string" ? JSON.parse(row.temporal_relevance) : row.temporal_relevance,
      weight: row.weight,
