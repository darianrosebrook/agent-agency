...
describe("KnowledgeSeeker", () => {
  ...
  describe("Query Processing", () => {
    ...
    it("should reject invalid queries", async () => {
      ...
      for (const invalidQuery of invalidQueries) {
        await expect(
          knowledgeSeeker.processQuery(invalidQuery)
        ).rejects.toThrow();
      }
    });
  ...
  describe("Result Processing", () => {
    it("should filter results by relevance threshold", async () => {
      ...
      expect(response.results.length).toBeLessThanOrEqual(
        highThresholdQuery.maxResults
      );

      // All results should meet threshold
      response.results.forEach((result) => {
        expect(result.relevanceScore).toBeGreaterThanOrEqual(
          highThresholdQuery.relevanceThreshold
        );
      });
    });
    ...
    it("should calculate confidence scores", async () => {
      ...
      const highQualityCount = response.results.filter(
        (r) => r.quality === "high"
      ).length;
