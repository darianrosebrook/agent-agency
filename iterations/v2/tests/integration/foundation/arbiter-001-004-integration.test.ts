...
describe('ARBITER Foundation Integration Tests (001-004)', () => {
  ...
  beforeEach(async () => {
    ...
    router = new TaskRoutingManager(registry, {
      enableBandit: true,
      minAgentsRequired: 1,
  ...
  describe('ARBITER-001: Agent Querying', () => {
    ...
    it('should rank agents by match score', async () => {
      ...
      // Full-stack agent should rank higher due to more language matches
    ...
    it('should route tasks to best-matching agents', async () => {
      ...
      await registry.registerAgent(
        createAgentWithCapabilities({
          languages: ['TypeScript'],
          taskTypes: ['code-editing'],
        }, { id: 'ts-specialist', performanceHistory: {
          successRate: 0.95,
          averageQuality: 0.9,
          averageLatency: 100,
          taskCount: 100,
      ...
      await registry.registerAgent(
        createAgentWithCapabilities({
          ...
          taskTypes: ['code-editing'],
        }, { id: 'general-agent', performanceHistory: {
          successRate: 0.7,
          averageQuality: 0.7,
          averageLatency: 200,
          taskCount: 50,
  ...
  describe('ARBITER-003: CAWS Validation', () => {
    ...
    it('should reject spec with missing acceptance criteria', async () => {
      ...
      const validation = await validator.validateWorkingSpec(invalidSpec as any);
    ...
    it('should validate specs for different risk tiers', async () => {
      ...
      expect(tier1Validation.valid).toBe(true);
    });
  });
  ...
  describe('ARBITER-004: Performance Tracking', () => {
    it('should track agent performance across tasks', async () => {
      ...
      expect(updatedAgent.performanceHistory.averageQuality).toBeGreaterThan(0.85);
    });
  });
  ...
  describe('Multi-Component Integration Workflows', () => {
    it('should complete full workflow: register → validate → route → track', async () => {
      ...
      expect(updatedAgent.performanceHistory.successRate).toBe(1.0);
      expect(updatedAgent.performanceHistory.averageQuality).toBeCloseTo(0.9, 1);
    });
  ...
  describe('Load Testing', () => {
    it('should handle 50 concurrent task routings', async () => {
      ...
      console.log(`✅ Integration: Routed 50 tasks in ${duration}ms across ${agentTaskCounts.size} agents`);
    });
    ...
    it('should maintain data consistency under concurrent operations', async () => {
      ...
      const outcomes = Array.from({ length: 20 }, (_, i) =>
        createMockOutcome(i % 4 !== 0, { // 75% success rate
          latencyMs: 100 + i * 5,
      ...
      await Promise.all(
        outcomes.map((outcome) =>
          registry.updatePerformance(agent.id, outcome)
        )
      );
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle routing when no agents are available', async () => {
      ...
      // Routing should still return a result (may indicate no agent found)
      const routing = await router.routeTask(task);
      expect(routing).toBeDefined();
      // selectedAgent may be undefined or a fallback
    });
