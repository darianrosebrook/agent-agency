...
describe('Agent Registry Persistence Integration', () => {
  ...
  describe('Database Persistence', () => {
    it('should persist agent registration to database', async () => {
      ...
      const mockRegister = jest.spyOn(dbClient, 'registerAgent').mockResolvedValue('test-agent-1');
    ...
    it('should rollback in-memory storage on database failure', async () => {
      ...
      const mockRegister = jest.spyOn(dbClient, 'registerAgent').mockRejectedValue(
        new Error('Database connection failed')
      );

      // Act & Assert
      await expect(registry.registerAgent(agentData)).rejects.toThrow(RegistryError);
      ...
      await expect(registry.getProfile('test-agent-fail')).rejects.toThrow(RegistryError);
    });
    ...
    it('should load agent from database when not in memory cache', async () => {
      ...
      const mockGet = jest.spyOn(dbClient, 'getAgent').mockResolvedValue(mockAgent);
    ...
    it('should record performance metrics to database', async () => {
      ...
      const mockRecord = jest.spyOn(dbClient, 'recordPerformance').mockResolvedValue(undefined);

      // Act
      await registry.updatePerformance(agentId, metrics);

      // Assert
      expect(mockRecord).toHaveBeenCalledWith(agentId, expect.objectContaining({
    ...
    it('should continue operation when performance recording fails', async () => {
      ...
      const mockRecord = jest.spyOn(dbClient, 'recordPerformance').mockRejectedValue(
  ...
  describe('Database Error Handling', () => {
    ...
    it('should handle database retrieval failures gracefully', async () => {
      ...
      const mockGet = jest.spyOn(dbClient, 'getAgent').mockRejectedValue(
      ...
      await expect(registry.getProfile(agentId)).rejects.toMatchObject({
        type: RegistryErrorType.DATABASE_ERROR,
        message: expect.stringContaining('Failed to retrieve agent from database'),
      });
    });
  });
  ...
  describe('ACID Compliance', () => {
    it('should maintain atomicity - all operations succeed or all fail', async () => {
      ...
      let callCount = 0;
      const mockRegister = jest.spyOn(dbClient, 'registerAgent').mockImplementation(async () => {
        callCount++;
        if (callCount === 1) {
      ...
      await expect(registry.getProfile('acid-test-agent')).rejects.toThrow(RegistryError);
