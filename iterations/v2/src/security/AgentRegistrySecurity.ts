...
export class AgentRegistrySecurity {
  ...
  async authorize(
    ...
    try {
      ...
      if (this.config.blockedUserIds?.includes(context.userId)) {
        await this.logSecurityViolation(context, action, resourceType, resourceId, 'User is blocked');
        return false;
      }
      ...
      if (resource && this.isCrossTenantAccess(context, resource)) {
        await this.logSecurityViolation(context, action, resourceType, resourceId, 'Cross-tenant access attempt');
        return false;
      }

      // Check rate limiting
      if (!this.checkRateLimit(context)) {
        await this.logSecurityViolation(context, action, resourceType, resourceId, 'Rate limit exceeded');
        return false;
      }

      // Check permissions based on action
      const requiredPermission = this.getRequiredPermission(action, resourceType);
      if (!context.permissions.includes(requiredPermission)) {
        await this.logAuthorizationFailure(context, action, resourceType, resourceId, requiredPermission);
        return false;
      }
  ...
  validateAgentData(data: Partial<AgentProfile>): ValidationResult {
    ...
    if (!data.modelFamily || typeof data.modelFamily !== 'string') {
      ...
      const allowedFamilies = ['gpt-4', 'claude-3', 'claude-3.5', 'gemini-pro', 'llama-3', 'mixtral'];
      if (!allowedFamilies.includes(data.modelFamily)) {
        errors.push(`Model family must be one of: ${allowedFamilies.join(', ')}`);
      }
    }
    ...
    if (data.capabilities) {
      if (typeof data.capabilities !== 'object') {
        ...
        // Validate task types
        if (caps.taskTypes) {
          const validTaskTypes = ['code-editing', 'research', 'code-review', 'documentation', 'testing', 'debugging', 'refactoring', 'api-design'];
        ...
        // Validate languages
        if (caps.languages) {
          const validLanguages = ['TypeScript', 'JavaScript', 'Python', 'Rust', 'Go', 'Java', 'C++', 'C#'];
  ...
  async logAuditEvent(event: AuditEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.debug('Audit event logged', { eventType: event.eventType, result: event.result });
    } catch (error) {
   ...
   */
  private isCrossTenantAccess(context: SecurityContext, resource: Partial<AgentProfile>): boolean {
   ...
   */
  private getRequiredPermission(action: AuditAction, resourceType: string): string {
  ...
  getSecurityStats(): {
    ...
    const violations = this.auditEvents.filter(e => e.eventType === AuditEventType.SECURITY_VIOLATION).length;
    const authFailures = this.auditEvents.filter(e => e.eventType === AuditEventType.AUTHENTICATION_FAILURE).length;
    const authzFailures = this.auditEvents.filter(e => e.eventType === AuditEventType.AUTHORIZATION_FAILURE).length;
