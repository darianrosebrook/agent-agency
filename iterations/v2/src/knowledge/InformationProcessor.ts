...
export class InformationProcessor implements IInformationProcessor {
  ...
  async processResults(
    ...
    try {
      ...
      processedResults = processedResults.filter(
        (result) => result.credibilityScore >= this.config.minCredibilityScore
      );
      ...
      processedResults.forEach((result) => {
        ...
        result.quality = this.determineQuality(
      ...
      processedResults = processedResults.slice(
        0,
        this.config.maxResultsToProcess
      );
      ...
      console.error(
        `Information processing failed for query ${query.id}:`,
        error
      );
  ...
  assessCredibility(result: SearchResult): number {
    ...
    const suspiciousDomains = ["blogspot", "wordpress", "medium", "reddit"];
  ...
  generateSummary(query: KnowledgeQuery, results: SearchResult[]): string {
    ...
    const highQualityResults = results.filter(
      (r) => r.quality === ResultQuality.HIGH
    );
  ...
  private applyDiversityConstraints(results: SearchResult[]): SearchResult[] {
    ...
    if (
      sourceTypes.size < diversity.minSourceTypes &&
      filteredResults.length > diversity.minSourceTypes
    ) {
      ...
      for (const result of filteredResults) {
        if (
          !typesAdded.has(result.sourceType) &&
          typesAdded.size < diversity.minSourceTypes
        ) {
      ...
      const remaining = filteredResults.filter(
      ...
      diverseResults.push(
        ...remaining.slice(0, diversity.minSources - diverseResults.length)
      );
  ...
  private createContentHash(result: SearchResult): string {
    ...
    const content = `${result.title}${result.content.substring(
      0,
      500
    )}`.toLowerCase();
    let hash = 0;
