...
CREATE TABLE agent_performance_profiles (
    ...
    trend_magnitude DECIMAL(4,3) NOT NULL CHECK (trend_magnitude >= -1 AND trend_magnitude <= 1),
    trend_confidence DECIMAL(3,2) NOT NULL CHECK (trend_confidence >= 0 AND trend_confidence <= 1),
    trend_time_window_hours INTEGER NOT NULL CHECK (trend_time_window_hours > 0),

    -- Outlier information
    outlier_count INTEGER NOT NULL DEFAULT 0,
    outlier_events JSONB,
...
CREATE INDEX idx_agent_performance_profiles_window_time ON agent_performance_profiles (aggregation_window, end_time DESC);
CREATE INDEX idx_agent_performance_profiles_confidence ON agent_performance_profiles (confidence_score DESC);
CREATE INDEX idx_agent_performance_profiles_trend ON agent_performance_profiles (trend_direction, trend_confidence DESC);
...
CREATE TABLE benchmark_datasets (
    ...
    access_pattern VARCHAR(50) CHECK (access_pattern IN ('local', 's3', 'http', 'database')),
...
CREATE TABLE benchmark_evaluations (
    ...
    -- Results
    overall_score DECIMAL(4,3) CHECK (overall_score >= 0 AND overall_score <= 1),
...
CREATE INDEX idx_benchmark_evaluations_score ON benchmark_evaluations (overall_score DESC) WHERE status = 'completed';
...
CREATE TABLE rl_training_batches (
    ...
    anonymization_level VARCHAR(20) NOT NULL DEFAULT 'differential' CHECK (anonymization_level IN ('basic', 'differential', 'secure')),

    -- Batch status
    status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'ready', 'consumed', 'expired')),
...
CREATE INDEX idx_rl_training_batches_quality ON rl_training_batches (quality_score DESC) WHERE status = 'ready';
CREATE INDEX idx_rl_training_batches_created ON rl_training_batches (created_at DESC);
CREATE INDEX idx_rl_training_batches_expires ON rl_training_batches (expires_at) WHERE status = 'ready';
...
CREATE TABLE system_health_metrics (
    ...
    -- Additional context
    system_status VARCHAR(20) DEFAULT 'healthy' CHECK (system_status IN ('healthy', 'degraded', 'critical', 'maintenance')),
...
CREATE INDEX idx_system_health_metrics_anomalies ON system_health_metrics (active_anomalies, critical_anomalies);
...
COMMENT ON TABLE performance_events IS 'Raw performance events from all agent interactions with integrity verification';
COMMENT ON TABLE agent_performance_profiles IS 'Aggregated performance profiles for agents with trend analysis';
COMMENT ON TABLE benchmark_datasets IS 'Benchmark datasets for model evaluation and comparison';
COMMENT ON TABLE benchmark_evaluations IS 'Results from benchmark evaluations against datasets';
COMMENT ON TABLE rl_training_batches IS 'Batches of RL training data ready for model training';
COMMENT ON TABLE performance_anomalies IS 'Detected performance anomalies with impact assessment';
COMMENT ON TABLE data_quality_metrics IS 'Data quality metrics for monitoring pipeline health';
COMMENT ON TABLE system_health_metrics IS 'System-wide performance health metrics';
