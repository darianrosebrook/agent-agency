...
export default function DatabaseAuditPanel({
  apiClient: _apiClient,
}: DatabaseAuditPanelProps) {
      ...
      <div className="mt-3 grid grid-cols-4 gap-4 text-sm">
        <div className="text-center">
          <div className="font-semibold text-blue-900">
            {auditInfo.totalTables}
          </div>
          <div className="text-blue-700">Tables</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-blue-900">
            {auditInfo.totalRows.toLocaleString()}
          </div>
          <div className="text-blue-700">Total Rows</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-blue-900">
            {formatBytes(auditInfo.totalSizeBytes)}
          </div>
          <div className="text-blue-700">Total Size</div>
        </div>
        <div className="text-center">
          <div className="font-semibold text-blue-900">
            {auditInfo.performanceMetrics.activeConnections}
          </div>
                    ...
                    {auditInfo.tables.map((table) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          <span
                            className={`text-xs font-medium ${getTableHealthColor(
                              table
                            )}`}
                          >
                            {table.rowCount === 0
                              ? "Empty"
                              : table.sizeBytes > 1000000
                              ? "Large"
                              : table.rowCount > 1000
                              ? "Active"
                              : "Healthy"}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-white p-3 rounded border">
                ...
                <div className="font-medium text-lg">
                  {auditInfo.performanceMetrics.avgQueryTime}ms
                </div>
              </div>
              <div className="bg-white p-3 rounded border">
                <div className="text-gray-600">Cache Hit Ratio</div>
                <div className="font-medium text-lg">
                  {auditInfo.performanceMetrics.cacheHitRatio}%
                </div>
              </div>
              <div className="bg-white p-3 rounded border">
              ...
              <div className="bg-white p-3 rounded border">
                ...
                <div className="font-medium text-lg">
                  {auditInfo.performanceMetrics.slowQueries}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {auditInfo.totalRows > 10000 && (
                <div className="text-blue-700 bg-blue-100 p-2 rounded">
                  📈 Database is actively used (
                  {auditInfo.totalRows.toLocaleString()} total rows)
                </div>
              )}
            </div>
          </div>
