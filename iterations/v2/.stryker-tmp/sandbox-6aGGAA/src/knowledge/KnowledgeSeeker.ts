...
export class KnowledgeSeeker implements IKnowledgeSeeker {
    ...
    try {
      ...
      searchResults.forEach((result, index) => {
        ...
        if (result.status === "fulfilled") {
          ...
          console.warn(`Search failed for provider ${provider.name}:`, result.reason);
      ...
      const processedResults = await this.processor.processResults(query, allResults);
  ...
  private validateQuery(query: KnowledgeQuery): void {
    ...
    if (query.timeoutMs <= 0 || query.timeoutMs > 300000) { // 5 minutes max
  ...
  private selectProviders(query: KnowledgeQuery): ISearchProvider[] {
    const availableProviders = Array.from(this.providers.values()).filter(async p =>
      await p.isAvailable()
    );
    ...
    switch (query.queryType) {
      case QueryType.TECHNICAL:
        ...
        suitableProviders = suitableProviders.filter(p =>
          p.type === "documentation_search" || p.type === "web_search"
        );
        break;
   ...
   */
  private cacheQueryResponse(query: KnowledgeQuery, response: KnowledgeResponse): void {
    const cacheKey = this.generateCacheKey(query);

    // Set cache expiry
    const expiresAt = new Date();
    expiresAt.setMilliseconds(expiresAt.getMilliseconds() + this.config.caching.cacheTtlMs);

    // Store with expiry (simplified - in production use proper cache)
    this.queryCache.set(cacheKey, response);

    // Clean up expired entries periodically
    if (this.queryCache.size > 100) { // Arbitrary cleanup trigger
  ...
  private calculateResponseConfidence(results: any[]): number {
    ...
    const qualityBonus = results.filter(r => r.quality === "high").length / results.length;

    return Math.min(1.0, (avgRelevance + avgCredibility) / 2 + qualityBonus * 0.1);
  }
  ...
  private cleanupExpiredCache(): void {
    ...
    for (const [key, response] of Array.from(this.queryCache.entries())) {
      if (now - response.respondedAt.getTime() > (this.config.caching.cacheTtlMs || 3600000)) {
