...
export abstract class BaseSearchProvider implements ISearchProvider {
  ...
  async isAvailable(): Promise<boolean> {
    ...
    // Check rate limits
    if (this.requestCounts.thisMinute >= this.config.rateLimit.requestsPerMinute) {
      return false;
    }
  ...
  protected async executeRequest<T>(
    ...
    try {
      ...
      // Update health metrics
      this.updateHealthMetrics(responseTime, true, error instanceof Error ? error.message : String(error));
   ...
   */
  private updateHealthMetrics(responseTime: number, hadError: boolean, errorMessage?: string): void {
    ...
    // Update error rate
    const totalRequests = this.healthStatus.requestsThisMinute + this.healthStatus.requestsThisHour;
    if (totalRequests > 0) {
      this.healthStatus.errorRate =
        (this.healthStatus.errorRate * (totalRequests - 1) + (hadError ? 1 : 0)) / totalRequests;
    }
  ...
  private resetCountersIfNeeded(): void {
    const now = Date.now();
    const minutesSinceReset = (now - this.requestCounts.lastReset.getTime()) / (1000 * 60);
  ...
  protected createSearchResult(
    ...
    return {
      ...
      content: providerData.snippet || providerData.description || providerData.content || "",
      ...
      publishedAt: providerData.publishedDate ? new Date(providerData.publishedDate) : undefined,
      provider: this.name,
  ...
  private inferSourceType(providerData: any): SourceType {
    ...
    if (domain.includes("news") || domain.includes("cnn") || domain.includes("bbc") ||
        domain.includes("nytimes") || domain.includes("reuters")) {
      return "news";
    }

    // Documentation sources
    if (domain.includes("docs") || domain.includes("github") || domain.includes("stackoverflow")) {
   ...
   */
  private assessQuality(relevanceScore: number, credibilityScore: number): ResultQuality {
...
export class GoogleSearchProvider extends BaseSearchProvider {
  ...
  async search(query: KnowledgeQuery): Promise<SearchResult[]> {
    ...
    try {
      ...
      const url = `https://www.googleapis.com/customsearch/v1?key=${this.config.apiKey}&cx=${this.config.options.searchEngineId}&q=${searchQuery}&num=${Math.min(query.maxResults, 10)}`;
  ...
  async search(query: KnowledgeQuery): Promise<SearchResult[]> {
    ...
    try {
      ...
      if (data.RelatedTopics) {
        data.RelatedTopics.slice(0, query.maxResults - results.length).forEach((topic: any) => {
...
export class ArXivSearchProvider extends BaseSearchProvider {
  ...
  async search(query: KnowledgeQuery): Promise<SearchResult[]> {
    ...
    try {
      const searchQuery = encodeURIComponent(query.query);
      const url = `http://export.arxiv.org/api/query?search_query=all:${searchQuery}&start=0&max_results=${Math.min(query.maxResults, 10)}`;
      ...
      if (titleMatch && summaryMatch && idMatch) {
        results.push(this.createSearchResult(
          queryId,
          {
            ...
            snippet: summaryMatch[1].replace(/<!\[CDATA\[|\]\]>/g, '').substring(0, 500),
            url: idMatch[1],
...
export class MockSearchProvider extends BaseSearchProvider {
  ...
  async search(query: KnowledgeQuery): Promise<SearchResult[]> {
    ...
    for (let i = 0; i < Math.min(query.maxResults, 3); i++) {
      mockResults.push(this.createSearchResult(
        query.id,
        {
          title: `Mock Result ${i + 1} for "${query.query}"`,
          snippet: `This is a mock search result ${i + 1} containing information about ${query.query}.`,
