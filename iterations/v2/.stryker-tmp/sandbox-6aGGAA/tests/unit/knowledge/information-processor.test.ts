...
describe("InformationProcessor", () => {
    ...
    mockResults = [
      {
        ...
        content: "TypeScript is a programming language developed by Microsoft. It is a strict syntactical superset of JavaScript.",
  ...
  describe("Result Processing", () => {
    it("should process results and filter by quality thresholds", async () => {
      ...
      processed.forEach(result => {
        expect(result.relevanceScore).toBeGreaterThanOrEqual(defaultConfig.minRelevanceScore);
        expect(result.credibilityScore).toBeGreaterThanOrEqual(defaultConfig.minCredibilityScore);
      });
    });
    ...
    it("should respect max results limit", async () => {
      ...
      const processed = await limitedProcessor.processResults(testQuery, mockResults);
    ...
    it("should sort results by combined score", async () => {
      ...
      for (let i = 1; i < processed.length; i++) {
        const currentScore = processed[i].relevanceScore + processed[i].credibilityScore;
        const previousScore = processed[i - 1].relevanceScore + processed[i - 1].credibilityScore;
  ...
  describe("Relevance Scoring", () => {
    ...
    it("should adjust scores based on query type", () => {
      ...
      const technicalScore = processor.scoreRelevance(technicalQuery, docResult);
    ...
    it("should boost scores for recent content", () => {
      ...
      const oldResult = { ...mockResults[3], publishedAt: new Date("2010-01-01") };
  ...
  describe("Credibility Assessment", () => {
    ...
    it("should penalize suspicious domains", () => {
      ...
      const suspiciousCredibility = processor.assessCredibility(suspiciousResult);
      const regularCredibility = processor.assessCredibility(regularResult);
  ...
  describe("Diversity Constraints", () => {
    it("should respect domain limits", async () => {
      const manySameDomain = Array(5).fill(null).map((_, i) => ({
        ...mockResults[0],
        id: `duplicate-${i}`,
        url: `https://typescriptlang.org/page${i}`,
      }));

      const processed = await processor.processResults(testQuery, manySameDomain);

      // Should limit to maxResultsPerDomain
      const domainCount = processed.filter(r => r.domain === "typescriptlang.org").length;
      expect(domainCount).toBeLessThanOrEqual(defaultConfig.diversity.maxResultsPerDomain);
    });
    ...
    it("should ensure source type diversity when possible", async () => {
      ...
      const processed = await processor.processResults(testQuery, diverseResults);

      const sourceTypes = new Set(processed.map(r => r.sourceType));
      expect(sourceTypes.size).toBeGreaterThanOrEqual(defaultConfig.diversity.minSourceTypes);
    });
  });
  ...
  describe("Configuration", () => {
    it("should respect disabled features", async () => {
      ...
      const processed = await disabledProcessor.processResults(testQuery, mockResults);
    ...
    it("should handle edge case configurations", async () => {
      ...
      const processed = await edgeProcessor.processResults(testQuery, mockResults);
