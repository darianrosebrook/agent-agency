...
describe("SearchProvider", () => {
  ...
  describe("SearchProviderFactory", () => {
    ...
    it("should throw error for unknown provider type", () => {
      ...
      expect(() => SearchProviderFactory.createProvider(invalidConfig)).toThrow();
    });
  });
  ...
  describe("BaseSearchProvider", () => {
    ...
    it("should assess result quality correctly", async () => {
      ...
      results.forEach(result => {
        expect(["high", "medium", "low", "unreliable"]).toContain(result.quality);

        // Quality should correlate with scores
        if (result.quality === "high") {
          expect(result.relevanceScore + result.credibilityScore).toBeGreaterThan(1.2);
        }
      });
    });
