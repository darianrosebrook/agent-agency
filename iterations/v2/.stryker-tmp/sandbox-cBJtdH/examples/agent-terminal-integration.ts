...
class EnhancedAgent {
  ...
  private async cleanupTerminalSession(
    taskId: string,
    sessionId: string
  ): Promise<void> {
    try {
      ...
      console.log(
        `Cleaned up terminal session ${sessionId} for task ${taskId}`
      );
    } catch (error) {
  ...
  private async executeTestSuite(
    ...
    if (task.metadata?.installDeps) {
      const installResult = await this.safeExecuteCommand(sessionId, "npm", [
        "ci",
      ]);
    ...
    // Run linting
    if (task.metadata?.runLint !== false) {
      // Default true
      const lintResult = await this.safeExecuteCommand(sessionId, "npm", [
        "run",
        "lint",
      ]);
    ...
    const testResult = await this.safeExecuteCommand(
      sessionId,
      "npm",
      testArgs,
      300000
    ...
    const summary = results
      .map((r) => `${r.stage}: ${r.success ? "PASS" : "FAIL"}`)
      .join(", ");
  ...
  private async executeBuild(
    ...
    const buildResult = await this.safeExecuteCommand(
      sessionId,
      "npm",
      buildArgs,
      timeout
  ...
  private async executeDeployment(
    ...
    if (task.metadata?.healthCheck) {
      const healthResult = await this.safeExecuteCommand(sessionId, "npm", [
        "run",
        "health-check",
      ]);
    ...
    if (task.metadata?.buildBeforeDeploy) {
      const buildResult = await this.safeExecuteCommand(sessionId, "npm", [
        "run",
        "build",
      ]);
    ...
    const deployResult = await this.safeExecuteCommand(
      sessionId,
      "npm",
      deployCommand,
      600000
    ...
    if (task.metadata?.postDeployCheck) {
      const verifyResult = await this.safeExecuteCommand(sessionId, "npm", [
        "run",
        "post-deploy-check",
      ]);
    ...
    const summary = steps
      .map((s) => `${s.step}: ${s.success ? "OK" : "FAILED"}`)
      .join(" → ");
  ...
  private async executePackageManagement(
    task: Task,
    sessionId: string
  ): Promise<TaskResult> {
    ...
    const result = await this.safeExecuteCommand(
      sessionId,
      packageManager,
      args
    );
    ...
    return {
      ...
      output: result.success
        ...
        : `Package management failed: ${result.stderr}`,
      metadata: {
        packageManager,
        command,
  ...
  private async executeCustomTerminalTask(
    task: Task,
    sessionId: string
  ): Promise<TaskResult> {
      ...
      if (!result.success) {
        ...
        switch (result.error) {
          ...
          case "UNSAFE_ARGUMENTS":
            throw new Error(
              `Unsafe arguments in command: ${command} ${args.join(" ")}`
            );
