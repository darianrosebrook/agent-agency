...
-- Partial index for active requests (removed due to IMMUTABLE function requirement)
-- CREATE INDEX IF NOT EXISTS idx_verification_requests_active ON verification_requests (created_at DESC)
-- WHERE
...
-- Error information
error_message TEXT, error_code VARCHAR(50),

-- Audit trail
verified_by VARCHAR(100), verification_version VARCHAR(20),

-- Result cache
result_hash VARCHAR(64) UNIQUE, cache_expiry TIMESTAMPTZ );
...
CREATE INDEX IF NOT EXISTS idx_verification_results_cache_expiry ON verification_results (cache_expiry)
WHERE
    cache_expiry IS NOT NULL;
...
-- Cached result
result_data JSONB NOT NULL, result_hash VARCHAR(64) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_verification_cache_access ON verification_cache (
...
VALUES (
        ...
        1,
        10000,
        '{"providers": ["google_fact_check", "snopes", "factcheck_org"]}'
    ),
    (
        'source_credibility',
        true,
        2,
        5000,
        '{"credibility_database": "media_bias_fact_check"}'
    ),
    (
        'cross_reference',
        true,
        3,
        15000,
        '{"max_sources": 5, "min_consensus": 0.7}'
    ),
    (
        'consistency_check',
        true,
    ...
    (
        ...
        6,
        10000,
        '{"statistical_tests": ["chi_square", "correlation", "significance"]}'
    ),
    (
        'insufficient_data',
        false,
        ...
        1000,
        '{}'
    ) -- Disabled by default, used for fallback
...
SELECT
    ...
    COUNT(
        CASE
            WHEN supporting_claim THEN 1
        END
    ) as supporting_count,
    COUNT(
        CASE
            WHEN NOT supporting_claim THEN 1
        END
...
COMMENT ON
TABLE verification_requests IS 'Stores verification requests with content, metadata, and processing status';

COMMENT ON
TABLE verification_results IS 'Stores verification results with confidence scores, evidence, and reasoning';

COMMENT ON
TABLE verification_methods IS 'Tracks performance and configuration of individual verification methods';

COMMENT ON
TABLE verification_evidence IS 'Stores detailed evidence used in verification processes';

COMMENT ON
TABLE verification_cache IS 'Caches verification results for improved performance';
