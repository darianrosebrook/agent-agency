...
export class ArbiterRuntime {
  ...
  private async initializeRegistry(): Promise<void> {
    try {
      ...
      const agents = await realRegistry.getAgents();
      for (const agent of agents) {
        await this.performanceTracker.recordAgentRegistration(agent.id, {
          capabilities: agent.capabilities.taskTypes,
          baselineMetrics: {
            latencyMs: 1000, // Default baseline
            accuracy: 0.8,
            costPerTask: 0.1,
            reliability: 0.9,
          },
  ...
  private async executeTask(task: ArbiterTask): Promise<void> {
    ...
    const assignedAgentId = record.metadata?.assignedAgentId as string || "arbiter-runtime";
    const routingDecision = {
      taskId: task.id,
      selectedAgent: assignedAgentId,
      routingStrategy: record.metadata?.routingStrategy as string || "direct-execution",
    ...
    try {
      ...
      if (failedVerification || erroredVerification) {
        throw new Error(
          `Verification failed with verdict ${verification.verdict} (confidence ${(
      ...
      if (executionId) {
        await this.performanceTracker.completeTaskExecution(executionId, {
          ...
          latencyMs: duration,
          tokensUsed: 0, // Would need to be extracted from execution result
          errorMessage: undefined,
          metadata: {
            outputPath: record.outputPath,
            verificationVerdict: verification.verdict,
            verificationConfidence: verification.confidence,
            cawsPassed: record.cawsResult?.passed ?? false,
            artifactsCreated: record.artifacts?.manifest.files.length ?? 0,
          },
        });
      }
    } catch (error) {
  ...
  private async runWithTaskOrchestrator(
    ...
    const envTimeoutMs = Number(
      process.env.ARBITER_ORCHESTRATOR_TIMEOUT_MS
    );
    const timeoutMs =
  ...
  private toRunnerTask(
    ...
    const assignedAgentId =
      fallbackAssignedAgentId && fallbackAssignedAgentId !== "arbiter-runtime"
        ...
        : undefined;
    const artifactRoot =
      this.workerArtifactsRoot ?? path.join(this.options.outputDir, "worker-artifacts");
  ...
  private async enforceCawsPolicies(
    ...
    await this.performanceTracker.recordConstitutionalValidation({
      ...
      validationResult: {
        ...
        violations: validation.remediation?.map((msg) => ({
        ...
        ruleCount: validation.ruleCount ?? 0,
      },
    });
