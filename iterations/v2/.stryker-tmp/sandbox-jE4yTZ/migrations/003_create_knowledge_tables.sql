...
-- Rate limiting
rate_limit_remaining INTEGER, rate_limit_reset_at TIMESTAMPTZ,

-- Error tracking
last_error_message TEXT, last_error_at TIMESTAMPTZ,
...
-- Cache content
content JSONB NOT NULL, metadata JSONB NOT NULL DEFAULT '{}',
...
CREATE INDEX IF NOT EXISTS idx_knowledge_cache_access ON knowledge_cache (
...
INSERT INTO
    search_provider_health (provider_name, available)
VALUES ('google', true),
    ('bing', true),
    ('duckduckgo', true),
    ('arxiv', true),
    ('pubmed', true) ON CONFLICT (provider_name) DO NOTHING;
...
SELECT
    ...
    kq.processed_at,
    CASE
        WHEN kq.processed_at IS NOT NULL THEN EXTRACT(
            EPOCH
            FROM (
                    kq.processed_at - kq.created_at
                )
        ) * 1000
        ELSE NULL
...
SELECT
    ...
    COUNT(
        CASE
            WHEN sr.quality = 'high' THEN 1
        END
    ) as high_quality_count,
    COUNT(
        CASE
            WHEN sr.quality = 'medium' THEN 1
        END
    ) as medium_quality_count,
    COUNT(
        CASE
            WHEN sr.quality = 'low' THEN 1
        END
    ) as low_quality_count,
    COUNT(
        CASE
            WHEN sr.quality = 'unreliable' THEN 1
        END
    ) as unreliable_count,
...
COMMENT ON
TABLE knowledge_queries IS 'Stores knowledge queries with processing metadata and status';

COMMENT ON
TABLE search_results IS 'Individual search results with quality scores and content';

COMMENT ON
TABLE knowledge_responses IS 'Aggregated responses with performance and quality metrics';

COMMENT ON
TABLE search_provider_health IS 'Health monitoring and rate limiting for search providers';

COMMENT ON
TABLE knowledge_cache IS 'Caching layer for queries and results to reduce API calls';
