...
impl ConfigLoader {
    ...
    async fn load_from_env(&self, config: &mut HashMap<String, serde_json::Value>) -> Result<()> {
        ...
        if loaded_count > 0 {
            info!(
                "Loaded {} configuration values from environment variables",
                loaded_count
            );
        }
    ...
    fn get_default_config(&self) -> HashMap<String, serde_json::Value> {
        ...
        defaults.insert(
            "database.idle_timeout_secs".to_string(),
        ...
        defaults.insert(
            "server.max_request_size_mb".to_string(),
            serde_json::Value::Number(10.into()),
        );
        ...
        defaults.insert(
            "agent.task_timeout_secs".to_string(),
            serde_json::Value::Number(300.into()),
        );
        defaults.insert(
            "agent.max_retry_attempts".to_string(),
            serde_json::Value::Number(3.into()),
        );
        defaults.insert(
            "agent.retry_delay_secs".to_string(),
        ...
        defaults.insert(
            "logging.max_file_size_mb".to_string(),
            serde_json::Value::Number(100.into()),
        );
        defaults.insert(
            "logging.max_files".to_string(),
            serde_json::Value::Number(10.into()),
        );
        ...
        defaults.insert(
            "security.max_login_attempts".to_string(),
            serde_json::Value::Number(5.into()),
        );
        ...
        defaults.insert(
            "cache.eviction_threshold_percent".to_string(),
            serde_json::Value::Number(80.into()),
        );

        // Resource defaults
        defaults.insert(
            "resources.cpu_limit_percent".to_string(),
            serde_json::Value::Number(80.into()),
        );
        defaults.insert(
            "resources.memory_limit_percent".to_string(),
            serde_json::Value::Number(80.into()),
        );
        defaults.insert(
            "resources.disk_limit_gb".to_string(),
            serde_json::Value::Number(10.into()),
        );
        defaults.insert(
            "resources.network_limit_mbps".to_string(),
            serde_json::Value::Number(100.into()),
        );
        ...
        defaults.insert(
            "tracing.sampling_rate_percent".to_string(),
            serde_json::Value::Number(10.into()),
        );
        defaults.insert(
            "tracing.export_interval_secs".to_string(),
            serde_json::Value::Number(60.into()),
        );
    ...
    pub async fn start_hot_reload(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            loop {
                ...
                if let Err(e) =
                    Self::check_and_reload(&config_path, &config, &last_modified, &watchers).await
                {
        ...
        info!(
            "Started hot-reloading for configuration file: {}",
            self.config_path
        );
        Ok(())
    }
...
static CONFIG_LOADER: once_cell::sync::OnceCell<Arc<ConfigLoader>> =
...
pub async fn init_config_loader(config_path: &str) -> Result<Arc<ConfigLoader>> {
    ...
    info!(
        "Configuration loader initialized with path: {}",
        config_path
    );
    Ok(loader)
}
