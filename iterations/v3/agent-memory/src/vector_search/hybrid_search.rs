...
impl HybridSearchEngine {
    ...
    pub async fn search_with_rrf(
        ...
        let vector_results = if let Some(embedding) = &query.embedding {
            self.vector_engine
                .vector_search(embedding, top_k * 2, &query.filters)
                .await?
        } else {
            Vec::new()
        };
        ...
        let text_results = if let Some(text) = &query.text {
            self.text_engine
                .text_search(text, top_k * 2, &query.filters)
                .await?
        } else {
            Vec::new()
        };
    ...
    fn reciprocal_rank_fusion(
        ...
        let mut score_map: HashMap<crate::memory_types::MemoryId, (f32, usize, usize)> =
            HashMap::new();
        ...
        let mut results: Vec<(crate::memory_types::MemoryId, f32, usize, usize)> = score_map
            .into_iter()
    ...
    async fn adaptive_weighted_search(
        &self,
        query: &SearchQuery,
        config: &VectorSearchConfig,
    ) -> MemoryResult<Vec<SearchResult>> {
        ...
        let mut result_map: HashMap<crate::memory_types::MemoryId, (SearchResult, f32)> =
            HashMap::new();
            ...
            result_map
                ...
                .and_modify(|(existing_result, total_weight)| {
                    ...
                    existing_result.score =
                        existing_result.score * *total_weight + result.score * weight;
                    *total_weight += weight;
...
impl VectorSearchEngine for HybridSearchEngine {
    async fn vector_search(
        ...
        self.vector_engine
            .vector_search(query_embedding, top_k, filters)
            .await
    }
    ...
    async fn text_search(
        ...
        self.text_engine
            .text_search(query_text, top_k, filters)
            .await
    }
    ...
    async fn index_memory(
        ...
        self.vector_engine
            .index_memory(memory_id, embedding, text_content)
            .await?;
        self.text_engine
            .index_memory(memory_id, embedding, text_content)
            .await?;
    ...
    async fn get_stats(&self) -> MemoryResult<SearchStats> {
        ...
        Ok(SearchStats {
            total_memories_indexed: vector_stats
                .total_memories_indexed
                .max(text_stats.total_memories_indexed),
            average_search_time_ms: (vector_stats.average_search_time_ms
                + text_stats.average_search_time_ms)
                / 2.0,
