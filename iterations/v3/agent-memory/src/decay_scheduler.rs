...
impl MemoryDecayScheduler {
    ...
    pub async fn new(config: DecaySchedulerConfig, db_pool: PgPool) -> MemoryResult<Self> {
        ...
        let decay_engine =
            Arc::new(crate::decay::MemoryDecayEngine::new(&decay_config, db_pool.clone()).await?);
    ...
    pub async fn start(&self) -> MemoryResult<()> {
        ...
        *running = true;
        ...
        info!(
            "Starting memory decay scheduler (interval: {}s)",
...
mod tests {
    ...
    async fn create_test_db_pool() -> Result<PgPool, MemoryError> {
        ...
        let pool = PgPoolOptions::new()
            ...
            .await
            .map_err(|e| {
                MemoryError::Other(format!("Failed to connect to test database: {}", e))
            })?;

        // Test the connection
        sqlx::query("SELECT 1").execute(&pool).await.map_err(|e| {
            MemoryError::Other(format!("Failed to test database connection: {}", e))
        })?;
    ...
    async fn test_scheduler_creation() {
        ...
        match create_test_db_pool().await {
            Ok(db_pool) => {
                ...
                assert!(
                    !status.running,
                    "Scheduler should not be running immediately after creation"
                );
                ...
                let custom_scheduler =
                    MemoryDecayScheduler::new(custom_config.clone(), db_pool.clone())
                        .await
                ...
                assert_eq!(
                    custom_status.interval_seconds,
                    custom_config.decay_interval_seconds
                );
                assert_eq!(
                    custom_status.max_concurrent_cycles,
                    custom_config.max_concurrent_cycles
                );
        ...
        match create_test_db_pool().await {
            Ok(db_pool) => {
                ...
                scheduler
                    .start()
                    .await
                    .expect("Starting already-running scheduler should not error");
                ...
                scheduler
                    .stop()
                    .await
                    .expect("Stopping already-stopped scheduler should not error");
            }
            Err(e) => {
