...
use agent_agency_contracts::errors::{MemoryError as ContractsMemoryError, MemoryResult};
...
use agent_agency_contracts::types::memory::{
    Experience, ExperienceOutcome, MemoryId, TaskPriority as ContractsTaskPriority,
    TemporalContext, TemporalQuery,
};
...
impl ContractsMemorySystem for MemorySystemAdapter {
    async fn store_experience(&self, experience: Experience) -> MemoryResult<MemoryId> {
        ...
        let memory_id = self
            .inner
            .store_experience(agent_experience)
            .await
    ...
    async fn retrieve_temporal_context(
        ...
        for (idx, exp) in experiences.iter().enumerate() {
            contexts.push(TemporalContext {
                ...
                duration_ms: exp.metadata.get("duration_ms").and_then(|v| v.as_u64()),
    ...
    async fn record_outcome(
        ...
        let mut experience = self
            .inner
            .manager()
            .retrieve_memory(agent_memory_id)
            .await
        ...
        self.inner
            .manager()
            .update_memory_metadata(agent_memory_id, metadata)
            .await
            .map_err(|e| ContractsMemoryError::OperationFailed {
    ...
    async fn retrieve_experience(&self, memory_id: MemoryId) -> MemoryResult<Experience> {
        ...
        let experience = Experience {
            ...
            metadata: {
                ...
                meta.insert(
                    "agent_id".to_string(),
                    serde_json::json!(agent_experience.agent_id),
                );
                meta.insert(
                    "task_id".to_string(),
                    serde_json::json!(agent_experience.task_id),
                );
                meta.insert(
                    "input".to_string(),
                    serde_json::json!(agent_experience.input),
                );
                meta.insert(
                    "output".to_string(),
                    serde_json::json!(agent_experience.output),
                );
                meta
            },
        };
        ...
        for agent_exp in experiences {
            results.push(Experience {
                ...
                metadata: {
                    ...
                    meta.insert(
                        "agent_id".to_string(),
                        serde_json::json!(agent_exp.agent_id),
                    );
