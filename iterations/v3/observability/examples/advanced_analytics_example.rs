...
use agent_agency_observability::{
    AnalyticsEngine, AnalyticsDashboard, AgentTelemetryCollector,
    AgentPerformanceSnapshot, CoordinationMetricsSnapshot, BusinessMetricsSnapshot,
    SystemHealthSnapshot, AnalyticsConfig, AnalyticsDashboardConfig,
    TrendAnalysis, AnomalyDetectionResult, PredictiveModelResult,
    OptimizationRecommendation, InsightType, InsightSeverity
};
use anyhow::Result;
    ...
    for day in 0..7 {
        ...
        for hour in 0..24 {
            ...
            for agent_id in &agents {
                ...
                analytics_engine.add_agent_performance_data(
                    agent_id.to_string(),
                    snapshot,
                ).await?;
            }
            ...
            analytics_engine.add_coordination_data(coordination_snapshot).await?;
            ...
            analytics_engine.add_business_data(business_snapshot).await?;
...
async fn demonstrate_trend_analysis(
    analytics_engine: &AnalyticsEngine,
) -> Result<()> {
    ...
    let response_time_data = vec![2000.0, 1950.0, 1900.0, 1850.0, 1800.0, 1750.0, 1700.0, 1650.0, 1600.0, 1550.0];
    let error_rate_data = vec![0.15, 0.13, 0.11, 0.09, 0.07, 0.05, 0.03, 0.01, 0.02, 0.04];

    // Analyze trends
    let success_trend = analytics_engine.analyze_trends("success_rate", &success_rate_data).await?;
    let response_trend = analytics_engine.analyze_trends("response_time", &response_time_data).await?;
    let error_trend = analytics_engine.analyze_trends("error_rate", &error_rate_data).await?;

    info!("Trend Analysis Results:");
    info!("  Success Rate Trend: {:?} (strength: {:.2}, confidence: {:.2})", 
          success_trend.trend_direction, success_trend.trend_strength, success_trend.confidence);
    info!("  Response Time Trend: {:?} (strength: {:.2}, confidence: {:.2})", 
          response_trend.trend_direction, response_trend.trend_strength, response_trend.confidence);
    info!("  Error Rate Trend: {:?} (strength: {:.2}, confidence: {:.2})", 
...
async fn demonstrate_anomaly_detection(
    analytics_engine: &AnalyticsEngine,
) -> Result<()> {
...
async fn demonstrate_predictive_analytics(
    analytics_engine: &AnalyticsEngine,
) -> Result<()> {
    info!("Demonstrating predictive analytics...");

    // Generate predictions for different metrics
    let capacity_prediction = analytics_engine.generate_predictions(
        "throughput",
        agent_agency_observability::PredictionType::CapacityPlanning,
    ).await?;

    let performance_prediction = analytics_engine.generate_predictions(
        "completion_rate",
        agent_agency_observability::PredictionType::PerformanceForecast,
    ).await?;

    let quality_prediction = analytics_engine.generate_predictions(
        "quality_score",
    ...
    info!("    Predicted Value: {:.2}", capacity_prediction.predicted_value);
    info!("    Confidence Interval: ({:.2}, {:.2})", 
          capacity_prediction.confidence_interval.0, capacity_prediction.confidence_interval.1);
    info!("    Model Accuracy: {:.2}", capacity_prediction.model_accuracy);
    ...
    info!("    Predicted Value: {:.2}", performance_prediction.predicted_value);
    info!("    Confidence Interval: ({:.2}, {:.2})", 
          performance_prediction.confidence_interval.0, performance_prediction.confidence_interval.1);
    info!("    Model Accuracy: {:.2}", performance_prediction.model_accuracy);
    info!("    Recommendations:");
    ...
    info!("    Predicted Value: {:.2}", quality_prediction.predicted_value);
    info!("    Confidence Interval: ({:.2}, {:.2})", 
          quality_prediction.confidence_interval.0, quality_prediction.confidence_interval.1);
    info!("    Model Accuracy: {:.2}", quality_prediction.model_accuracy);
    info!("    Recommendations:");
...
async fn demonstrate_optimization_recommendations(
    ...
    let recommendations = analytics_engine.generate_optimization_recommendations().await?;

    info!("Optimization Recommendations:");
    for (i, rec) in recommendations.iter().enumerate() {
        info!("  {}. {} ({:?} priority)", i + 1, rec.description, rec.priority);
        info!("     Type: {:?}", rec.recommendation_type);
        info!("     Expected Improvement: {:.1}%", rec.expected_improvement);
        info!("     Implementation Effort: {:?}", rec.implementation_effort);
...
async fn demonstrate_analytics_dashboard(
    analytics_dashboard: &AnalyticsDashboard,
) -> Result<()> {
    info!("Demonstrating analytics dashboard...");

    // Create a dashboard session
    let session_id = analytics_dashboard.create_session(
    ...
    info!("    Health: {}", dashboard_data.system_overview.system_health);
    info!("    Active Agents: {}", dashboard_data.system_overview.active_agents);
    info!("    Total Tasks: {}", dashboard_data.system_overview.total_tasks);
    info!("    Performance Score: {:.2}", dashboard_data.system_overview.performance_score);
    info!("    Quality Score: {:.2}", dashboard_data.system_overview.quality_score);
    info!("    Efficiency Score: {:.2}", dashboard_data.system_overview.efficiency_score);

    info!("  Trend Analysis Summary:");
    info!("    Total Trends: {}", dashboard_data.trend_analysis.total_trends);
    info!("    Positive Trends: {}", dashboard_data.trend_analysis.positive_trends);
    info!("    Negative Trends: {}", dashboard_data.trend_analysis.negative_trends);
    info!("    Stable Trends: {}", dashboard_data.trend_analysis.stable_trends);
    info!("    Volatile Trends: {}", dashboard_data.trend_analysis.volatile_trends);

    info!("  Anomaly Detection Summary:");
    info!("    Total Anomalies: {}", dashboard_data.anomaly_detection.total_anomalies);
    info!("    Critical Anomalies: {}", dashboard_data.anomaly_detection.critical_anomalies);
    info!("    High Anomalies: {}", dashboard_data.anomaly_detection.high_anomalies);

    info!("  Predictive Insights Summary:");
    info!("    Total Predictions: {}", dashboard_data.predictive_insights.total_predictions);

    info!("  Optimization Recommendations:");
    info!("    Total Recommendations: {}", dashboard_data.optimization_recommendations.len());

    info!("  Performance Insights:");
    info!("    Total Insights: {}", dashboard_data.performance_insights.len());

    // Get real-time updates
    let real_time_update = analytics_dashboard.get_real_time_updates(
        &session_id,
    ...
    if let Some(trend_updates) = real_time_update.trend_updates {
        info!("  Trend Updates: {} new trends, {} updated trends", 
              trend_updates.new_trends.len(), trend_updates.updated_trends.len());
    }

    if let Some(anomaly_updates) = real_time_update.anomaly_updates {
        info!("  Anomaly Updates: {} new anomalies, {} resolved anomalies", 
              anomaly_updates.new_anomalies.len(), anomaly_updates.resolved_anomalies.len());
    }

    if let Some(prediction_updates) = real_time_update.prediction_updates {
        info!("  Prediction Updates: {} new predictions, {} updated predictions", 
              prediction_updates.new_predictions.len(), prediction_updates.updated_predictions.len());
    }

    if let Some(optimization_updates) = real_time_update.optimization_updates {
        info!("  Optimization Updates: {} new recommendations, {} updated recommendations", 
              optimization_updates.new_recommendations.len(), optimization_updates.updated_recommendations.len());
    }
