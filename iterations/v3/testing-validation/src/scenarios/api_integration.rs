...
async fn test_database_connectivity(
    ...
    let result = postgres
        .execute_query("SELECT 1 as test_value", &[])
        .await?;
...
async fn test_task_management_endpoints(
    ...
    postgres
        .execute(
            "DELETE FROM execution_plans WHERE id = $1 OR id = $2",
            &[&plan_id_str, &pause_plan_id_str],
        )
        .await?;
...
async fn test_query_management_endpoints(
    ...
    // Test query deletion
    let delete_result = postgres
        .execute("DELETE FROM saved_queries WHERE id = $1", &[&query_id_str])
        .await?;
