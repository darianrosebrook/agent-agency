...
impl PatternAnalyzer {
    ...
    pub async fn analyze_execution_records(&self, records: &[ExecutionRecord]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        // Group records by characteristics
        let success_records: Vec<&ExecutionRecord> = records.iter().filter(|r| r.success).collect();
        let failure_records: Vec<&ExecutionRecord> = records.iter().filter(|r| !r.success).collect();
    ...
    /// Analyze success patterns
    async fn analyze_success_patterns(&self, records: &[&ExecutionRecord]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for record in records {
            let specialty = record.metadata.get("specialty")
                .and_then(|v| v.as_str())
                .unwrap_or("General");
            specialty_groups.entry(specialty.to_string()).or_default().push(record);
        }
            ...
            if group_records.len() >= self.min_pattern_frequency as usize {
                let pattern = SuccessPattern {
                    ...
                    conditions: HashMap::from([
                        ("specialty".to_string(), serde_json::Value::String(specialty)),
                    ...
                    average_quality: group_records.iter().map(|r| r.quality_score).sum::<f64>() / group_records.len() as f64,
    ...
    /// Analyze failure patterns
    async fn analyze_failure_patterns(&self, records: &[&ExecutionRecord]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if records.len() < self.min_pattern_frequency as usize {
            return Ok(());
        }
        ...
        for record in records {
            let error_type = record.error_message.as_ref()
        ...
        for (error_type, group_records) in error_groups {
            if group_records.len() >= self.min_pattern_frequency as usize {
                let pattern = FailurePattern {
                    ...
                    common_errors: group_records.iter()
    ...
    /// Identify optimal configurations
    async fn identify_optimal_configs(&self, records: &[ExecutionRecord]) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        // Group records by configuration characteristics
        let mut config_groups: HashMap<String, Vec<&ExecutionRecord>> = HashMap::new();
        
        for record in records {
            let config_key = format!("worker_{}_quality_{:.2}", 
                record.worker_id.0, 
        ...
        for (config_key, group_records) in config_groups {
            if group_records.len() >= self.min_pattern_frequency as usize {
                let success_rate = group_records.iter().filter(|r| r.success).count() as f64 / group_records.len() as f64;
                let avg_execution_time = group_records.iter().map(|r| r.execution_time_ms).sum::<u64>() as f64 / group_records.len() as f64;
                let avg_quality = group_records.iter().map(|r| r.quality_score).sum::<f64>() / group_records.len() as f64;
    ...
    pub async fn match_task_pattern(&self, task_characteristics: &HashMap<String, serde_json::Value>) -> Result<Vec<PatternMatch>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Match against success patterns
        {
            ...
            for pattern in patterns.iter() {
                let match_score = calculate_pattern_match_score(task_characteristics, &pattern.conditions);
