...
/// Stub implementation of fairness monitor
pub struct StubFairnessMonitor;

#[async_trait::async_trait]
impl FairnessMonitor for StubFairnessMonitor {
    async fn get_fairness_metrics(&self) -> Result<FairnessMetrics, Box<dyn std::error::Error + Send + Sync>> {
        Ok(FairnessMetrics {
    ...
    async fn record_task_assignment(&self, _worker_id: WorkerId, _task_id: crate::parallel_types::TaskId) -> Result<()> {
