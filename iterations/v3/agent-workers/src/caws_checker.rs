...
impl CawsChecker {
    ...
    pub async fn check_compliance(&self, task: &str) -> Result<CawsCheckResult, WorkerError> {
                ...
                match serde_yaml::from_str::<serde_json::Value>(task) {
                    ...
                    Err(e) => {
                        ...
                        return Ok(CawsCheckResult {
                            ...
                            recommendations: vec![
                                "Task specification must be valid JSON or YAML".to_string()
                            ],
                        });
                    }
                }
        ...
        let diff_stats = DiffStats {
            files_changed: change_budget
                .get("max_files")
                .and_then(|v| v.as_u64())
                .map(|n| n as u32)
                .unwrap_or(0),
            lines_added: change_budget
                .get("max_loc")
        ...
        let validation_context = ValidationContext {
            ...
            diff_stats,
            test_results: task_spec
                .get("test_results")
                .cloned()
                .and_then(|v| serde_json::from_value(v).ok()),
            security_scan: task_spec
                .get("security_scan")
                .cloned()
                .and_then(|v| serde_json::from_value(v).ok()),
        };
        ...
        if validation_result.compliance_score < 0.8 {
            recommendations.push(
                "Consider reviewing and addressing violations to improve compliance score"
                    .to_string(),
            );
        }
        if !violations.is_empty() {
            recommendations.push(format!(
                "Address {} violation(s) to achieve full compliance",
                violations.len()
            ));
        }
