...
impl PerformanceMonitor {
    ...
    pub async fn record_inference(
        ...
        let latencies: Vec<f64> = model_history
            .iter()
        ...
        let error_count = model_history.iter().filter(|m| m.error_rate > 0.0).count() as f64;
        ...
        let metrics = ModelMetrics {
            ...
            error_rate: if success {
                error_rate
            } else {
                error_rate + 1.0 / recent_count
            },
            cpu_usage,
        ...
        self.check_performance_regression(model_id, model_history)
            .await;
    ...
    pub async fn get_metrics_history(
        &self,
        model_id: &str,
    ) -> Result<Vec<ModelMetrics>, ModelManagementError> {
    ...
    async fn check_performance_regression(&self, model_id: &str, history: &[ModelMetrics]) {
        ...
        let recent_p95_avg =
            recent.iter().map(|m| m.p95_latency_ms).sum::<f64>() / recent.len() as f64;
        let older_p95_avg =
            older.iter().map(|m| m.p95_latency_ms).sum::<f64>() / older.len() as f64;
        ...
        let recent_error_rate =
            recent.iter().map(|m| m.error_rate).sum::<f64>() / recent.len() as f64;
