...
impl QueryPerformanceMonitor {
    ...
    pub async fn record_query_execution(&self, query_text: &str, execution_time_ms: u64) {
        ...
        metric.average_execution_time_ms =
            metric.total_execution_time_ms as f64 / metric.execution_count as f64;
        ...
        if is_slow {
            ...
            metric.slow_execution_rate =
                metric.slow_execution_count as f64 / metric.execution_count as f64;
    ...
    pub async fn record_query_execution_timed(&self, query_text: &str, start_time: Instant) {
        let duration = start_time.elapsed();
        let execution_time_ms = duration.as_millis() as u64;
        self.record_query_execution(query_text, execution_time_ms)
            .await;
    }
    ...
    pub async fn get_top_slow_queries(&self, limit: usize) -> Vec<QueryMetrics> {
        ...
        metrics.sort_by(|a, b| {
            b.average_execution_time_ms
                .partial_cmp(&a.average_execution_time_ms)
    ...
    pub async fn get_queries_with_high_slow_rate(
        &self,
        limit: usize,
        min_rate: f64,
    ) -> Vec<QueryMetrics> {
        ...
        metrics.sort_by(|a, b| {
            b.slow_execution_rate
                .partial_cmp(&a.slow_execution_rate)
                .unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    pub async fn get_performance_summary(&self) -> PerformanceSummary {
        ...
        let avg_execution_time: f64 = if total_queries > 0 {
            let total_time: u64 = metrics_map
                .values()
...
macro_rules! time_query {
    ($monitor:expr, $query_text:expr, $query:block) => {{
        ...
        let result = $query;
        $monitor
            .record_query_execution_timed($query_text, start_time)
            .await;
        result
    }};
}
...
mod tests {
    ...
    async fn test_record_query_execution() {
        ...
        monitor
            .record_query_execution("SELECT * FROM users", 50)
            .await;
        monitor
            .record_query_execution("SELECT * FROM users", 75)
            .await;
        monitor
            .record_query_execution("SELECT * FROM users", 100)
            .await;
    ...
    async fn test_slow_query_detection() {
        ...
        monitor
            ...
            .await;
        monitor
            .record_query_execution("SELECT * FROM users", 150)
            .await; // Slow
        monitor
            .record_query_execution("SELECT * FROM users", 200)
            .await; // Slow
    ...
    async fn test_performance_summary() {
        ...
        monitor
            ...
            .await;
        monitor
            .record_query_execution("SELECT * FROM tasks", 1500)
            .await; // Slow
