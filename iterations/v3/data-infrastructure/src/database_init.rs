...
pub async fn run_migrations(pool: &PgPool) -> Result<()> {
    ...
    let possible_dirs = [
        ...
        Path::new("../data-infrastructure/migrations"),
        Path::new("./iterations/v3/data-infrastructure/migrations"),
        Path::new("iterations/v3/data-infrastructure/migrations"),
    ];

    let migrations_dir = possible_dirs.iter().find(|p| p.exists()).ok_or_else(|| {
        anyhow::anyhow!("Migrations directory not found. Tried: {:?}", possible_dirs)
    })?;
    ...
    let executed: Vec<String> =
        sqlx::query_scalar("SELECT version FROM migration_log ORDER BY version")
            .fetch_all(pool)
            .await
            .unwrap_or_default();
    ...
    for (filename, path) in migration_files {
        ...
        for (idx, statement) in statements.iter().enumerate() {
            ...
            match sqlx::query(trimmed).execute(&mut *tx).await {
                Ok(_) => {
                    info!(
                        "Migration {} statement {} executed successfully",
                        version,
                        idx + 1
                    );
                }
