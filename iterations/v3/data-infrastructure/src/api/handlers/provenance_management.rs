...
pub async fn list_provenance_records(
    ...
    match state.api.db_client.list_provenance_records().await {
        Ok(records) => {
            let record_list: Vec<serde_json::Value> = records.into_iter().map(|record| {
                serde_json::json!({
                    "id": record.id,
...
pub async fn link_provenance_to_commit(
    ...
    let provenance_id = link_data.get("provenance_id")
        .and_then(|v| v.as_str())
        .ok_or(StatusCode::BAD_REQUEST)?;
    
    let commit_hash = link_data.get("commit_hash")
        .and_then(|v| v.as_str())
        .ok_or(StatusCode::BAD_REQUEST)?;
    
    let provenance_uuid = uuid::Uuid::parse_str(provenance_id)
    ...
    match state.api.db_client.link_provenance_to_commit(&provenance_uuid, commit_hash).await {
        ...
        Err(e) => {
            error!("Failed to link provenance record {} to commit {}: {}", provenance_id, commit_hash, e);
...
pub async fn verify_provenance_trailer(
    ...
    match state.api.db_client.verify_provenance_trailer(&commit_hash).await {
        ...
        Err(e) => {
            error!("Failed to verify provenance trailer for commit {}: {}", commit_hash, e);
            Err(StatusCode::INTERNAL_SERVER_ERROR)
        }
    }
}
...
pub async fn get_provenance_by_commit(
    ...
    match state.api.db_client.get_provenance_by_commit(&commit_hash).await {
        Ok(records) => {
            let record_list: Vec<serde_json::Value> = records.into_iter().map(|record| {
                serde_json::json!({
                    "id": record.id,
        ...
        Err(e) => {
            error!("Failed to get provenance records for commit {}: {}", commit_hash, e);
            Err(StatusCode::NOT_FOUND)
        }
    }
}
