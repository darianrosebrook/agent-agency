...
function cmdScaffold({ projectDir, debug }) {
  ...
  try {
    ...
    // Update manifest
    manifest.scaffolded = [
      ...new Set([...(manifest.scaffolded || []), ...scaffoldedFiles]),
    ];
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
    logDebug(
      debug,
      "Updated scaffold manifest with",
      scaffoldedFiles.length,
      "files"
    );

    console.log(
      "✅ CAWS scaffold complete -",
      scaffoldedFiles.length,
      "files scaffolded"
    );
  } catch (err) {
...
function cmdUnsaffold({ projectDir, debug }) {
  ...
  try {
    ...
    if (removedCount === scaffoldedFiles.length) {
      ...
      fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));
      console.log(
        "✅ CAWS unsaffold complete -",
        removedCount,
        "files removed,",
        manifest.scaffolded.length,
        "files remain"
      );
    }
  } catch (err) {
...
function main() {
  ...
  const projectDir =
    projectDirIdx !== -1 && rest[projectDirIdx + 1]
