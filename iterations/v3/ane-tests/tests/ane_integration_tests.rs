...
use ane_tests::*;
use ane_tests::errors::{ANEError, IntoANEError};
use ane_tests::resource_pool::{Pool, PoolBuilder};
use ane_tests::compat;
use ane_tests::models::coreml_model;
use ane_tests::infer::execute;
use ane_tests::metrics::ewma::{Ewma, PerformanceTracker};
use ane_tests::manager;
use ane_tests::compat::coreml;
use ane_tests::compat::iokit;
use std::path::Path;
...
mod tests {
    ...
    mod compat_tests {
        ...
        fn test_coreml_availability() {
            ...
            assert!(!compat.coreml::detect_coreml_capabilities().supported_precisions.is_empty());
    ...
    mod model_tests {
        ...
        fn test_compilation_options() {
            let opts = coreml_model::CompilationOptions::default();
        ...
        fn test_model_schema_creation() {
            let inputs = vec![
                coreml_model::IOTensorSpec {
                    name: "input".to_string(),
                    shape: vec![1, 3, 224, 224],
                    dtype: coreml_model::DType::F32,
                    optional: false,
                }
            ];
            ...
            let schema = coreml_model::ModelSchema { inputs, outputs };
        ...
        fn test_memory_estimation() {
            let schema = coreml_model::ModelSchema {
                inputs: vec![coreml_model::IOTensorSpec {
                    name: "input".to_string(),
                    shape: vec![1, 3, 224, 224],
                    dtype: coreml_model::DType::F32,
                    optional: false,
                }],
                outputs: vec![coreml_model::IOTensorSpec {
                    name: "output".to_string(),
                    shape: vec![1, 1000],
                    dtype: coreml_model::DType::F32,
                    optional: false,
            ...
            let metadata = coreml_model::ModelMetadata {
            ...
            let model = coreml_model::LoadedCoreMLModel {
    ...
    mod inference_tests {
        ...
        fn test_inference_options() {
            let opts = execute::InferenceOptions {
                timeout_ms: 5000,
                batch_size: Some(4),
        ...
        async fn test_inference_timeout() {
            ...
            let model = coreml_model::LoadedCoreMLModel {
                model_id: "test_model".to_string(),
                compiled_path: Path::new("test.mlmodelc").to_path_buf(),
                metadata: coreml_model::ModelMetadata {
                    path: Path::new("test.mlmodel").to_path_buf(),
                    size_bytes: 1024,
            ...
            let result = execute::execute_inference(&model, &input, &opts).await;
    ...
    mod metrics_tests {
        ...
        fn test_ewma_calculation() {
            ...
            prev_value = Ewma::Ewma::update(prev_value, 120.0, alpha);
            assert!(prev_value > 100.0 && prev_value < 120.0);

            // Test convergence
            let mut value = 0.0;
            for _ in 0..100 {
                value = Ewma::Ewma::update(value, 10.0, 0.1);
            }
        ...
        fn test_performance_tracker() {
            let mut tracker = Ewma::PerformanceTracker::new();
