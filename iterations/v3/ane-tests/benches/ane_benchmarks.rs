...
fn bench_resource_pool_admission(c: &mut Criterion) {
    let pool = resource_pool::PoolBuilder::new()
        .max_concurrent(10)
...
fn bench_resource_pool_concurrent_admission(c: &mut Criterion) {
    ...
    c.bench_function("resource_pool_concurrent_admission", |b| {
        b.iter(|| {
            let test_pool = ane_tests::resource_pool::PoolBuilder::new()
                .max_concurrent(4)
                .memory_total_mb(4096)
                .build()
                .expect("Pool creation failed");
            ...
            tokio::runtime::Runtime::new().unwrap().block_on(async {
                let admission = black_box(test_pool.admit(256).await.unwrap());
                drop(admission);
            });
        });
    });
...
fn bench_ewma_calculation(c: &mut Criterion) {
    ...
    c.bench_function("ewma_update", |b| {
        b.iter(|| {
            prev_value = metrics::ewma::Ewma::update(black_box(prev_value), black_box(sample), black_box(alpha));
        });
    });
}

fn bench_performance_tracker(c: &mut Criterion) {
    let mut tracker = metrics::ewma::PerformanceTracker::new();
...
fn bench_memory_estimation(c: &mut Criterion) {
    ...
    let schema = models::coreml_model::ModelSchema {
        inputs: vec![
            models::coreml_model::IOTensorSpec {
                name: "input".to_string(),
                shape: vec![1, 3, 224, 224],
                dtype: models::coreml_model::DType::F32,
                optional: false,
            }
        ],
        outputs: vec![
    ...
    let metadata = models::coreml_model::ModelMetadata {
    ...
    let model = ane_tests::models::coreml_model::LoadedCoreMLModel {
    ...
    c.bench_function("memory_estimation", |b| {
        b.iter(|| {
            let _memory_mb = ane_tests::models::coreml_model::estimate_memory_usage(
                black_box(&model)
            );
        });
    });
...
fn bench_inference_options_creation(c: &mut Criterion) {
    c.bench_function("inference_options_creation", |b| {
        b.iter(|| {
            let _opts = infer::execute::InferenceOptions {
...
fn bench_error_creation(c: &mut Criterion) {
    c.bench_function("ane_error_creation", |b| {
        b.iter(|| {
            let _err = errors::ANEError::ModelNotFound(
    ...
    c.bench_function("ane_error_display", |b| {
        let err = errors::ANEError::Timeout(5000);
        b.iter(|| {
    ...
    let schema = models::coreml_model::ModelSchema {
        inputs: vec![
            ...
            models::coreml_model::IOTensorSpec {
                ...
                shape: vec![1, 128],
                dtype: models::coreml_model::DType::I32,
                optional: true,
            }
        ],
        outputs: vec![
