...
use crate::compat::{coreml, iokit};
use crate::resource_pool::{Pool, PoolBuilder, PoolStats};
use crate::models::coreml_model::{LoadedCoreMLModel, CompilationOptions, estimate_memory_usage};
use crate::infer::execute::{execute_inference, InferenceOptions as ExecuteOptions, InferenceResult};
use crate::metrics::ewma::{Ewma, PerformanceTracker, PerformanceSummary};
...
pub struct ANEPerformanceMetrics {
    ...
    /// Average throughput in inferences per second
    pub average_throughput_ips: f64,
    /// Memory usage in MB
...
impl ANEManager {
    ...
    pub async fn detect_capabilities() -> ANECapabilities {
        ...
        let coreml_caps = coreml::detect_coreml_capabilities();
    ...
    pub async fn load_model(&self, model_path: &str) -> Result<String> {
        ...
        let loaded_model = crate::models::coreml_model::load_model(model_path, &compilation_options)?;
        
        // Validate ANE compatibility
        crate::models::coreml_model::validate_ane_compatibility(&loaded_model)?;
    ...
    pub async fn execute_inference(&self, model_id: &str, input: &[f32]) -> Result<Vec<f32>> {
        ...
        let loaded_model = LoadedCoreMLModel {
            ...
            metadata: crate::models::coreml_model::ModelMetadata {
            ...
            schema: crate::models::coreml_model::ModelSchema {
                inputs: vec![crate::models::coreml_model::IOTensorSpec {
                    name: "input".to_string(),
                    shape: model.input_shape.clone(),
                    dtype: crate::models::coreml_model::DType::F32,
                    optional: false,
                }],
                outputs: vec![crate::models::coreml_model::IOTensorSpec {
    ...
    async fn update_performance_metrics(&self, model_id: &str, result: &InferenceResult) {
        // Update model-specific metrics
        {
            ...
            if let Some(model_metrics) = metrics.get_mut(model_id) {
                ...
                if result.memory_usage_mb > model_metrics.memory_usage_mb {
                    model_metrics.memory_usage_mb = result.memory_usage_mb;
                }
