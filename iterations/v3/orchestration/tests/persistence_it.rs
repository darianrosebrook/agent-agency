...
async fn postgres_persistence_smoke() {
    ...
    use council::contracts::{FinalVerdict, FinalDecision};

    let db = Db::connect(&std::env::var("DATABASE_URL").unwrap(), 1).await.unwrap();
    let writer = PostgresVerdictWriter::new(db.pool.clone());
    let verdict = FinalVerdict { decision: FinalDecision::Reject, votes: vec![], dissent: "it".into(), remediation: vec!["r".into()], constitutional_refs: vec!["CAWS:Scope".into()] };
