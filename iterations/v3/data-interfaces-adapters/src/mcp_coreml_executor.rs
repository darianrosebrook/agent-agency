...
impl RealCoreMLIngestionExecutor {
    pub fn new(
        ...
        _yolo_model_path: Option<PathBuf>,
    ) -> Self {
        let circuit_breaker_config = EnrichmentCircuitBreakerConfig::default();
        
        let mut asr_enricher = AsrEnricher::new(circuit_breaker_config.clone());
        if let Some(ref path) = whisper_model_path {
            asr_enricher = asr_enricher.with_whisper_model_path(path.clone());
        }
...
impl CoreMLIngestionExecutor for RealCoreMLIngestionExecutor {
    async fn transcribe_audio(
        ...
        let result = self.asr_enricher.enrich_audio(&audio_data, content_type)
            .await
    ...
    async fn detect_objects(
        ...
        let result = self.visual_captioning_enricher.enrich_visual(&image_data, content_type)
            .await
            .map_err(|e| format!("Object detection failed: {}", e))?;
        
        let objects = result.extracted_metadata.get("objects")
    ...
    async fn extract_text_from_image(
        ...
        let result = self.vision_enricher.enrich_image(&image_data, content_type)
            .await
    ...
    async fn process_video(
        ...
        let result = ingestor.ingest(data_input)
            .await
...
pub fn create_coreml_executor(
    ...
    Arc::new(RealCoreMLIngestionExecutor::new(whisper_model_path, yolo_model_path))
}
