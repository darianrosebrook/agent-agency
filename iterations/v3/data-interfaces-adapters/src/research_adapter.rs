...
use data_interfaces::service_contracts::{
    ResearchService, ServiceError,
};
use agent_agency_contracts::{
    TaskRequest, TaskResponse, WorkingSpec,
    types::validation::ValidationIssue,
};
use std::sync::Arc;
...
use agent_research::planning_agent::refinement_engine::{RefinementEngine, DefaultRefinementEngine};
...
impl ResearchServiceAdapter {
    ...
    pub fn with_defaults() -> Self {
        ...
        Self::new(config, caws_validator, validation_pipeline, refinement_engine)
    }
}
...
impl ResearchService for ResearchServiceAdapter {
    async fn execute_task(&self, request: TaskRequest) -> Result<TaskResponse, ServiceError> {
        ...
        // Call planning agent
        let planning_response = self.planning_agent.plan_task(planning_request)
            .await
            .map_err(|e| match e {
        ...
        Ok(TaskResponse {
            ...
            task_id,
            status: if planning_response.validation_results.overall_status == ValidationStatus::Passed {
            ...
            progress: None,
            error: if planning_response.validation_results.overall_status == ValidationStatus::Passed {
                None
            } else {
                Some(agent_agency_contracts::TaskError {
                    code: "VALIDATION_FAILED".to_string(),
                    message: format!("Validation issues: {:?}", planning_response.validation_results.issues),
                    details: None,
