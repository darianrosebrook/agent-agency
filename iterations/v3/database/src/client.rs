...
impl DatabaseMetrics {
    ...
    pub fn record_connection_acquisition(&self, duration: StdDuration) {
        ...
        // Update simple running average (sufficient for basic monitoring)
        // For advanced statistical analysis, consider implementing Welford's algorithm
        // in a dedicated metrics collection system
        let total = self.total_queries.load(Ordering::Relaxed) + 1;
        self.total_queries.store(total, Ordering::Relaxed);
...
mod tests {
    ...
    /// Provides in-memory simulation of database operations
    ...
    impl DatabaseClient for MockDatabaseClient {
        ...
        fn metrics(&self) -> Arc<DatabaseMetrics> {
            Arc::new(DatabaseMetrics::default())
        }

        fn config(&self) -> &DatabaseConfig {
            // Return a default config for testing
            static DEFAULT_CONFIG: once_cell::sync::Lazy<DatabaseConfig> = once_cell::sync::Lazy::new(|| DatabaseConfig::default());
            &DEFAULT_CONFIG
        }
    }
