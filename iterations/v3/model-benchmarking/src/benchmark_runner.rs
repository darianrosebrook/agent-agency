...
impl BenchmarkRunner {
    ...
    pub async fn run_micro_benchmark(&self, model: &ModelSpecification) -> Result<BenchmarkResult> {
        ...
        info!("Micro benchmark completed in {:?} - Score: {:.2}", total_time, score);
    ...
    pub async fn run_compliance_benchmark(&self, model: &ModelSpecification) -> Result<BenchmarkResult> {
        ...
        info!("Compliance benchmark completed in {:?} - Score: {:.2}", total_time, score);
    ...
    async fn execute_compliance_test(&self, _model: &ModelSpecification) -> Result<ComplianceTestResult> {
    ...
    fn calculate_overall_score(&self, metrics: &BenchmarkMetrics) -> f64 {
        ...
        let scores = [metrics.accuracy, metrics.speed / 100.0, metrics.efficiency, metrics.quality, metrics.compliance];
