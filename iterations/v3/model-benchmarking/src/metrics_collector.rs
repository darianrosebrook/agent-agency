...
impl MetricsCollector {
    ...
    pub async fn get_model_benchmark_results(&self, model_id: Uuid) -> Result<Vec<BenchmarkResult>> {
        let results = self.benchmark_results.read().await;
        Ok(results.get(&model_id).cloned().unwrap_or_default())
    }

    /// Get performance history for a model
    pub async fn get_performance_history(&self, model_id: Uuid, limit: Option<usize>) -> Result<Vec<PerformanceSnapshot>> {
    ...
    pub async fn get_model_summary(&self, model_id: Uuid) -> Result<Option<ModelPerformanceSummary>> {
    ...
    pub async fn calculate_performance_trend(&self, model_id: Uuid, window_size: usize) -> Result<PerformanceTrend> {
        let history = self.get_performance_history(model_id, Some(window_size)).await?;
    ...
    async fn update_model_summary(&self, result: &BenchmarkResult) -> Result<()> {
        ...
        let summary = summaries.entry(result.model_id).or_insert_with(|| ModelPerformanceSummary {
            model_id: result.model_id,
        ...
        if let Some(results) = model_results.get(&result.model_id) {
            let total_score: f64 = results.iter().map(|r| r.score).sum();
            summary.average_score = total_score / results.len() as f64;
        }
        
        // Calculate performance trend
        summary.performance_trend = self.calculate_performance_trend(result.model_id, 10).await
    ...
    fn calculate_linear_trend(&self, scores: &[f64]) -> f64 {
        ...
        let numerator: f64 = scores.iter().enumerate()
            .map(|(i, score)| (i as f64 - x_mean) * (score - y_mean))
            .sum();
        
        let denominator: f64 = scores.iter().enumerate()
    ...
    pub async fn generate_performance_report(&self) -> Result<PerformanceReport> {
        ...
        if !summaries.is_empty() {
            report.average_performance = summaries.iter().map(|s| s.average_score).sum::<f64>() / summaries.len() as f64;
            ...
            for summary in &summaries {
                ...
                *report.performance_distribution.entry(category.to_string()).or_insert(0) += 1;
            }
        }
    ...
    async fn generate_recommendations(&self, report: &mut PerformanceReport, summaries: &[ModelPerformanceSummary]) {
        for summary in summaries {
            match summary.performance_trend {
                PerformanceTrend::Declining => {
                    report.recommendations.push(Recommendation {
                        ...
                        description: format!("Model {} shows declining performance trend", summary.model_name),
                ...
                PerformanceTrend::Stable if summary.average_score < 0.5 => {
                    report.recommendations.push(Recommendation {
                        ...
                        description: format!("Model {} has consistently low performance", summary.model_name),
