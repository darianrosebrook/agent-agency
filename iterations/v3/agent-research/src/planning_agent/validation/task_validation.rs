...
pub fn validate_task_request(
    ...
    if task_request.description.len() < 10 {
        return Err(PlanningError::InvalidTaskRequest(
            "Task description must be at least 10 characters".to_string(),
        ));
    }

    if task_request.description.len() > 10000 {
        return Err(PlanningError::InvalidTaskRequest(
            "Task description cannot exceed 10,000 characters".to_string(),
        ));
    }
...
pub fn assess_risk(
    ...
    if description.to_lowercase().contains("production")
        && description.to_lowercase().contains("emergency")
    {
