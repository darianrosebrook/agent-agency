...
impl AuthorityValidator {
    ...
    pub async fn validate_authority(
        ...
        let credibility_assessment =
            self.generate_credibility_assessment(claim, &source_scores, overall_score);
    ...
    fn assess_source_credibility(&self, source: &str) -> f64 {
        ...
        let https_bonus: f64 = if source_lower.starts_with("https://") {
            0.1
        } else {
            0.0
        };
    ...
    fn generate_credibility_assessment(
        &self,
        claim: &str,
        ...
        let high_credibility_sources = source_scores
            .values()
    ...
    pub fn add_authoritative_domain(&mut self, domain: String, score: f64) {
        self.authoritative_domains
            .insert(domain, score.min(1.0).max(0.0));
    }
