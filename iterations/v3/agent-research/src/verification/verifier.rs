...
use crate::verification::types::{
    CheckResult, CoreferenceResolution as VerificationCoreferenceResolution,
    VerificationResult as ClaimVerificationResult, *,
};
use crate::verification::verification_types::{
    FunctionDefinition, TestConsistency as VerificationTestConsistency,
    TestCoverage as VerificationTestCoverage, TestOutput, TestQuality as VerificationTestQuality,
...
impl MultiModalVerificationEngine {
        ...
        for claim in claims {
            ...
            let verified_claim = VerifiedClaim {
                ...
                evidence: verification_result
                    ...
                    .map(|content| crate::extraction_types::Evidence {
                        id: uuid::Uuid::new_v4(),
                        claim_id: claim.id,
    ...
    pub async fn verify_single_claim(
        ...
        let score = 0.30 * xrefs.confidence
        ...
        Ok(ClaimVerificationResult {
            ...
            verified: score > 0.7,
            confidence: score,
            checks: all_evidence_strings
                .into_iter()
                .map(|evidence_content| CheckResult {
                    check_type: crate::verification::types::CheckType::Other(
                        "evidence".to_string(),
                    ),
                    passed: true,
                    confidence: 0.8,
    ...
    async fn cross_reference_validate(&self, claim: &AtomicClaim) -> Result<CheckResult> {
        // discover docs/specs
        let spec_score = self
            .analyze_specification_coverage(claim, &["docs", "specs"])
            .await?;
        ...
        for f in docs {
            ...
            let m = self
                .keyword_matcher
                .search_keywords_in_content(&c, &kws)
                .await?;
            let (_, rel) = self
                .keyword_matcher
                .analyze_keyword_relevance(&c, &m)
                .await?;
            if rel > 0.0 {
                hits += 1;
            }
        ...
        let best = hist
            .iter()
            .filter_map(|h| {
                self.calculate_claim_similarity(claim, h)
                    .now_or_never()
                    .and_then(|r| r.ok())
            })
            .fold(0.0, f64::max);
        ...
        Ok(CheckResult {
            ...
            evidence: vec![format!(
                "spec:{spec_score:.2} docs:{doc_score:.2} hist:{best:.2}"
            )],
            timestamp: chrono::Utc::now(),
        })
    }
    ...
    async fn validate_context_dependencies(&self, claim: &AtomicClaim) -> Result<CheckResult> {
        ...
        let score = if reqs.is_empty() {
            1.0
        } else {
            available as f64 / reqs.len() as f64
        };
    ...
    fn extract_search_keywords(&self, text: &str) -> Vec<String> {
        const STOP: &[&str] = &[
            "the", "a", "an", "and", "or", "if", "then", "with", "for", "of", "to", "in", "on",
            "by", "at", "is", "are", "be", "this", "that",
        ];
        ...
        match term.to_lowercase().as_str() {
            ...
            "database" | "db" => {
                claims.push(HistoricalClaim {
                    ...
                    source_references: Some(vec![
                        "PostgreSQL Docs".to_string(),
                        "Performance Studies".to_string(),
                    ]),
                    cross_references: None,
                    validation_metadata: None,
    ...
    async fn calculate_claim_similarity(
        ...
        // Analyze both claims
        let claim_analysis = semantic_analyzer
            .analyze_semantics(&claim.claim_text)
            .await?;
        let historical_analysis = semantic_analyzer
            .analyze_semantics(&historical.claim_text)
            .await?;
        ...
        let intent_similarity =
            self.calculate_intent_similarity(&claim_analysis.intent, &historical_analysis.intent);
        similarity_score += intent_similarity * 0.4;

        // 2. Keyword overlap (30% weight)
        let keyword_similarity = self
            .calculate_keyword_similarity(&claim_analysis.synonyms, &historical_analysis.synonyms);
        similarity_score += keyword_similarity * 0.3;

        // 3. Semantic score correlation (20% weight)
        let semantic_correlation =
            1.0 - (claim_analysis.semantic_score - historical_analysis.semantic_score).abs();
        similarity_score += semantic_correlation * 0.2;

        // 4. Content length similarity (10% weight)
        let length_ratio =
            (claim.claim_text.len() as f64 / historical.claim_text.len() as f64).min(1.0);
    ...
    fn assess_available_context(&self, claim: &AtomicClaim, reqs: &[String]) -> usize {
        ...
        if claim.claim_text.contains("API")
            || claim.claim_text.contains("database")
            || claim.claim_text.contains("test")
        {
            context_score += 1;
        }

        // Check if claim has performance criteria
        if claim.claim_text.contains("ms")
            || claim.claim_text.contains("seconds")
            || claim.claim_text.contains("coverage")
        {
            context_score += 1;
        }
    ...
    fn validate_scope_boundaries(&self, claim: &AtomicClaim) -> f64 {
        ...
        if claim.claim_text.contains("within")
            || claim.claim_text.contains("scope")
            || claim.claim_text.contains("boundary")
        {
            scope_score += 0.2;
        }

        // Check if claim has measurable outcomes
        if claim.claim_text.contains("achieve")
            || claim.claim_text.contains("deliver")
            || claim.claim_text.contains("complete")
        {
            scope_score += 0.2;
        }

        // Check if claim has clear success criteria
        if claim.claim_text.contains("success")
            || claim.claim_text.contains("pass")
            || claim.claim_text.contains("meet")
        {
            scope_score += 0.2;
        }
    ...
    pub async fn check_test_consistency(
        &self,
        code_output: &CodeOutput,
        test_output: &TestOutput,
    ) -> Result<VerificationTestConsistency> {
        ...
        let public_functions: Vec<_> = code_structure
            .functions
            .iter()
    ...
    fn check_test_coverage(
        &self,
        public_functions: &[&FunctionDefinition],
        test_output: &TestOutput,
    ) -> Result<VerificationTestCoverage> {
        let mut issues = Vec::new();
    ...
    fn check_test_relevance(
        &self,
        code_output: &CodeOutput,
        test_output: &TestOutput,
    ) -> Result<VerificationTestRelevance> {
        let mut issues = Vec::new();
        let mut score: f32 = 1.0;
        ...
        if total_tests > 0 {
            ...
            if assertions_per_test < 1.0 {
                issues.push(format!(
                    "Low assertion density: {:.1} assertions per test",
                    assertions_per_test
                ));
                score -= 0.1;
            }
    ...
    fn check_test_quality(&self, test_output: &TestOutput) -> Result<VerificationTestQuality> {
        ...
        if !has_setup_teardown && total_lines > 50 {
            issues.push(
                "Large test file without setup/teardown - potential state sharing".to_string(),
            );
            score -= 0.1;
        }
    ...
    pub async fn process(
        ...
        let final_confidence = if claims.is_empty() {
            0.0
        } else {
            overall_confidence / claims.len() as f64
        };
