...
impl Query {
    ...
    pub fn apply<'a>(&self, events: &'a [EventEnvelope]) -> Vec<&'a EventEnvelope> {
        let mut results: Vec<&EventEnvelope> = events
            .iter()
            .filter(|e| {
                // Filter by plan_id
                ...
                if let Some(ref kinds) = self.kinds {
                    let event_kind_name = match &e.kind {
                        ...
                        crate::evaluation::trace::EventKind::WorkerAssigned { .. } => {
                            "WorkerAssigned"
                        }
                        crate::evaluation::trace::EventKind::WorkerReleased { .. } => {
                            "WorkerReleased"
                        }
                        ...
                        crate::evaluation::trace::EventKind::Custom { event_type, .. } => {
                            event_type.as_str()
                        }
                    };
