    ...
    fn generate_security_changes(
        ...
        if security_score < 0.6 {
            changes.push(RequiredChange {
                ...
                rationale: "Input validation is critical for preventing injection attacks"
                    .to_string(),
            });
        }
        ...
        if !spec_description.to_lowercase().contains("encryption") {
            changes.push(RequiredChange {
                ...
                impact: ChangeImpact::Major,
                rationale: "Data encryption protects sensitive information at rest and in transit"
                    .to_string(),
            });
        }
    ...
    fn generate_critical_issues(
        ...
        if security_score < 0.4 {
            issues.push(CriticalIssue {
                ...
                evidence: vec![
                    "Low security score indicates significant vulnerabilities".to_string()
                ],
            });
        }
    ...
    async fn review_spec(&self, context: &ReviewContext) -> CouncilResult<JudgeVerdict> {
        ...
        let verdict = if security_score >= 0.8 {
            JudgeVerdict::Approve {
                ...
                reasoning: format!(
                    "High security score ({:.2}) indicates robust security controls",
                    security_score
                ),
            ...
            JudgeVerdict::Refine {
                confidence: security_score,
                reasoning: format!(
                    "Moderate security score ({:.2}) requires security improvements",
                    security_score
                ),
            ...
            JudgeVerdict::Reject {
                ...
                reasoning: format!(
                    "Low security score ({:.2}) indicates critical security concerns",
                    security_score
                ),
                critical_issues: issues,
                alternative_approaches: vec![
                    "Implement comprehensive security controls".to_string(),
