...
impl CawsQualityGateExecutor {
    ...
    pub async fn execute_quality_gates(
        ...
        let waived_violations: Vec<_> = violations.iter()
            .filter(|v| v.waived)
            .cloned()
            .collect();
        ...
        let context_str = report.get("context")
            .and_then(|c| c.as_str())
            .unwrap_or(context)
            .to_string();
        
        let files_scoped = report.get("files_scoped")
            .and_then(|f| f.as_u64())
    ...
    fn parse_violations(&self, report: &serde_json::Value) -> Result<Vec<QualityGateViolation>> {
        ...
        let violations_array = report.get("violations")
        ...
        for violation_json in violations_array {
            let gate = violation_json.get("gate")
                .and_then(|g| g.as_str())
                .unwrap_or("unknown")
                .to_string();
            
            let violation_type = violation_json.get("type")
                .and_then(|t| t.as_str())
                .unwrap_or("unknown")
                .to_string();
            
            let message = violation_json.get("message")
                .and_then(|m| m.as_str())
                .unwrap_or("")
                .to_string();
            
            let file = violation_json.get("file")
                .and_then(|f| f.as_str())
                .map(|f| f.to_string());
            
            // Check if violation is waived
            let waived_by = violation_json.get("waivedBy")
                .and_then(|w| w.as_str())
                .map(|w| w.to_string());
            
            let waived = waived_by.is_some();
            
            let waiver_title = violation_json.get("waiverTitle")
                .and_then(|t| t.as_str())
                .map(|t| t.to_string());
            
            let waiver_expires = violation_json.get("waiverExpires")
                .and_then(|e| e.as_str())
                .map(|e| e.to_string());
    ...
    fn parse_waivers(&self, report: &serde_json::Value) -> Result<Vec<WaiverInfo>> {
        let empty_vec = Vec::new();
        let waivers_obj = report.get("waivers")
        ...
        for waiver_json in waivers_obj {
            ...
            let gates = waiver_json.get("gates")
            ...
            let expires_at = waiver_json.get("expires_at")
                .and_then(|e| e.as_str())
                .unwrap_or("")
                .to_string();
