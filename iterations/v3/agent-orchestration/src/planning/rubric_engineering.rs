...
impl TaskSurface {
    ...
    pub fn classify(working_spec: &WorkingSpec, artifacts: Option<&ExecutionArtifacts>) -> Self {
        ...
        if title_lower.contains("performance")
            || desc_lower.contains("performance")
            || title_lower.contains("optimize")
        ...
        if title_lower.contains("infrastructure")
        ...
        if let Some(artifacts) = artifacts {
            ...
            let doc_file_count = artifacts
                .code_changes
                .new_files
                .iter()
                .filter(|f| {
                    f.path.ends_with(".md") || f.path.ends_with(".rst") || f.path.contains("docs/")
                })
                .count();
            let total_new_files = artifacts.code_changes.new_files.len();
...
impl RubricEngine {
    ...
    pub async fn calculate_weighted_score(
        ...
        let weighted_score = component_scores.evidence_completeness
    ...
    pub async fn adjust_rubric_weights(&self, surface: &TaskSurface) -> Result<()> {
        ...
        let surface_history: Vec<&PerformanceHistoryEntry> =
            history.iter().filter(|e| e.surface == *surface).collect();
        ...
        match &rubric.reward_adjustment.strategy {
            AdjustmentStrategy::ExponentialMovingAverage { alpha } => {
                self.adjust_with_ema(surface, &surface_history, *alpha, &rubric)
                    .await?;
            }
            AdjustmentStrategy::PerformanceCorrelation {
            ...
            AdjustmentStrategy::MultiArmedBandit { epsilon } => {
                self.adjust_with_bandit(surface, &surface_history, *epsilon, &rubric)
                    .await?;
            }
        }
    ...
    async fn adjust_with_ema(
        ...
        let successful: Vec<&PerformanceHistoryEntry> =
            history.iter().filter(|e| e.success).copied().collect();

        if successful.is_empty() {
            return Ok(());
        }

        // TODO: Use these averages in EMA calculation for v4
        let _avg_evidence = successful
            .iter()
            .map(|e| e.component_scores.evidence_completeness)
            .sum::<f64>()
            / successful.len() as f64;

        let _avg_budget = successful
            .iter()
            .map(|e| e.component_scores.budget_adherence)
            .sum::<f64>()
            / successful.len() as f64;

        let _avg_gate = successful
            .iter()
            .map(|e| e.component_scores.gate_integrity)
            .sum::<f64>()
            / successful.len() as f64;

        let _avg_provenance = successful
            .iter()
        ...
        if total_correlation > 0.0 {
            ...
            new_weights.evidence_completeness = (1.0 - learning_rate)
                * current_rubric.weights.evidence_completeness
                + learning_rate * new_weights.evidence_completeness;
            new_weights.budget_adherence = (1.0 - learning_rate)
                * current_rubric.weights.budget_adherence
                + learning_rate * new_weights.budget_adherence;
            new_weights.gate_integrity = (1.0 - learning_rate)
                * current_rubric.weights.gate_integrity
                + learning_rate * new_weights.gate_integrity;
            new_weights.provenance_clarity = (1.0 - learning_rate)
                * current_rubric.weights.provenance_clarity
            ...
            if let Some(rubric) = rubrics.get_mut(surface) {
                ...
                info!(
                    "Adjusted weights for {:?}: E={:.3}, B={:.3}, G={:.3}, P={:.3}",
                    surface,
                    rubric.weights.evidence_completeness,
    ...
    async fn adjust_with_correlation(
        ...
        self.adjust_with_ema(surface, history, 0.3, current_rubric)
            .await?;
        ...
        F2: Fn(&PerformanceHistoryEntry) -> f64,
    {
        ...
        let numerator: f64 = history
            .iter()
            .map(|e| (x_fn(e) - x_mean) * (y_fn(e) - y_mean))
            .sum();

        let x_variance: f64 = history.iter().map(|e| (x_fn(e) - x_mean).powi(2)).sum();

        let y_variance: f64 = history.iter().map(|e| (y_fn(e) - y_mean).powi(2)).sum();
    ...
    pub async fn get_performance_stats(&self, surface: &TaskSurface) -> Option<PerformanceStats> {
        ...
        let success_rate = surface_history.iter().filter(|e| e.success).count() as f64
            / surface_history.len() as f64;

        let avg_score = surface_history.iter().map(|e| e.total_score).sum::<f64>()
            / surface_history.len() as f64;

        let avg_quality = surface_history.iter().map(|e| e.quality_score).sum::<f64>()
            / surface_history.len() as f64;
