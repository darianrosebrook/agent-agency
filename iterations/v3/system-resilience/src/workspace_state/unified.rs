...
use super::context_generator::{ContextCriteria, ContextGenerator, WorkspaceContext};
...
use super::state_types::{
    StateId, WorkspaceConfig, WorkspaceDiff, WorkspaceError, WorkspaceResult, WorkspaceState,
};
...
impl UnifiedWorkspaceStateManager {
    ...
    pub fn new(
        ...
        let context_generator = if config
            .context_config
            .as_ref()
            .map(|c| c.enabled)
            .unwrap_or(false)
        ...
        let file_watcher_handler = if config
            .watch_config
            .as_ref()
            .map(|c| c.enabled)
            .unwrap_or(false)
        {
            Some(Arc::new(
                super::file_watcher_trait::FileWatcherEventHandler::new(
                    event_sender.clone(),
                    workspace_root.as_ref(),
                ),
            ))
    ...
    pub async fn generate_code_context(
        ...
        let context_generator = self.context_generator.as_ref().ok_or_else(|| {
            WorkspaceError::Configuration("Context generation not enabled".to_string())
        })?;

        let context = context_generator
            .generate_code_context(language, framework)
            .await?;
        let duration_ms = start_time.elapsed().as_millis() as u64;

        // Record metrics
        self.record_context_generation(ContextType::Code, duration_ms, context.files.len())
            .await;

        // Emit event
        let _ = self
            .event_sender
            .send(WorkspaceStateEvent::ContextGenerated {
    ...
    pub async fn generate_documentation_context(&self) -> Result<WorkspaceContext, WorkspaceError> {
        ...
        // Record metrics
        self.record_context_generation(
    ...
    pub async fn generate_config_context(&self) -> Result<WorkspaceContext, WorkspaceError> {
        ...
        // Record metrics
        self.record_context_generation(ContextType::Config, duration_ms, context.files.len())
            .await;
    ...
    pub async fn generate_context(
        ...
        // Record metrics
        self.record_context_generation(ContextType::General, duration_ms, context.files.len())
            .await;
    ...
    async fn record_context_generation(
        &self,
    ...
    async fn start_file_watching_internal(&mut self) -> Result<(), WorkspaceError> {
        let watch_config = self.config.watch_config.as_ref().ok_or_else(|| {
            WorkspaceError::Configuration("File watching config not found".to_string())
        })?;

        let handler = self.file_watcher_handler.as_ref().ok_or_else(|| {
            WorkspaceError::Configuration("File watcher handler not initialized".to_string())
        })?;
        ...
        let embedding_extensions: Vec<String> =
            watch_config.embedding_extensions.iter().cloned().collect(); // Clone vector
        ...
        self.file_watcher_handle = Some(tokio::spawn(async move {
            ...
            while let Ok(event) = event_receiver.recv().await {
                match event {
                    ...
                    | WorkspaceStateEvent::FileModified { path, state_id: _ } => {
                        ...
                        if watch_config_clone.auto_capture_state {
                            ...
                            tokio::task::spawn_blocking(move || {
                                ...
                                tracing::debug!(
                                    "Auto-capture skipped in async context (git2 Send safety)"
                                );
                            });
                        }
                        ...
                        if should_embed {
                            if let Some(embedding_svc) = embedding_service.clone() {
                                ...
                                let embedding_debounce_map_clone =
                                    Arc::clone(&embedding_debounce_map);
                                ...
                                tokio::spawn(async move {
                                    ...
                                    let debounce_duration =
                                        std::time::Duration::from_millis(watch_config_debounce);
                                    let now = std::time::Instant::now();

                                    let should_process = {
                                        let mut debounce_map =
                                            embedding_debounce_map_clone.write().await;
                                    ...
                                    if should_process {
                                        ...
                                        if let Ok(content) =
                                            tokio::fs::read_to_string(&full_path_clone).await
                                        {
                                            ...
                                            let start_time = std::time::Instant::now();
                                            let embedding_result = embedding_svc
                                                .generate_embedding(&text_for_embedding)
                                                .await;
                                            match embedding_result {
                                                Ok(embedding) => {
                                                    ...
                                                    let store_result = embedding_svc
                                                    ...
                                                    if let Err(e) = store_result {
                                                        ...
                                                        let mut m = metrics_clone.write().await;
                                                        let total =
                                                            m.embeddings.embeddings_generated;
                                                        m.embeddings.embeddings_generated += 1;
                                                        m.embeddings.files_embedded += 1;
                                                        m.embeddings.last_embedding_time =
                                                            Some(Utc::now());
                                                        m.embeddings.average_generation_time_ms = (m
                                                            .embeddings
                                                ...
                                                Err(e) => {
                                                    ...
                                                    // Emit failure event
                                                    let _ = event_sender_task.send(
                                                        WorkspaceStateEvent::EmbeddingGenerated {
                                                            path: path_clone.clone(),
                                                            success: false,
                                                            duration_ms: 0,
                                                        },
                    ...
                    WorkspaceStateEvent::FileDeleted {
                        path: _,
                        state_id: _,
                    } => {
    ...
    pub fn file_watcher_handler(
        &self,
    ) -> Option<Arc<super::file_watcher_trait::FileWatcherEventHandler>> {
    ...
    pub async fn generate_file_embedding(
        ...
        let embedding_service = self.embedding_service.as_ref().ok_or_else(|| {
            WorkspaceError::Configuration("Embedding service not configured".to_string())
        })?;
        ...
        let _ = self
            .event_sender
            .send(WorkspaceStateEvent::EmbeddingGenerated {
    ...
    pub async fn generate_and_store_file_embedding(
        &self,
        file_path: &Path,
        ...
        // Store embedding
        embedding_service
            .store_file_embedding(file_path.to_path_buf(), &content, embedding, Some(metadata))
            .await
            .map_err(|e| WorkspaceError::Embedding(e))?;
    ...
    pub async fn search_files_by_similarity(
        ...
        embedding_service
            .search_files_by_similarity(query, limit)
            .await
            .map_err(|e| WorkspaceError::Embedding(e))
    }
