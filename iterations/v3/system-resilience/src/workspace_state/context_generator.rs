...
impl ContextGenerator {
    ...
    pub async fn generate_documentation_context(&self) -> Result<WorkspaceContext, WorkspaceError> {
        ...
        let state = Arc::clone(&self.state_manager)
            .get_state(state_result.data)
            .await?;
    ...
    pub async fn generate_context(
        ...
        for (path, file_state) in &state.files {
            ...
            let is_config =
                self.matches_patterns(path, &vec!["*.toml", "*.yaml", "*.yml", "*.json"]);
            ...
            if !criteria.languages.is_empty() {
                let matches = criteria
                    .languages
                    .iter()
                    .any(|lang| self.matches_language(path, lang));
                if !matches {
                    continue;
                }
            }
    ...
    fn matches_framework(&self, path: &Path, framework: &str) -> bool {
        ...
        match framework.to_lowercase().as_str() {
            "react" => {
                path.to_string_lossy().contains("react")
