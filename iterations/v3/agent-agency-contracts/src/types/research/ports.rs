...
use super::dto::{Embedding, EntityKey};
...
pub trait KnowledgeBase: Send + Sync {
    ...
    fn lookup<'a>(
        &'a self,
        key: &'a EntityKey,
    ) -> BoxFuture<'a, Result<Option<String>, KnowledgeError>>;
    ...
    fn search<'a>(
        ...
        query: &'a str,
        limit: usize,
    ) -> BoxFuture<'a, Result<Vec<String>, KnowledgeError>>;

    /// Batch lookup multiple entities (prevents N+1)
    fn batch_lookup<'a>(
        &'a self,
        keys: &'a [EntityKey],
    ) -> BoxFuture<'a, Result<Vec<Option<String>>, KnowledgeError>>;
}
    ...
    fn test_embedding_provider_object_safety() {
        ...
        impl EmbeddingProvider for TestProvider {
            fn embed<'a>(
                ...
                Box::pin(async move { Ok(Embedding(vec![0.0; 128])) })
            }
