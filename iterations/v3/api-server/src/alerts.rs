...
use crate::rto_rpo_monitor::{RtoRpoMonitor, ComplianceAlert};
...
pub struct AlertManager {
    ...
    /// Integration with RTO/RPO monitor for compliance alerts
    rto_rpo_monitor: Option<Arc<RtoRpoMonitor>>,
...
impl AlertManager {
    ...
    pub fn new(rto_rpo_monitor: Option<Arc<RtoRpoMonitor>>) -> Self {
    ...
    async fn load_default_alert_definitions(&self) -> Result<(), String> {
        let definitions = vec![
            ...
            AlertDefinition {
                ...
                tags: ["system".to_string(), "performance".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                tags: ["system".to_string(), "memory".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                notification_channels: vec!["pagerduty-critical".to_string(), "slack-alerts".to_string()],
                escalation_policy: Some("critical-escalation".to_string()),
                tags: ["api".to_string(), "availability".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                auto_resolve_after: Some(3),
                notification_channels: vec!["pagerduty-critical".to_string(), "email-admin".to_string()],
                escalation_policy: Some("critical-escalation".to_string()),
                tags: ["database".to_string(), "availability".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                notification_channels: vec!["slack-alerts".to_string()],
                escalation_policy: Some("default-escalation".to_string()),
                tags: ["api".to_string(), "errors".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                notification_channels: vec!["email-admin".to_string(), "pagerduty-critical".to_string()],
                escalation_policy: Some("compliance-escalation".to_string()),
                tags: ["compliance".to_string(), "rto".to_string()].into_iter().collect(),
            },
            ...
            AlertDefinition {
                ...
                notification_channels: vec!["email-admin".to_string(), "pagerduty-critical".to_string()],
                escalation_policy: Some("compliance-escalation".to_string()),
                tags: ["compliance".to_string(), "rpo".to_string()].into_iter().collect(),
            },
        ];
        ...
        info!("Loaded {} default alert definitions", alert_definitions.len());
        Ok(())
    }
    ...
    async fn load_default_notification_channels(&self) -> Result<(), String> {
        let channels = vec![
            NotificationChannel {
                ...
                config: [
                    ...
                    ("to_addresses".to_string(), "admin@company.com,devops@company.com".to_string()),
                ].into_iter().collect(),
                enabled: true,
                retry_attempts: 3,
                retry_delay_secs: 60,
            ...
            NotificationChannel {
                ...
                config: [
                    ("webhook_url".to_string(), "https://hooks.slack.com/services/...".to_string()),
            ...
            NotificationChannel {
                ...
                config: [
                    ("integration_key".to_string(), "your_pagerduty_integration_key".to_string()),
                    ("api_endpoint".to_string(), "https://events.pagerduty.com/v2/enqueue".to_string()),
                ].into_iter().collect(),
                enabled: true,
                retry_attempts: 3,
                retry_delay_secs: 10,
        ...
        info!("Loaded {} default notification channels", notification_channels.len());
        Ok(())
    }
    ...
    async fn evaluate_alert_definition(&self, definition: &AlertDefinition) -> Result<(), String> {
        let condition_result = match &definition.condition {
            AlertCondition::MetricThreshold { metric_name, operator, threshold, duration_secs } => {
                self.evaluate_metric_threshold(metric_name, *operator, *threshold, *duration_secs).await
            }
            AlertCondition::ServiceHealth { service_name, expected_status } => {
                self.evaluate_service_health(service_name, expected_status).await
            }
            AlertCondition::ErrorRate { service_name, threshold_percent, time_window_secs } => {
                self.evaluate_error_rate(service_name, *threshold_percent, *time_window_secs).await
            }
            AlertCondition::ComponentUnavailable { component_name } => {
                self.evaluate_component_availability(component_name).await
            }
            AlertCondition::Custom { expression, parameters: _ } => {
    ...
    async fn evaluate_service_health(&self, service_name: &str, expected_status: &str) -> Result<bool, String> {
    ...
    async fn evaluate_error_rate(&self, service_name: &str, threshold_percent: f64, time_window_secs: u64) -> Result<bool, String> {
    ...
    async fn evaluate_custom_condition(&self, expression: &str) -> Result<bool, String> {
        match expression {
            "rto_violation_detected" => {
                if let Some(monitor) = &self.rto_rpo_monitor {
                    let status = monitor.get_compliance_status().await;
                    Ok(!status.rto_compliant)
                } else {
                    Ok(false)
                }
            }
    ...
    async fn trigger_or_update_alert(&self, definition: &AlertDefinition) -> Result<(), String> {
        ...
        if let Some(existing_alert) = active_alerts.values_mut()
            .find(|a| a.definition_id == definition.id && a.status == AlertStatus::Active)
        {
            ...
            // Send notification
            self.send_alert_notification(&alert, &definition.notification_channels).await?;

            // Record in history
            self.record_alert_history(&alert_id, AlertAction::Triggered, serde_json::json!({
                "definition_id": definition.id,
                "severity": format!("{:?}", definition.severity)
            })).await?;

            info!("Triggered new alert: {} ({:?})", alert.title, alert.severity);
        }
    ...
    async fn check_auto_resolve_alert(&self, definition_id: &str) -> Result<(), String> {
        ...
        if let Some(auto_resolve_after) = definition.auto_resolve_after {
            ...
            if let Some(alert) = active_alerts.values_mut()
                .find(|a| a.definition_id == definition_id && a.status == AlertStatus::Active)
            {
                ...
                if time_since_trigger.num_minutes() > 5 { // Mock: resolve after 5 minutes of no violations
                    alert.status = AlertStatus::Resolved;
                    alert.updated_at = Utc::now();

                    // Record in history
                    self.record_alert_history(&alert.id, AlertAction::Resolved, serde_json::json!({
                        "auto_resolved": true,
    ...
    async fn check_alert_escalation(&self, alert: &mut ActiveAlert) -> Result<(), String> {
        ...
        if new_level > alert.escalation_level {
            ...
            // Record escalation
            self.record_alert_history(&alert.id, AlertAction::Escalated, serde_json::json!({
                "from_level": alert.escalation_level - 1,
                "to_level": alert.escalation_level
            })).await?;

            info!("Escalated alert {} to level {}", alert.title, alert.escalation_level);
        }
    ...
    async fn send_alert_notification(&self, alert: &ActiveAlert, channels: &[String]) -> Result<(), String> {
    ...
    async fn record_alert_history(&self, alert_id: &str, action: AlertAction, details: serde_json::Value) -> Result<(), String> {
            ...
            if let Some(channel) = channels.get(channel_id) {
                if channel.enabled {
                    if let Err(e) = self.send_to_channel(channel, &request.message).await {
                        warn!("Failed to send notification to channel {}: {}", channel_id, e);
    ...
    async fn send_to_channel(&self, channel: &NotificationChannel, message: &str) -> Result<(), String> {
        match channel.channel_type {
            NotificationChannelType::Email => {
                self.send_email_notification(channel, message).await
            }
            NotificationChannelType::Slack => {
                self.send_slack_notification(channel, message).await
            }
            ...
            NotificationChannelType::SMS => {
                self.send_sms_notification(channel, message).await
            }
        }
    }

    /// Send email notification
    async fn send_email_notification(&self, _channel: &NotificationChannel, _message: &str) -> Result<(), String> {
    ...
    async fn send_slack_notification(&self, channel: &NotificationChannel, message: &str) -> Result<(), String> {
    ...
    async fn send_webhook_notification(&self, channel: &NotificationChannel, message: &str) -> Result<(), String> {
        if let Some(webhook_url) = channel.config.get("webhook_url") {
            let client = reqwest::Client::new();
            let payload = serde_json::json!({
    ...
    async fn send_pagerduty_notification(&self, _channel: &NotificationChannel, _message: &str) -> Result<(), String> {
    ...
    async fn send_sms_notification(&self, _channel: &NotificationChannel, _message: &str) -> Result<(), String> {
    ...
    pub async fn acknowledge_alert(&self, alert_id: &str, user_id: &str) -> Result<(), String> {
        ...
        if let Some(alert) = active_alerts.get_mut(alert_id) {
            ...
            // Record in history
            self.record_alert_history(alert_id, AlertAction::Acknowledged, serde_json::json!({
                "user_id": user_id
            })).await?;
    ...
    pub async fn resolve_alert(&self, alert_id: &str, user_id: &str) -> Result<(), String> {
        ...
        if let Some(alert) = active_alerts.get_mut(alert_id) {
            ...
            // Record in history
            self.record_alert_history(alert_id, AlertAction::Resolved, serde_json::json!({
                "user_id": user_id,
    ...
    pub async fn get_alert_statistics(&self) -> AlertStatistics {
        ...
        let resolved_alerts: Vec<_> = recent_history.iter()
            .filter(|h| matches!(h.action, AlertAction::Resolved))
            .collect();

        if !resolved_alerts.is_empty() {
            let total_resolution_time: i64 = resolved_alerts.iter()
                .filter_map(|h| {
                    // Find the corresponding trigger event
                    history.iter()
                        .find(|trigger| trigger.alert_id == h.alert_id && matches!(trigger.action, AlertAction::Triggered))
                        .map(|trigger| h.timestamp.signed_duration_since(trigger.timestamp).num_minutes())
                })
                .sum();

            stats.average_resolution_time_minutes = total_resolution_time as f64 / resolved_alerts.len() as f64;
        }
...
impl Clone for AlertManager {
    fn clone(&self) -> Self {
        Self {
            ...
            rto_rpo_monitor: self.rto_rpo_monitor.clone(),
...
mod tests {
    ...
    async fn test_metric_threshold_evaluation() {
        ...
        let result = manager.evaluate_metric_threshold("cpu_usage_percent", ThresholdOperator::GreaterThan, 80.0, 300).await;
        assert!(result.is_ok());
