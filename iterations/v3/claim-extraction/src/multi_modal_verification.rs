        ...
        let metadata_list =
            task::spawn_blocking(move || -> Result<Vec<(String, String, u64, bool)>> {
                ...
                for entry in WalkDir::new(&traversal_root)
                    ...
                    .into_iter()
                    .filter_entry(|e| {
                        !ignore_dirs.contains(e.file_name().to_str().unwrap_or(""))
                    })
                    .flatten()
                {
...
// [refactor candidate]: split into claim_extraction/verification/authority_validator.rs - authority indicators extraction and related data structures
    async fn extract_authority_indicators(
        ...
        if claim_lower.contains("machine learning") || claim_lower.contains("ai") {
            indicators.expertise_domains.push("machine_learning".to_string());
        }

        // Extract publication indicators
        if claim_lower.contains("published") || claim_lower.contains("paper") {
            indicators.publication_indicators.push("peer_reviewed".to_string());
        }
        if claim_lower.contains("conference") || claim_lower.contains("journal") {
            indicators.publication_indicators.push("academic".to_string());
        }
        if claim_lower.contains("blog") || claim_lower.contains("article") {
            indicators.publication_indicators.push("online".to_string());
        }
