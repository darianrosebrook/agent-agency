...
use crate::{
    ...
    storage::{SourceIntegrityStorage, PostgresSourceIntegrityStorage},
    tampering_detector::{TamperingDetector, TamperingDetectionResult},
    types::*,
};
...
impl SourceIntegrityService {
    ...
    pub async fn verify_source_integrity(
        ...
        debug!("Starting source integrity verification for source: {}", source_id);

        // Calculate content hash
        let (calculated_hash, hash_duration_ms) = self.hasher.calculate_hash_with_timing(content)?;
        debug!("Content hash calculated: {} (took {}ms)", calculated_hash, hash_duration_ms);

        // Get existing record if it exists
        let existing_record = self.storage.get_record_by_source(source_id, source_type).await?;
        ...
        let tampering_result = if self.config.tampering_detection_enabled {
            self.tampering_detector.detect_tampering(
                content,
        ...
        let hash_match = stored_hash.as_ref().map_or(true, |hash| hash == &calculated_hash);
        ...
        let record_id = if let Some(mut existing_record) = existing_record {
            ...
            for (key, value) in metadata {
                existing_record.verification_metadata.insert(key.clone(), value.clone());
            }
        ...
        self.storage.store_verification(&verification_record).await?;

        // Create alert if tampering detected and alerts are enabled
        if tampering_detected && self.config.alert_on_tampering {
            self.create_tampering_alert(&record_id, &tampering_result).await?;
        }
    ...
    pub async fn get_source_record(
        ...
        self.storage.get_record_by_source(source_id, source_type).await
    }
    ...
    pub async fn get_verification_history(
        ...
        if let Some(record) = self.get_source_record(source_id, source_type).await? {
            self.storage.get_verification_history(&record.id, limit).await
        } else {
            Ok(Vec::new())
        }
    }
    ...
    pub async fn get_statistics(
        ...
        self.storage.get_statistics(time_range_start, time_range_end).await
    }
    ...
    async fn create_tampering_alert(
        ...
        let alert_type = if tampering_result.indicators.contains(&TamperingIndicator::HashMismatch) {
        ...
        let alert_message = format!(
            ...
            tampering_result.indicators.iter()
                .map(|i| i.to_string())
                .collect::<Vec<_>>()
                .join(", ")
        );
        ...
        let alert_data = HashMap::from([
            ("confidence_score".to_string(), serde_json::Value::Number(
                serde_json::Number::from_f64(tampering_result.confidence_score).unwrap()
            )),
            ("detection_time_ms".to_string(), serde_json::Value::Number(
                serde_json::Number::from(tampering_result.detection_time_ms as u64)
            )),
            ("analysis_details".to_string(), serde_json::to_value(&tampering_result.analysis_details)?),
        ]);
...
pub trait SourceIntegrityVerifiable {
    ...
    async fn verify_integrity(&self, service: &SourceIntegrityService) -> Result<IntegrityVerificationResult>;
...
mod tests {
    ...
    impl SourceIntegrityStorage for MockStorage {
        ...
        async fn get_record_by_source(&self, _source_id: &str, _source_type: &SourceType) -> Result<Option<SourceIntegrityRecord>> {
            Ok(None)
        }
        ...
        async fn store_verification(&self, _verification: &CreateSourceIntegrityVerification) -> Result<Uuid> {
            Ok(Uuid::new_v4())
        }
        ...
        async fn get_verification_history(&self, _source_integrity_id: &Uuid, _limit: Option<i32>) -> Result<Vec<SourceIntegrityVerification>> {
            Ok(Vec::new())
        }
        
        async fn get_alerts(&self, _source_integrity_id: &Uuid, _limit: Option<i32>) -> Result<Vec<SourceIntegrityAlert>> {
            Ok(Vec::new())
        }
        
        async fn get_statistics(&self, _time_range_start: Option<DateTime<Utc>>, _time_range_end: Option<DateTime<Utc>>) -> Result<SourceIntegrityStats> {
    ...
    async fn test_verify_source_integrity_new_source() {
        ...
        let result = service.verify_source_integrity(
            "test_source",
            &SourceType::Content,
            "test content",
