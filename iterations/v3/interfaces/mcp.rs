...
impl McpServer {
    ...
    async fn handle_list_tasks(&self, request_id: Value, args: &serde_json::Map<String, Value>) -> Result<McpResponse, McpError> {
        ...
        // Get active executions from progress tracker
        let executions = if let Some(tracker) = &self.progress_tracker {
            tracker.get_active_executions().await
                .map_err(|e| McpError {
                    code: -32603,
        ...
        let filtered_executions: Vec<_> = executions.into_iter()
            .filter(|exec| {
                if let Some(filter) = status_filter {
                    format!("{:?}", exec.status).to_lowercase() == filter.to_lowercase()
                } else {
                    true
                }
            })
        ...
        let tasks: Vec<serde_json::Value> = filtered_executions.into_iter()
            .map(|exec| {
                serde_json::json!({
                    "task_id": exec.task_id,
                    "status": format!("{:?}", exec.status).to_lowercase(),
                    "progress_percentage": exec.completion_percentage,
                    "current_phase": exec.current_phase,
                    "started_at": exec.start_time,
                    "last_updated": exec.last_update,
                    "description": exec.metadata.get("description")
                        .and_then(|v| v.as_str())
