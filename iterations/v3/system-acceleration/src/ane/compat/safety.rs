...
pub mod memory {
    ...
    pub fn validate_allocation_size(elements: usize, element_size: usize) -> Result<()> {
        ...
        let total_bytes = elements.checked_mul(element_size)
            .ok_or_else(|| ANEError::InvalidInput("Integer overflow in allocation size calculation".to_string()))?;

        if total_bytes > MAX_ALLOCATION_BYTES {
            return Err(ANEError::InvalidInput(
                format!("Allocation too large: {} bytes exceeds safety limit of {} bytes",
    ...
    pub fn safe_array_access<T>(array: &[T], index: usize) -> Result<&T> {
        if index >= array.len() {
            return Err(ANEError::InvalidInput(
                format!("Array index {} out of bounds for array of length {}", index, array.len())
            ));
        }
        Ok(&array[index])
    }
    ...
    pub fn validate_string_length(s: &str, max_length: usize) -> Result<()> {
        if s.len() > max_length {
            return Err(ANEError::InvalidInput(
                format!("String length {} exceeds maximum allowed length {}", s.len(), max_length)
            ));
        }

        // Check for null bytes which would cause C string issues
        if s.contains('\0') {
            return Err(ANEError::InvalidInput("String contains null bytes".to_string()));
        }
...
pub mod dtype {
    ...
    pub fn dtype_size_bytes(dtype: &str) -> Result<usize> {
        match dtype {
            ...
            "U8" => Ok(1),
            "BOOL" => Ok(1),
            _ => Err(ANEError::InvalidInput(format!("Unknown data type: {}", dtype))),
        }
    }
...
pub mod shape {
    ...
    pub fn validate_shape(shape: &[usize]) -> Result<()> {
        ...
        for &dim in shape {
            total_elements = total_elements.checked_mul(dim as usize)
                .ok_or_else(|| ANEError::InvalidInput("Shape dimensions too large, would cause integer overflow".to_string()))?;
        }
        ...
        if total_elements > MAX_ELEMENTS {
            return Err(ANEError::InvalidInput(
                format!("Shape too large: {} elements exceeds limit of {}", total_elements, MAX_ELEMENTS)
            ));
        }
    ...
    pub fn shape_elements(shape: &[usize]) -> Result<usize> {
        ...
        for &dim in shape {
            total = total.checked_mul(dim as usize)
                .ok_or_else(|| ANEError::InvalidInput("Shape too large for calculation".to_string()))?;
        }
        Ok(total)
    }
}
