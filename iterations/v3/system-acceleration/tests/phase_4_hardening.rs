...
async fn test_phase_4_device_matrix() {
    ...
    match DeviceMatrix::detect_current_device() {
        Ok(device_caps) => {
            ...
            println!(
                "   - ANE Performance Score: {:.2}",
                device_caps.ane_performance_score
            );
            println!(
                "   - Memory Bandwidth: {:.0} GB/s",
                device_caps.memory_bandwidth_gbps
            );
            println!("   - Unified Memory: {} GB", device_caps.unified_memory_gb);
            println!("   - ANE Cores: {}", device_caps.ane_cores);
            println!(
                "   - Recommended Precision: {}",
                device_caps.recommended_precision
            );

            // Verify reasonable values
            assert!(
                device_caps.ane_performance_score >= 0.0
                    && device_caps.ane_performance_score <= 1.0,
                "ANE performance score should be between 0.0 and 1.0"
            );
            assert!(
                device_caps.unified_memory_gb > 0,
                "Unified memory should be > 0"
            );
            assert!(
                !device_caps.recommended_precision.is_empty(),
                "Recommended precision should not be empty"
            );
        }
        Err(e) => {
            println!(
                "⚠️ Device detection failed (expected on non-Apple Silicon): {}",
                e
            );
...
fn test_phase_4_platform_optimizations() {
    ...
    for device in &test_devices {
        ...
        if device.unified_memory_gb >= 64 {
            assert_eq!(
                optimizations.get("memory_pool_size"),
        ...
        if device.ane_performance_score > 0.8 {
            assert_eq!(
                optimizations.get("preferred_compute_units"),
                Some(&"all".to_string()),
...
async fn test_phase_4_circuit_breaker() {
    ...
    assert!(
        ...
        "Circuit breaker should allow attempts initially"
    );
    ...
    for _ in 0..5 {
        ...
        assert!(
            circuit_breaker.can_attempt(),
            "Should still allow attempts after success"
        );
    }
    ...
    for i in 0..3 {
        ...
        if i < 2 {
            assert!(
                circuit_breaker.can_attempt(),
                "Should allow attempts before threshold"
            );
        }
    }
    ...
    assert!(
        circuit_breaker.can_attempt(),
        "Circuit breaker should allow attempts after recovery timeout"
    );
    ...
    assert!(
        circuit_breaker.can_attempt(),
        "Should allow attempts after successful recovery"
    );
...
fn test_phase_4_inference_metrics() {
    ...
    assert_eq!(
        ...
        "Should start with zero inferences"
    );
    assert_eq!(
        summary.successful_inferences, 0,
        "Should start with zero successes"
    );
    ...
    let summary = metrics.get_summary();
    assert_eq!(
        summary.total_inferences, 3,
        "Should have 3 total inferences"
    );
    assert_eq!(
        summary.successful_inferences, 3,
        "Should have 3 successful inferences"
    );
    assert!(
        (summary.avg_latency_ms - 75.0).abs() < 1.0,
        "Average latency should be ~75ms"
    );
    ...
    let summary = metrics.get_summary();
    assert_eq!(
        summary.total_inferences, 5,
        "Should have 5 total inferences"
    );
    assert_eq!(
        summary.successful_inferences, 3,
        "Should still have 3 successful inferences"
    );
    assert_eq!(
        summary.failed_inferences, 1,
        "Should have 1 failed inference"
    );
...
fn test_phase_4_graceful_degradation() {
    ...
    let test_scenarios = vec![
        ...
        (
            "precision error",
            "Unsupported precision FP16",
            Some(DegradationStrategy::ReducePrecision),
        ),
        (
            "ANE error",
            "ANE acceleration failed",
    ...
    assert_eq!(
        ...
        Some(&"FP32".to_string()),
        "Should reduce precision to FP32"
    );
...
async fn test_phase_4_hardened_executor() {
    ...
    match HardenedInferenceExecutor::new() {
        Ok(executor) => {
            ...
            println!(
                "   Device: {}",
                executor.get_device_capabilities().chip_family
            );
            println!(
                "   ANE Score: {:.2}",
                executor.get_device_capabilities().ane_performance_score
            );

            // Test successful operation
            println!("2. Testing successful operation...");
            let result = executor
                .execute_inference(|| async {
            ...
            assert!(
                ...
                "Should timeout with slow operation"
            );
            // Note: We can't check exact error type since ANEError doesn't implement PartialEq

            let summary = metrics.get_summary();
            assert_eq!(
                summary.timeout_inferences, 1,
                "Should have recorded 1 timeout"
            );
        ...
        Err(e) => {
            println!(
                "⚠️ Hardened executor creation failed (expected on non-Apple Silicon): {}",
                e
            );
...
fn test_phase_4_health_monitoring() {
    ...
    assert_eq!(
        ...
        "Should be healthy after successes"
    );
    ...
    assert_eq!(
        ...
        "Should be degraded after failures"
    );
    ...
    assert_eq!(
        ...
        "Should be critical after many failures"
    );
...
fn test_phase_4_resource_management() {
    ...
    assert_eq!(
        ...
        0.0,
        "Should start with 0% memory usage"
    );
    assert!(
        manager.can_allocate(1024 * 1024 * 1024),
        "Should allow reasonable allocation"
    );

    // Test allocation
    println!("2. Testing resource allocation...");
    assert!(
        manager.allocate(1024 * 1024 * 1024).is_ok(),
        "Should allow 1GB allocation"
    ); // 1GB
    assert_eq!(
        manager.memory_usage_percent(),
        12.5,
        "Should be 12.5% usage (1GB of 8GB)"
    );
    ...
    assert!(
        manager.allocate(large_allocation).is_err(),
        "Should reject allocation that exceeds threshold"
    );
    ...
    assert!(
        ...
        "Should not trigger cleanup at 75%"
    );
    ...
    assert!(
        (manager.memory_usage_percent() - 62.5).abs() < 0.1,
        "Should be ~62.5% usage after freeing 2GB"
    );
...
async fn test_phase_4_comprehensive_hardening() {
    ...
    if has_device_support {
        ...
        assert!(
            ...
            "Should generate platform optimizations"
        );
        println!(
            "   ✅ Platform optimizations: {} settings applied",
            optimizations.len()
        );

        // Test resource management with device-aware sizing
        let resource_manager =
            resource_management::ResourceManager::new(device_caps.unified_memory_gb);
        println!(
            "   ✅ Resource management: {}GB memory limit configured",
    ...
    assert!(
        circuit_breaker.can_attempt(),
        "Circuit breaker should remain stable with successes"
    );
    ...
    let summary = metrics.get_summary();
    assert_eq!(
        summary.successful_inferences, 5,
        "Should record all successful inferences"
    );
