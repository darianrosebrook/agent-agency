...
impl ErrorRateTracker {
    ...
    pub fn record_request(&mut self, success: bool, error_message: Option<String>) {
        ...
        if !success {
            self.errors
                .push_back((now, error_message.unwrap_or_else(|| "unknown_error".into())));
        }
    ...
    fn cleanup_old_records(&mut self) {
        ...
        while let Some((t, _)) = self.errors.front() {
            if *t < cutoff {
        ...
        while let Some((t, _)) = self.total_requests.front() {
            if *t < cutoff {
    ...
    pub fn error_stats(&self) -> ErrorStats {
        ...
        let requests_1h = self
            .total_requests
            .iter()
            .filter(|(t, _)| *t > cutoff_1h)
            .count();
        let requests_24h = self
            .total_requests
            .iter()
        ...
        ErrorStats {
            ...
            error_rate_1h: if requests_1h > 0 {
                errors_1h as f64 / requests_1h as f64
            } else {
                0.0
            },
            error_rate_24h: if requests_24h > 0 {
                errors_24h as f64 / requests_24h as f64
            } else {
                0.0
            },
        }
