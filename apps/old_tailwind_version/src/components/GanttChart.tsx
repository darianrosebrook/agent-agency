...
import type { TimelineTask, ZoomLevel } from "./TimelineView";
import {
  Avatar,
  AvatarFallback,
  AvatarImage,
...
export function GanttChart({
  ...
  const [hoveredTask, setHoveredTask] = useState<string | null>(
    null,
  );
  ...
  const tasksByWorker = useMemo(() => {
    ...
    grouped.forEach((tasks, workerId) => {
      tasks.sort(
          ...
          if (daysBetween <= threshold) {
            ...
            if (currentGroup.length > 0) {
              groups.push({
                ...
                endDate:
                  currentGroup[currentGroup.length - 1].endDate,
                  ...
                  >
                    ...
                    {showDetailedView
                          ...
                          return (
                            <Tooltip key={task.id}>
                              <TooltipTrigger asChild>
                                <div
                                  className={`absolute top-2 h-12 rounded-lg border-l-4 ${getStatusColor(task.status)} px-3 py-1.5 cursor-pointer transition-all hover:shadow-lg hover:z-10 ${
                                    hoveredTask === task.id
                      ...
                      : // Grouped view - show combined task groups
                        (
                          groupedTasksByWorker.get(workerId) ||
                          []
                        ).map((group) => {
                          const position =
                            calculateTaskPosition(
