...
export function AIAgentsTabContent() {
  ...
  const toggleAgent = async (agentId: string) => {
    ...
    try {
      ...
      alert(`Failed to ${newEnabledState ? 'enable' : 'disable'} agent: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  };
            ...
            {agents.map((agent) => (
              <div key={agent.id} className={styles.agentCard}>
                ...
                <button
                  ...
                  className={cn(
                    styles.toggleSwitch,
                    agent.is_active ? styles.toggleSwitchEnabled : styles.toggleSwitchDisabled
                  )}
                  type="button"
                >
                  <div
                    className={cn(
                      styles.toggleThumb,
                      agent.is_active ? styles.toggleThumbEnabled : styles.toggleThumbDisabled
                    )}
                  />
                </button>
              </div>
