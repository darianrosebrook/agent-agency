...
/**
 * Database Setup and Migration Script for Agent Agency V2
 *
 * This script handles complete database setup for the V2 iteration including:
...
const CONFIG = {
  ...
  // V2 specific
  appUser: "agent_agency_app",
  appPassword: process.env.DB_APP_PASSWORD || "change_me_in_production",
};
...
class DatabaseSetup {
  ...
  async createRole() {
    try {
      ...
      if (roleCheck.rows.length > 0) {
        this.log(`Role '${CONFIG.appUser}' already exists`, "warning");
        return;
      }

      // Create role
      await this.superClient.query(`
        CREATE ROLE ${CONFIG.appUser} LOGIN
        PASSWORD '${CONFIG.appPassword}'
        NOSUPERUSER NOCREATEDB NOCREATEROLE;
      `);

      this.log(`Created database role '${CONFIG.appUser}'`, "success");
    } catch (error) {
    ...
    try {
      ...
      const checks = [
        ...
        },
        {
          name: "Core tables",
          check: async () => {
            const tables = ["tenants", "agent_profiles", "performance_events"];
  ...
  async cleanup() {
    try {
      ...
      // Drop role
      try {
        await this.superClient.query(`DROP ROLE IF EXISTS ${CONFIG.appUser}`);
        this.log(`Dropped role '${CONFIG.appUser}'`, "success");
      } catch (error) {
  ...
  printUsage() {
    console.log(`
Database Setup Script for Agent Agency V2
...
Environment Variables:
  ...
  DB_NAME              - Database name (default: agent_agency_v2)
  DB_USER              - App username (default: postgres)
  DB_PASSWORD          - App password (default: '')
  DB_SUPERUSER         - Superuser for setup (default: postgres)
  DB_SUPERPASSWORD     - Superuser password (default: DB_PASSWORD)
  DB_APP_PASSWORD      - App user password (default: change_me_in_production)
...
Examples:
  ...
  DB_NAME=my_app node scripts/setup-database.js init
